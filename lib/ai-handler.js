...
async function checkCache(msg, isNew) {
  try {
    const { data } = await supabase
      ...
      .select('ai_response, matched_products, expires_at')
      .eq('cache_key', buildCacheKey(msg, isNew))
      .single();
    if (!data) return null;
    // 如果有 expires_at 且已過期，視為無效
    if (data.expires_at && new Date(data.expires_at).getTime() < Date.now()) return null;
    return { cached_response: data.ai_response, matched_products: data.matched_products };
  } catch { return null; }
}
...
async function writeCache(msg, response, products, isNew) {
  try {
    await supabase.from('quickbuy_response_cache').upsert(
      {
        cache_key: buildCacheKey(msg, isNew),
        user_message_sample: msg.slice(0, 200),
...
async function getSystemPrompt() {
  try {
    ...
    const { data } = await supabase
      ...
      .limit(1)
      .maybeSingle();
    // value 是 jsonb，可能是 JSON string 或純文字
    const raw = data?.value;
    const val = typeof raw === 'string' ? raw : raw != null ? String(raw) : null;
