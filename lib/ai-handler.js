...
// ── 快取檢查 ──
async function checkCache(userMessage) {
  try {
    const { data } = await supabase
      .from('quickbuy_response_cache')
      .select('cached_response, matched_products')
      .eq('query_key', userMessage.toLowerCase().trim())
      .single();
    return data || null;
  } catch {
    return null;
...
// ── 快取寫入 ──
async function writeCache(userMessage, response, products) {
  try {
    await supabase.from('quickbuy_response_cache').upsert({
      query_key: userMessage.toLowerCase().trim(),
...
const SYSTEM_PROMPT = `你是 Snap-on 授權代理「易大立有限公司」的 Line 客服助理。
...
export async function generateAIResponse(userMessage, displayName, lineUserId) {
  ...
  try {
    ...
    // 6. 寫入快取
    if (products.length > 0) {
      await writeCache(userMessage, aiResponse, products);
    }
