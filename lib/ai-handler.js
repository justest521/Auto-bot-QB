...
const CHAT_HISTORY_LIMIT = 6; // 最近 6 則對話（3 組來回）
...
const CACHE_TTL_MS = 24 * 60 * 60 * 1000; // 24 小時快取過期
...
async function checkCache(userMessage, isNewCustomer) {
  try {
    ...
    if (data.updated_at) {
      ...
      if (age > CACHE_TTL_MS) return null; // 過期，不使用快取
    }
