...
/**
 * Escape PostgreSQL LIKE / ILIKE wildcard characters inside pattern values.
 * Use this for the search term inside %value% patterns so that user-supplied
 * `%` or `_` are treated as literals, not wildcards.
 * PostgREST uses `\` as the escape character in ILIKE patterns.
 */
...
/**
 * Get today's date string in Taiwan time (UTC+8), formatted as YYYY-MM-DD.
 * Use instead of new Date().toISOString().slice(0,10) to avoid off-by-one
 * errors near midnight in UTC.
 */
