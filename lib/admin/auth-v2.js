...
// Session 快取：避免每次 API 都查 DB（省 ~200ms）
...
export async function validateSession(token) {
  ...
  if (sessionCache.size > 100) {
    ...
    for (const [k, v] of sessionCache) {
      if (now - v.ts > SESSION_CACHE_TTL) sessionCache.delete(k);
    }
  }
...
export async function loginStep1(username, password) {
  ...
  // Password correct → check if first-login (skip OTP)
  if (user.must_change_password) {
    // First-time login: skip OTP, go straight to session but flag password change
    ...
    const session = await createSession(user.id, { headers: { get: () => null } });
  ...
  // OTP 開關：若環境變數 SKIP_OTP=true 或無 RESEND_API_KEY，跳過 OTP 直接登入
  const skipOtp = process.env.SKIP_OTP === 'true' || !process.env.RESEND_API_KEY;
  if (skipOtp) {
    await supabase.from('admin_users').update({
    ...
    const session = await createSession(user.id, { headers: { get: () => null } });
    return {
      ok: true, step: 'first_login', token: session.token, expiresAt: session.expiresAt,
      userId: user.id, mustChangePassword: false,
      user: { id: user.id, username: user.username, email: user.email, display_name: user.display_name, role: user.role?.code, role_label: user.role?.label },
    };
  }
