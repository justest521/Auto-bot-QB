...
export function toNumber(value) {
  ...
  // 移除貨幣符號前綴或後綴
  cleaned = cleaned.replace(/^(US\$|NT\$|NTS|NTD|TWD|USD|EUR|JPY|\$|¥|€|元)/i, '');
  cleaned = cleaned.replace(/(US\$|NT\$|NTS|NTD|TWD|USD|EUR|JPY|\$|¥|€|元)$/i, '');
...
export function isMissingRelationError(error) {
  ...
  // Exclude column-level errors like "column X of relation Y does not exist"
  if (/column\s+.*does not exist/i.test(msg)) return false;
  return /(relation|table).*does not exist|schema cache/i.test(msg);
}
