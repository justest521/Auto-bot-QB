...
const t = {
  ...
  fontSize: {
    h1: 22,        // 頁面主標題
    h2: 17,        // 區塊標題 / Modal 標題 (mobile 18)
    h3: 14,        // 卡片標題 / 欄位群組標題
    body: 14,      // 正文 / 表格內文 / 輸入框（CJK +1px 降低疲勞）
    caption: 13,   // 次要文字 / 表頭 / 小按鈕
    tiny: 12,      // Label / eyebrow / 極小文字
    badge: 12,     // 狀態標籤
  },
  // 字重
  fontWeight: {
...
const presets = {
  ...
  modalBody: (size = 'md') => ({
    ...
    boxShadow: '0 20px 60px rgba(8,12,20,0.25), 0 4px 16px rgba(8,12,20,0.08)',
  }),
  // Modal 標題列
  modalTitle: (isMobile) => ({
...
const S = {
  ...
  header: { minHeight: 54, background: t.color.bgCard, borderBottom: `1px solid ${t.color.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: `6px ${t.spacing.xxl}px`, position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 1px 3px rgba(0,0,0,0.03)' },
  content: { flex: 1, padding: `${t.spacing.xl}px ${t.spacing.xxl}px 32px`, minHeight: 'calc(100vh - 52px)' },

  // ── 卡片與面板 ──
  card: { background: t.color.bgCard, border: `1px solid ${t.color.border}`, borderRadius: t.radius.lg, padding: `${t.spacing.lg}px ${t.spacing.lg + 2}px`, marginBottom: t.spacing.md, boxShadow: '0 1px 2px rgba(0,0,0,0.04), 0 2px 8px rgba(0,0,0,0.04)', transition: 'all 0.25s ease' },
  panelMuted: { background: t.color.bgPanel, border: `1px solid ${t.color.border}`, borderRadius: 10, padding: '10px 14px' },

  // ── 表單元素 ──
  input: { background: t.color.bgCard, border: `1px solid ${t.color.border}`, borderRadius: t.radius.md, padding: '8px 12px', color: t.color.textPrimary, fontSize: t.fontSize.body, outline: 'none', width: '100%', boxSizing: 'border-box', fontFamily: t.font.base, transition: 'border-color 0.2s, box-shadow 0.2s' },
  btnPrimary: { background: t.color.brand, color: '#fff', border: 'none', borderRadius: t.radius.md, padding: '7px 16px', fontWeight: t.fontWeight.semibold, cursor: 'pointer', fontSize: t.fontSize.body, fontFamily: t.font.base, letterSpacing: 0.2, boxShadow: '0 1px 2px rgba(0,0,0,0.06), 0 1px 3px rgba(22,163,74,0.15)', transition: 'background 0.15s, box-shadow 0.15s, filter 0.15s' },
  ...
  pageTitle: { fontSize: t.fontSize.h1, fontWeight: t.fontWeight.bold, color: t.color.textPrimary, letterSpacing: -0.3, marginBottom: 2, lineHeight: 1.3 },
  pageDesc: { fontSize: t.fontSize.body, color: t.color.textMuted, lineHeight: 1.6, maxWidth: 760 },
  eyebrow: { fontSize: t.fontSize.tiny, color: t.color.brand, fontWeight: t.fontWeight.semibold, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: t.spacing.xs, fontFamily: t.font.mono },
