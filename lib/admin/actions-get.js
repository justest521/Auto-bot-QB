  ...
  switch (action) {
      ...
      case 'sale_detail': {
        ...
        try {
          ...
          const { data: shipmentsList } = await supabase.from('erp_shipments').select('id, shipment_no, carrier, tracking_no, status, ship_date, remark, created_at').or(shipOrClauses.join(',')).order('created_at', { ascending: true });
      ...
      case 'products': {
        ...
        const selectFields = lite
          ? 'item_number,description,tw_retail_price,tw_reseller_price,us_price,cost_price,stock_qty'
      ...
      case 'quick_receive_match': {
        ...
        const { data: product } = await supabase
          .from('quickbuy_products')
          .select('item_number, description, tw_retail_price, tw_reseller_price, cost_price, stock_qty')
