      ...
      case 'sale_detail': {
        ...
        try {
          ...
          if (items.length > 0) {
            ...
            if (itemNumbers.length > 0) {
              const { data: stockRows } = await supabase.from('erp_products').select('item_number, stock_qty').in('item_number', itemNumbers);
              const stockMap = {};
        ...
        const enriched = await Promise.all((data || []).map(async (a) => {
          try {
            ...
            if (a.doc_type === 'sale' && a.doc_id) {
              ...
              let rawItems = [];
              if (sale?.slip_number) {
                const { data: r1 } = await supabase.from('erp_order_items').select('item_number_snapshot, description_snapshot, qty, unit_price, line_total').like('sale_ref', `%${sale.slip_number}%`);
                rawItems = r1 || [];
              }
              if (rawItems.length === 0) {
                const { data: r2 } = await supabase.from('erp_order_items').select('item_number_snapshot, description_snapshot, qty, unit_price, line_total').eq('sale_ref', String(a.doc_id));
                rawItems = r2 || [];
              }
              const items = rawItems.map(i => ({ item_number: i.item_number_snapshot, description: i.description_snapshot, quantity: i.qty, unit_price: i.unit_price, subtotal: i.line_total }));
