      ...
      case 'document_chain': {
        ...
        if (orderPaymentRows.length > 0) {
          orderPaymentRows.forEach(p => {
            ...
            chain.push({ dot: '#16a34a', label: `收${payTypeLabels[p.payment_type] || '款'}`, ref: p.payment_number, refType: 'payment', detail: `${fmtAmt(p.amount)}（${ml}）`, time: p.confirmed_at || p.created_at, status: 'done' });
          });
        }
