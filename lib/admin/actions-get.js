...
export async function handleGetAction(action, searchParams) {
  ...
  switch (action) {
      ...
      case 'approvals': {
        ...
        // Helper: fetch customer's historical order summary
        const fetchCustomerHistory = async (customerId) => {
          ...
          try {
            ...
            return { order_count: orders.length, total_spent: totalSpent, recent_orders: orders.slice(0, 5).map(o => ({ order_no: o.order_no, amount: o.total_amount, date: o.created_at?.slice(0, 10), status: o.status })) };
          } catch (_) { return null; }
        };
