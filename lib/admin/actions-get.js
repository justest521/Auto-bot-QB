      ...
      case 'document_chain': {
        ...
        if (resolvedOrderId) {
          fetchPromises.push(
            ...
            supabase.from('erp_shipments').select('id, shipment_no, status, ship_date, created_at, erp_shipment_items(order_item_id, qty_shipped)').eq('order_id', resolvedOrderId).order('created_at').then(r => { shipmentRows = r.data || []; }),
        ...
        if (shipmentRows.length > 0) {
          ...
          shipmentRows.forEach(sh => {
            ...
            const itemNotes = (sh.erp_shipment_items || []).map(si => {
              ...
              return oi ? `${oi.item_number_snapshot} x${si.qty_shipped}` : `x${si.qty_shipped}`;
            });
