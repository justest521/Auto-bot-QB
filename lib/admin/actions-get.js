      ...
      case 'products': {
        ...
        if (trimmed && (!data || data.length === 0) && !error) {
          ...
          const { data: fallbackData, count: fallbackCount, error: fallbackErr } = await supabase
            .from('quickbuy_products')
            .select('item_number,description,us_price,tw_retail_price,tw_reseller_price,product_status,category,replacement_model,weight_kg,origin_country,image_url,stock_qty', { count: 'exact' })
