        ...
        if (order) {
          ...
          const [quoteRes, approvalsRes, shipmentsRes, relatedApprovalsRes] = await Promise.all([
            order.quote_id
              ...
              : { data: null },
            supabase.from('erp_approvals').select('status, created_at, approved_at, approved_by, remark').eq('doc_id', orderId.toString()).eq('doc_type', 'order').order('created_at', { ascending: true }),
