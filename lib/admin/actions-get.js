...
export async function handleGetAction(action, searchParams) {
  ...
  switch (action) {
      ...
      case 'document_chain': {
        ...
        if (resolvedOrderId) {
          fetchPromises.push(
            supabase.from('qb_sales_history').select('id, slip_number, status, sale_date, created_at').eq('source_id', resolvedOrderId).order('created_at').then(r => { saleRows = r.data || []; }),
            ...
            supabase.from('erp_order_payments').select('id, payment_number, payment_type, payment_method, amount, status, confirmed_at, created_at').eq('order_id', resolvedOrderId).eq('status', 'confirmed').order('created_at').then(r => { paymentRows = r.data || []; }),
        ...
        // Quote
        if (quoteData) {
          chain.push({ dot: '#16a34a', label: '報價', ref: quoteData.quote_no, refType: 'quote', time: quoteData.created_at, status: 'done' });
        }
        // Order
        if (orderData) {
          chain.push({ dot: '#16a34a', label: '訂單建立', ref: orderData.order_no, refType: 'order', time: orderData.created_at, status: 'done' });
          // Approval
        ...
        saleRows.forEach(sale => {
          ...
          chain.push({ dot: sc, label: '銷貨', ref: sale.slip_number, refType: 'sale', detail: saleStatusLabels[sk] || sk, time: sale.created_at, status: sk === 'paid' ? 'done' : 'current' });
        });
        // AR (Invoice)
        if (invoiceData) {
          const invPk = (invoiceData.payment_status || 'unpaid').toLowerCase();
          chain.push({ dot: invPk === 'paid' ? '#16a34a' : '#f59e0b', label: '應收帳款', ref: invoiceData.invoice_no, refType: 'invoice', detail: invPk === 'paid' ? '已付清' : invPk === 'partial' ? '部分付款' : '未付款', time: invoiceData.created_at, status: invPk === 'paid' ? 'done' : 'current' });
        }
        // Payments
        paymentRows.forEach(p => {
          const tl = payTypeLabels[p.payment_type] || '收款';
          const ml = payMethodLabels[p.payment_method] || p.payment_method || '';
          chain.push({ dot: '#16a34a', label: '付款', ref: p.payment_number, refType: 'payment', detail: `${tl} NT$${Number(p.amount || 0).toLocaleString()}（${ml}）`, time: p.confirmed_at || p.created_at, status: 'done' });
        });
        if (orderData && !paymentRows.length) {
        ...
        if (orderData) {
          ...
          const ps = (orderData.payment_status || '').toLowerCase();
          const ss = (orderData.shipping_status || '').toLowerCase();
          if (os === 'completed' || (ps === 'paid' && (ss === 'shipped' || ss === 'delivered'))) {
            chain.push({ dot: '#16a34a', label: '完成', detail: '訂單完成', status: 'done' });
          }
        }
