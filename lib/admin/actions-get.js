  ...
  switch (action) {
      ...
      case 'order_items_with_stock': {
        ...
        if (order) {
          ...
          if (order.quote_id) {
            const { data: linkedQuote } = await supabase.from('erp_quotes').select('quote_no, created_at').eq('id', order.quote_id).maybeSingle();
            if (linkedQuote) {
      ...
      case 'products': {
        ...
        let query = supabase
          .from('quickbuy_products')
          .select(
            'item_number,description,us_price,tw_retail_price,tw_reseller_price,product_status,category,replacement_model,weight_kg,origin_country,image_url,stock_qty,cost_price',
            { count: 'exact' }
          )
        ...
        if (trimmed) {
          ...
          // 模糊搜尋：料號 ilike + 品名 ilike
          query = query.or(`item_number.ilike.%${escaped}%,description.ilike.%${escaped}%`);
        }

        const { data, count, error } = await query;
        // Fallback: if or() returns nothing, try plain ilike on item_number only
        if (trimmed && (!data || data.length === 0) && !error) {
          const escaped2 = trimmed.replace(/['"]/g, '');
          const { data: fallbackData, count: fallbackCount, error: fallbackErr } = await supabase
            .from('quickbuy_products')
            .select('item_number,description,us_price,tw_retail_price,tw_reseller_price,product_status,category,replacement_model,weight_kg,origin_country,image_url,stock_qty,cost_price', { count: 'exact' })
            .ilike('item_number', `%${escaped2}%`)
            .order('item_number', { ascending: true })
            .range(0, limit - 1);
          if (!fallbackErr && fallbackData && fallbackData.length > 0) {
            return Response.json({ rows: fallbackData, total: fallbackCount || fallbackData.length, page, limit });
          }
        }
        if (error) return Response.json({ error: error.message }, { status: 500 });
