...
export async function handleGetAction(action, searchParams) {
  ...
  switch (action) {
      ...
      case 'procurement_center': {
        ...
        // 5b. Get current stock from quickbuy_products
        let stockMap = {};
        ...
        // Sort: latest PO date first (newest on top), then by status
        rows.sort((a, b) => (b.latest_po_date || '').localeCompare(a.latest_po_date || '') || a.item_number.localeCompare(b.item_number));
