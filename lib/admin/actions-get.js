      ...
      case 'order_items_with_stock': {
        ...
        if (order) {
          ...
          const [quoteRes, approvalsRes, shipmentsRes, relatedApprovalsRes] = await Promise.all([
            ...
            supabase.from('erp_shipments').select('shipment_no, created_at, erp_shipment_items(order_item_id, qty_shipped)').eq('order_id', orderId).order('created_at', { ascending: true }),
          ...
          if (shipmentsRes.data?.length) {
            ...
            for (const ship of shipmentsRes.data) {
              ...
              const shipItemDetail = (ship.erp_shipment_items || [])
                .map(si => `${oiLookup[si.order_item_id] || '?'} x${si.qty_shipped}`)
                .join('、');
