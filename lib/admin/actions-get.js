        ...
        const enriched = await Promise.all((data || []).map(async (a) => {
          try {
            ...
            if (a.doc_type === 'purchase_order' && a.doc_id) {
              const { data: po } = await supabase.from('erp_purchase_orders').select('id, po_no, total_amount, vendor:erp_vendors(vendor_name)').eq('id', a.doc_id).maybeSingle();
              ...
              return { ...a, amount: a.amount || po?.total_amount || null, vendor: vendorInfo, items };
            }
