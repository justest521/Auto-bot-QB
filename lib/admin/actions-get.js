  ...
  switch (action) {
      ...
      case 'order_items_with_stock': {
        ...
        if (order) {
          ...
          // 查詢 PO 相關到貨記錄（stock-in events）
          if (uniquePOs.length > 0) {
            const poIds = uniquePOs.map(p => p.id);
            const { data: stockInItems } = await supabase
              ...
              .in('stock_in_id', (await supabase.from('erp_stock_ins').select('id').in('po_id', poIds)).data?.map(s => s.id) || [])
              .order('created_at', { referencedTable: 'erp_stock_ins', ascending: true });

            if (stockInItems?.length) {
              // Group by stock_in_id to show one event per stock-in
              const siGrouped = {};
              for (const si of stockInItems) {
