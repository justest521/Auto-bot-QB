...
export async function handleGetAction(action, searchParams) {
  ...
  switch (action) {
      ...
      case 'invoice_allocations': {
        ...
        if (inv?.order_id) {
          const { data: opRows } = await supabase.from('qb_payments')
      ...
      case 'document_chain': {
        ...
        if (orderPaymentRows.length > 0) {
          orderPaymentRows.forEach(p => {
            ...
            chain.push({ dot: '#16a34a', label: `收${payTypeLabels[p.payment_type] || '款'}`, ref: p.payment_number, refType: 'payment', detail: `${fmtAmt(p.amount)}（${ml}）`, time: p.confirmed_at || p.created_at, status: 'done', proof_url: p.proof_url || null, verified: p.verified || false, payment_id: p.id });
          });
        }
