...
export async function handleGetAction(action, searchParams) {
  ...
  switch (action) {
      ...
      case 'invoices': {
        ...
        // Look up invoice_date + invoice_type from erp_invoices by invoice_no
        ...
        if (invNos.length > 0) {
          const { data: invs } = await supabase
            .from('erp_invoices')
            .select('invoice_no, invoice_date, invoice_type')
            .in('invoice_no', invNos);
          (invs || []).forEach(inv => {
            invDateMap[inv.invoice_no] = inv.invoice_date || null;
            invTypeMap[inv.invoice_no] = inv.invoice_type || 'B2B';
          });
        }
        ...
        const rows = (sales || []).map(s => ({
          ...
          invoice_date:  s.invoice_number ? (invDateMap[s.invoice_number] || null) : null,
          invoice_type:  s.invoice_number ? (invTypeMap[s.invoice_number] || 'B2B') : null,
