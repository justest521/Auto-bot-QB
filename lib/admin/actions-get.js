...
export async function handleGetAction(action, searchParams) {
  ...
  switch (action) {
      ...
      case 'inventory': {
        ...
        let query = supabase
          .from('quickbuy_products')
          .select('item_number,description,category,stock_qty,safety_stock,product_status,image_url,tw_retail_price,tw_reseller_price,us_price,weight_kg,origin_country,replacement_model', { count: 'exact' })
          .order('item_number', { ascending: true })
          .range(offset, offset + limit - 1);
