  ...
  switch (action) {
      ...
      case 'products': {
        ...
        if (trimmed) {
          ...
          // 模糊搜尋：料號 ilike + 品名 ilike + 全文搜尋 (fallback)
          const orParts = [`item_number.ilike.%${escaped}%`, `description.ilike.%${escaped}%`];
          // 嘗試全文搜尋 (只對英文/數字有效)
          const ftsTokens = escaped.split(/\s+/).filter(Boolean);
          if (ftsTokens.length > 0 && /^[a-zA-Z0-9\s\-\/]+$/.test(escaped)) {
            orParts.push(`search_text.fts.${ftsTokens.join(' & ')}`);
          }
