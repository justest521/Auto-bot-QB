...
export async function handleGetAction(action, searchParams) {
  ...
  switch (action) {
      ...
      case 'invoices': {
        ...
        const buildBase = () => {
          let q2 = supabase.from('qb_sales_history');
        ...
        const [totalAmtRes, invoicedRes, notInvoicedRes] = await Promise.all([
          buildBase().select('total'),
          buildBase().select('id', { count: 'exact', head: true }).not('invoice_number', 'is', null),
          buildBase().select('id', { count: 'exact', head: true }).is('invoice_number', null),
        ]);
