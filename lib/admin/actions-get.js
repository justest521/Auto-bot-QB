...
export async function handleGetAction(action, searchParams) {
  ...
  switch (action) {
      ...
      case 'document_chain': {
        ...
        let receiptRows = [], allocationRows = [];
        const fetchPromises = [];
        if (resolvedOrderId) {
          fetchPromises.push(
        ...
        // Receipt payments (from allocation or receipt rows)
        if (allocationRows.length > 0) {
          allocationRows.forEach(a => {
            ...
            chain.push({ dot: '#16a34a', label: '收款', ref: rcptNo, refType: 'payment', detail: `${fmtAmt(a.allocated_amount)}（${ml}）`, time: a.created_at, status: 'done' });
          });
        } else if (receiptRows.length > 0) {
          receiptRows.forEach(r => {
            const ml = payMethodLabels[r.payment_method] || r.payment_method || '';
            chain.push({ dot: '#16a34a', label: '收款', ref: r.receipt_no, refType: 'payment', detail: `${fmtAmt(r.total_amount)}（${ml}）`, time: r.created_at, status: 'done' });
          });
        } else if (invoiceData) {
          const invPk = (invoiceData.payment_status || 'unpaid').toLowerCase();
          if (invPk === 'paid') {
