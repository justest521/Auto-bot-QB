        ...
        const enriched = await Promise.all((data || []).map(async (a) => {
          try {
            if (a.doc_type === 'order' && a.doc_id) {
              ...
              const costMap = {};
              if (itemNos.length) {
                const { data: prods } = await supabase.from('quickbuy_products').select('item_number, tw_reseller_price').in('item_number', itemNos);
                (prods || []).forEach(p => { costMap[p.item_number] = Number(p.tw_reseller_price || 0); });
              }
