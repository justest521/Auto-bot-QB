...
import { escapeLikePattern, escapePostgrestValue, taiwanToday } from '@/lib/security/sanitize';
...
export async function handleGetAction(action, searchParams) {
  ...
  switch (action) {
          ...
          if (lineUserIds.length > 0) {
            ...
            if (displayNames.length > 0) {
              ...
              const orFilter = uniqueDisplayNames
                .flatMap((name) => {
                  const safe = escapeLikePattern(escapePostgrestValue(name));
                  return [
                    `name.ilike.%${safe}%`,
      ...
      case 'formal_customers': {
        ...
        try {
          let customerStageReady = true;
          let data, count, error, stageReady;

          if (search) {
            // Search mode: fetch candidates then sort by total spending descending
            const candidateBuilder = (columns) => {
              let query = supabase
                .from('erp_customers')
                .select(columns, { count: 'exact' })
                .or(`customer_code.ilike.%${search}%,name.ilike.%${search}%,company_name.ilike.%${search}%,phone.ilike.%${search}%,tax_id.ilike.%${search}%`);
              if (salesPersonFilter) query = query.eq('sales_person', salesPersonFilter);
              return query.limit(200);
            };
            const candidateRes = await runErpCustomerQuery(candidateBuilder);
            error = candidateRes.error;
            stageReady = candidateRes.stageReady;
            const candidates = candidateRes.data || [];
            count = candidates.length;

            if (candidates.length > 0) {
              const customerIds = candidates.map(c => c.id);
              // Sort by order count (採購次數) desc, then by total amount as tiebreaker
              ...
              for (const row of (orderStats || [])) {
                ...
                totalByCustomer[row.customer_id] = (totalByCustomer[row.customer_id] || 0) + Number(row.total_amount || 0);
              }
              candidates.sort((a, b) => {
                const cntDiff = (countByCustomer[b.id] || 0) - (countByCustomer[a.id] || 0);
                if (cntDiff !== 0) return cntDiff;
                return (totalByCustomer[b.id] || 0) - (totalByCustomer[a.id] || 0);
              });
              // Attach order_count for display in dropdown
              candidates.forEach(c => { c._order_count = countByCustomer[c.id] || 0; });
            }
            data = candidates.slice(offset, offset + limit);
          } else {
            const queryBuilder = (columns) => {
              let query = supabase
                .from('erp_customers')
                .select(columns, { count: 'exact' })
            ...
            const res = await runErpCustomerQuery(queryBuilder);
            data = res.data; count = res.count; error = res.error; stageReady = res.stageReady;
          }
          if (error) return Response.json({ error: error.message }, { status: 500 });
          customerStageReady = stageReady;
      ...
      case 'customer_detail': {
        ...
        try {
          ...
          if (displayName) {
            const safeDisplayName = escapeLikePattern(escapePostgrestValue(displayName));
            const { data: candidateRows, error: candidateError } = await runErpCustomerQuery((columns) =>
              supabase
                .from('erp_customers')
                .select(columns)
                .or(`name.ilike.%${safeDisplayName}%,company_name.ilike.%${safeDisplayName}%,display_name.ilike.%${safeDisplayName}%`)
            );
      ...
      case 'order_items_with_stock': {
        ...
        const { data: posBySourceId } = await supabase
          ...
          .contains('source_order_ids', [orderId])
          .order('created_at', { ascending: false });
        ...
        if (posBySourceId?.length) {
          ...
          const { data: posByRemark } = await supabase
            .from('erp_purchase_orders')
            .select('id, po_no, status, total_amount, po_date, created_at')
            .ilike('remark', `%${escapeLikePattern(orderData.order_no)}%`)
            .order('created_at', { ascending: false });
        ...
        const timeline = [];
        const { data: order } = await supabase.from('erp_orders').select('*, customer:erp_customers!customer_id(id, name, company_name, phone, tax_id, line_user_id, line_notify_enabled)').eq('id', orderId).maybeSingle();
        const orderHasLine = !!(order?.customer?.line_user_id);

        // Build order_data for frontend (includes customer info, status, etc.)
        let orderDataForFrontend = null;
        if (order) {
        ...
        if (order) {
          ...
          // 收集銷貨單 + 採購單的 ID，用來批次查詢相關審核記錄
          const relatedDocIds = [
            ...(linkedSales || []).map(s => String(s.id)),
            ...uniquePOs.map(p => String(p.id)),
          ];

          const [quoteRes, approvalsRes, shipmentsRes, relatedApprovalsRes] = await Promise.all([
            order.quote_id
              ? supabase.from('erp_quotes').select('quote_no, created_at').eq('id', order.quote_id).maybeSingle()
              : { data: null },
            supabase.from('erp_approvals').select('status, created_at, approved_at, approved_by, remark').eq('doc_id', orderId.toString()).eq('doc_type', 'order').neq('status', 'withdrawn').order('created_at', { ascending: true }),
            supabase.from('erp_shipments').select('shipment_no, created_at, erp_shipment_items(order_item_id, item_number, qty_shipped)').eq('order_id', orderId).order('created_at', { ascending: true }),
            relatedDocIds.length > 0
              ? supabase.from('erp_approvals').select('doc_type, doc_id, status, created_at, approved_at, approved_by, remark').in('doc_id', relatedDocIds).in('doc_type', ['sale', 'purchase_order']).order('created_at', { ascending: true })
              : { data: [] },
          ]);

          // 建立 doc_id → approval[] 的查詢表
          const relatedApprovalMap = {};
          for (const ap of (relatedApprovalsRes.data || [])) {
            if (!relatedApprovalMap[ap.doc_id]) relatedApprovalMap[ap.doc_id] = [];
            relatedApprovalMap[ap.doc_id].push(ap);
          }
          ...
          for (const po of uniquePOs) {
            ...
            const poApprovals = relatedApprovalMap[String(po.id)] || [];
            for (const ap of poApprovals) {
              timeline.push({ event: `採購單送審 ${po.po_no}`, time: ap.created_at, status: 'done' });
              if (ap.status === 'approved') {
                timeline.push({ event: `採購單已核准 ${po.po_no}`, time: ap.approved_at || ap.created_at, status: 'done', by: ap.approved_by });
              } else if (ap.status === 'rejected') {
                timeline.push({ event: `採購單已駁回 ${po.po_no}`, time: ap.approved_at || ap.created_at, status: 'rejected', by: ap.approved_by });
              } else {
                timeline.push({ event: `採購單審核中 ${po.po_no}`, time: null, status: 'pending' });
              }
            }
          }

          // 查詢訂單品項的到貨記錄（stock-in events）— 用 item_number 匹配，因為快速進貨 po_id 可能為 NULL
          if (itemNumbers.length > 0) {
            const { data: stockInItems } = await supabase
              .from('erp_stock_in_items')
              .select('item_number, qty_received, stock_in_id, erp_stock_ins(id, stock_in_no, created_at, po_id)')
              .in('item_number', itemNumbers)
              .order('created_at', { referencedTable: 'erp_stock_ins', ascending: true });

            // Only show stock-ins that happened after the order was created
            const orderCreatedAt = order.created_at ? new Date(order.created_at) : null;
            const relevantItems = (stockInItems || []).filter(si => {
              if (!orderCreatedAt || !si.erp_stock_ins?.created_at) return false;
              return new Date(si.erp_stock_ins.created_at) >= orderCreatedAt;
            });

            if (relevantItems.length > 0) {
              // Group by stock_in_id to show one event per stock-in
              const siGrouped = {};
              for (const si of relevantItems) {
                const siId = si.stock_in_id;
                if (!siGrouped[siId]) siGrouped[siId] = { stock_in: si.erp_stock_ins, items: [] };
                siGrouped[siId].items.push(si);
              }
              for (const [, group] of Object.entries(siGrouped)) {
                const itemBadges = group.items.map(i => `${i.item_number} x${i.qty_received}`).join('、');
                timeline.push({
                  event: `已到貨 ${group.stock_in?.stock_in_no || ''}`,
                  detail: itemBadges,
                  time: group.stock_in?.created_at,
                  status: 'done',
                  icon: '📦',
                });
              }
          ...
          // 重用已查過的 linkedSales，不再重複查；加入銷貨單審核進度
          if (linkedSales?.length) {
            for (const sale of linkedSales) {
              timeline.push({ event: `建立銷貨單 ${sale.slip_number}`, time: sale.created_at, status: 'done' });
              const saleApprovals = relatedApprovalMap[String(sale.id)] || [];
              for (const ap of saleApprovals) {
                timeline.push({ event: `銷貨單送審 ${sale.slip_number}`, time: ap.created_at, status: 'done' });
                if (ap.status === 'approved') {
                  timeline.push({ event: `銷貨單已核准 ${sale.slip_number}`, time: ap.approved_at || ap.created_at, status: 'done', by: ap.approved_by });
                } else if (ap.status === 'rejected') {
                  timeline.push({ event: `銷貨單已駁回 ${sale.slip_number}`, time: ap.approved_at || ap.created_at, status: 'rejected', by: ap.approved_by });
                } else {
                  timeline.push({ event: `銷貨單審核中 ${sale.slip_number}`, time: null, status: 'pending' });
                }
              }
            }
          }
          if (shipmentsRes.data?.length) {
            // Build order_item_id → item_number lookup
            const oiLookup = Object.fromEntries((items || []).map(i => [i.id, i.item_number_snapshot]));
            // Also build total shipped qty for backorder calculation
            const totalShippedByOi = {};
            const totalShippedByItemNo = {};
            for (const ship of shipmentsRes.data) {
              const shipLineNote = orderHasLine ? '✅ 已發送 LINE 出貨通知' : '⚠️ 未綁定 LINE，未推播';
              // Build shipped item detail string
              const shipItemDetail = (ship.erp_shipment_items || [])
                .map(si => {
                  const name = oiLookup[si.order_item_id] || si.item_number || '?';
                  if (si.order_item_id) totalShippedByOi[si.order_item_id] = (totalShippedByOi[si.order_item_id] || 0) + Number(si.qty_shipped || 0);
                  if (si.item_number) totalShippedByItemNo[si.item_number] = (totalShippedByItemNo[si.item_number] || 0) + Number(si.qty_shipped || 0);
                  return `${name} x${si.qty_shipped}`;
                })
                .join('、');
              timeline.push({ event: `建立出貨單 ${ship.shipment_no}`, detail: shipItemDetail || null, time: ship.created_at, status: 'done', note: shipLineNote, line_sent: orderHasLine });
            }
            // 欠貨：計算尚未出完的品項
            const backorderItems = (items || []).filter(oi => {
              const shipped = totalShippedByOi[oi.id] || totalShippedByItemNo[oi.item_number_snapshot] || 0;
              return shipped < Number(oi.qty || 0);
            });
            if (backorderItems.length > 0) {
              const boNotes = backorderItems.map(oi => {
                const shipped = totalShippedByOi[oi.id] || totalShippedByItemNo[oi.item_number_snapshot] || 0;
                return `${oi.item_number_snapshot} 欠 ${Number(oi.qty) - shipped}`;
              });
              timeline.push({ event: '欠貨', detail: `待出貨：${boNotes.join('、')}`, status: 'backorder' });
            }
          }
        }

        return Response.json({ items: finalItems, linked_sales: linkedSales || [], linked_pos: uniquePOs, timeline, order_data: orderDataForFrontend });
      }
      ...
      case 'orders': {
        ...
        try {
          // If searching by customer name, find matching customer IDs first
          let customerSearchIds = null;
          if (search) {
            const { data: matchedCustomers } = await runErpCustomerQuery((columns) =>
              supabase.from('erp_customers').select('id').or(`name.ilike.%${search}%,company_name.ilike.%${search}%`).limit(200)
            );
            customerSearchIds = (matchedCustomers || []).map(c => c.id);
          }
          ...
          if (search) {
            // Build OR filter: match order fields OR customer IDs
            const fieldFilters = `order_no.ilike.%${search}%,status.ilike.%${search}%,payment_status.ilike.%${search}%,shipping_status.ilike.%${search}%,remark.ilike.%${search}%,sales_person.ilike.%${search}%`;
            if (customerSearchIds && customerSearchIds.length > 0) {
              query = query.or(`${fieldFilters},customer_id.in.(${customerSearchIds.join(',')})`);
            } else {
              query = query.or(fieldFilters);
            }
          }
          // Support comma-separated status values (for tab group filtering)
          if (statusFilter) {
          ...
          const ACTIVE_ORDER_STATUSES = ['draft', 'pending_approval', 'confirmed', 'processing'];
          const summary = rows.reduce((acc, row) => {
            const s = String(row.status || '').toLowerCase();
            if (s !== 'cancelled') acc.total_amount += Number(row.total_amount || 0);
            if (ACTIVE_ORDER_STATUSES.includes(s)) acc.pending_count += 1;
            return acc;
          }, { total_amount: 0, pending_count: 0 });

          // Tab group counts (parallel HEAD queries for each group)
          const buildBaseCount = (statuses) => {
            let cq = supabase.from('erp_orders').select('*', { count: 'exact', head: true });
            if (statuses.length > 0) cq = statuses.length === 1 ? cq.eq('status', statuses[0]) : cq.in('status', statuses);
            if (dateFrom) cq = cq.gte('order_date', dateFrom);
            if (dateTo)   cq = cq.lte('order_date', dateTo);
            return cq;
          };
          const [actionRes, progressRes, shippedRes, cancelRes] = await Promise.all([
            buildBaseCount(['pending', 'pending_approval']),
            buildBaseCount(['confirmed', 'processing', 'purchasing']),
            buildBaseCount(['shipped', 'completed']),
            buildBaseCount(['cancelled', 'rejected']),
          ]);
          const tab_counts = {
          ...
          return Response.json({ rows, total: count || 0, page, limit, summary, tab_counts, table_ready: true });
        } catch (err) {
          console.error('[orders] Error:', err?.message || err);
          return Response.json({ rows: [], total: 0, page, limit, summary: { total_amount: 0, pending_count: 0 }, table_ready: false, _error: err?.message });
        }
      }
      ...
      case 'sale_detail': {
        ...
        try {
          ...
          if (invoiceNumber) {
            const [{ data: invoiceRow }, { data: erpInvRow }] = await Promise.all([
              supabase.from('qb_invoices').select('*').eq('invoice_number', invoiceNumber).limit(1),
              supabase.from('erp_invoices').select('invoice_type,buyer_tax_id,buyer_name,carrier_type,carrier_id,invoice_date').eq('invoice_no', invoiceNumber).limit(1),
            ]);

            invoice = invoiceRow?.[0] || null;
            // Merge e-invoice fields from erp_invoices
            if (erpInvRow?.[0]) {
              const ei = erpInvRow[0];
              invoice = invoice || {};
              invoice.invoice_type  = ei.invoice_type  || 'B2B';
              invoice.buyer_tax_id  = ei.buyer_tax_id  || '';
              invoice.buyer_name    = ei.buyer_name    || '';
              invoice.carrier_type  = ei.carrier_type  || '';
              invoice.carrier_id    = ei.carrier_id    || '';
              invoice.invoice_date  = ei.invoice_date  || invoice.invoice_date || null;
            }
          ...
          const { data: shipmentsList } = await supabase.from('erp_shipments').select('id, shipment_no, carrier, tracking_no, status, ship_date, remark, created_at, erp_shipment_items(order_item_id, product_id, qty_shipped)').or(shipOrClauses.join(',')).order('created_at', { ascending: true });

          // Enrich erp_shipment_items with item_number for cross-table matching in frontend.
          // order_item_id may be qb_order_items.id (new-code) OR erp_order_items.id (old-code).
          if (shipmentsList && shipmentsList.length > 0) {
            const allOrderItemIds = [];
            shipmentsList.forEach(sh => {
              (sh.erp_shipment_items || []).forEach(si => {
                if (si.order_item_id) allOrderItemIds.push(si.order_item_id);
              });
            });
            if (allOrderItemIds.length > 0) {
              const uniqueIds = [...new Set(allOrderItemIds)];
              // Check both tables — qb_order_items (new path) and erp_order_items (old path)
              const [{ data: qbItems }, { data: erpItems }] = await Promise.all([
                supabase.from('qb_order_items').select('id, item_number').in('id', uniqueIds),
                supabase.from('erp_order_items').select('id, item_number_snapshot').in('id', uniqueIds),
              ]);
              const itemNoById = {};
              (qbItems || []).forEach(i => { if (i.item_number) itemNoById[i.id] = i.item_number; });
              (erpItems || []).forEach(i => { if (i.item_number_snapshot && !itemNoById[i.id]) itemNoById[i.id] = i.item_number_snapshot; });
              shipmentsList.forEach(sh => {
                sh.erp_shipment_items = (sh.erp_shipment_items || []).map(si => ({
                  ...si,
                  item_number: si.order_item_id ? (itemNoById[si.order_item_id] || null) : null,
                }));
              });
            }
          }
          ...
          // Fetch linked payments via erp_order_id
          let payments = [];
          if (first.erp_order_id) {
            const { data: payRows } = await supabase
              .from('qb_payments')
              .select('id, payment_number, payment_type, payment_method, amount, confirmed_at, created_at, proof_url, verified')
              .eq('erp_order_id', first.erp_order_id)
              .eq('status', 'confirmed')
              .order('created_at', { ascending: true });
            const payTypeLabels = { deposit: '訂金', partial: '部分付款', final: '尾款', full: '全額付款' };
            const payMethodLabels = { cash: '現金', transfer: '匯款', check: '支票', monthly: '月結' };
            payments = (payRows || []).map(p => ({
              id: p.id,
              payment_number: p.payment_number,
              amount: Number(p.amount || 0),
              type: payTypeLabels[p.payment_type] || p.payment_type || '收款',
              method: payMethodLabels[p.payment_method] || p.payment_method || '匯款',
            ...
            for (const p of payments) {
              const verifiedTag = p.verified ? ' ✓核帳' : '';
              timeline.push({
                event: `付款 ${p.payment_number}`,
                time: p.confirmed_at || p.created_at,
                status: 'done',
                detail: `${p.type} NT$${Number(p.amount).toLocaleString()}（${p.method}）${verifiedTag}`,
                proof_url: p.proof_url,
                payment_id: p.id,
              });
            }
          }
          sale.proof_url = first.proof_url || null;
          sale.erp_order_id = first.erp_order_id || null;
          sale.payment_status = first.payment_status || null;
      ...
      case 'inventory': {
        ...
        let query = supabase
          .from('quickbuy_products')
          .select('item_number,description,category,stock_qty,safety_stock,cost_price,product_status,image_url,tw_retail_price,tw_reseller_price,us_price,weight_kg,origin_country,replacement_model', { count: 'exact' })
          .order('item_number', { ascending: true })
          .range(offset, offset + limit - 1);
      ...
      case 'shipments': {
        ...
        let query = supabase
          .from('erp_shipments')
          .select('*, customer:erp_customers!customer_id(id, name, company_name)', { count: 'exact' })
          .order('created_at', { ascending: false })
          .range(offset, offset + limit - 1);
      ...
      case 'stock_ins': {
        const search = (searchParams.get('search') || '').trim();
        const statusFilter = (searchParams.get('status') || '').trim();
        const vendorFilter = (searchParams.get('vendor_id') || '').trim().replace(/^undefined$/, '');
        const page = parseInt(searchParams.get('page') || '1', 10);
        const limit = parseLimit(30, 100);
        const offset = (page - 1) * limit;
        ...
        if (dateFrom) query = query.gte('stock_in_date', dateFrom);
        if (dateTo) query = query.lte('stock_in_date', dateTo);
      ...
      case 'vendor_payments': {
        const search = (searchParams.get('search') || '').trim();
        const statusFilter = (searchParams.get('status') || '').trim();
        const poIdFilter = (searchParams.get('po_id') || '').trim();
        const page = parseInt(searchParams.get('page') || '1', 10);
        const limit = parseLimit(30, 100);
        const offset = (page - 1) * limit;
      ...
      case 'vendor_payables': {
        ...
        const [unpaidRes, paidRes, overdueRes] = await Promise.all([
          ...
          supabase.from('erp_vendor_payables').select('total_amount, paid_amount').in('payment_status', ['unpaid', 'partial']).lt('due_date', taiwanToday()),
        ]);
      ...
      case 'stock_adjustments': {
        ...
        let itemsMap = {};
        if (ids.length) {
          const { data: allItems } = await supabase.from('erp_stock_adjustment_items').select('*').in('adjustment_id', ids);
          (allItems || []).forEach(it => { if (!itemsMap[it.adjustment_id]) itemsMap[it.adjustment_id] = []; itemsMap[it.adjustment_id].push(it); });
        }
        const rows = (data || []).map(r => ({ ...r, items: itemsMap[r.id] || [] }));
      ...
      case 'stock_transfers': {
        ...
        const { data, count, error } = await supabase.from('erp_stock_transfers').select('*', { count: 'exact' })
          .order('created_at', { ascending: false }).range(offset, offset + limit - 1);
        if (error) { if (isMissingRelationError(error)) return missingRelationResponse(error, 'erp_stock_transfers'); return Response.json({ error: error.message }, { status: 500 }); }

        // Fetch items for each transfer
        const ids = (data || []).map(r => r.id);
        let itemsMap = {};
        if (ids.length) {
          const { data: allItems } = await supabase.from('erp_stock_transfer_items').select('*').in('transfer_id', ids);
          (allItems || []).forEach(it => { if (!itemsMap[it.transfer_id]) itemsMap[it.transfer_id] = []; itemsMap[it.transfer_id].push(it); });
        }
        const rows = (data || []).map(r => ({ ...r, items: itemsMap[r.id] || [] }));
        return Response.json({ rows, total: count || 0, page, limit });
      }
      ...
      /* ===================== 組合單 ===================== */
      case 'stock_assemblies': {
        const page = parseInt(searchParams.get('page') || '1', 10);
        const limit = parseLimit(30, 100);
        const offset = (page - 1) * limit;

        const { data, count, error } = await supabase.from('erp_stock_assemblies').select('*', { count: 'exact' })
          .order('created_at', { ascending: false }).range(offset, offset + limit - 1);
        if (error) { if (isMissingRelationError(error)) return missingRelationResponse(error, 'erp_stock_assemblies'); return Response.json({ error: error.message }, { status: 500 }); }

        // Fetch items for each assembly
        const ids = (data || []).map(r => r.id);
        let itemsMap = {};
        if (ids.length) {
          const { data: allItems } = await supabase.from('erp_stock_assembly_items').select('*').in('assembly_id', ids);
          (allItems || []).forEach(it => { if (!itemsMap[it.assembly_id]) itemsMap[it.assembly_id] = []; itemsMap[it.assembly_id].push(it); });
        }
        const rows = (data || []).map(r => ({ ...r, items: itemsMap[r.id] || [] }));
        return Response.json({ rows, total: count || 0, page, limit });
      }
      ...
      /* ===================== 進銷存月度趨勢 (PSI enhanced) ===================== */
      case 'psi_monthly_trend': {
        // Returns last 12 months of sales, purchases, returns for charts
        const months = 12;
        const rows = [];
        const now = new Date();
        for (let i = months - 1; i >= 0; i--) {
          ...
          rows.push({ month: from.slice(0, 7), from, to });
        }

        // Fetch all sales & purchases in the 12-month window
        const windowFrom = rows[0].from;
        const windowTo = rows[rows.length - 1].to;
        const [salesRes, purchRes, retRes] = await Promise.all([
          supabase.from('qb_sales_history').select('sale_date,total,cost,gross_profit').gte('sale_date', windowFrom).lte('sale_date', windowTo),
          supabase.from('erp_stock_ins').select('stock_in_date,total_amount').eq('status', 'confirmed').gte('stock_in_date', windowFrom).lte('stock_in_date', windowTo),
          supabase.from('erp_returns').select('return_date,refund_amount').gte('return_date', windowFrom).lte('return_date', windowTo),
        ]);

        const trend = rows.map(r => {
          const sSales = (salesRes.data || []).filter(s => s.sale_date >= r.from && s.sale_date <= r.to);
          const sPurch = (purchRes.data || []).filter(p => p.stock_in_date >= r.from && p.stock_in_date <= r.to);
          const sRet = (retRes.data || []).filter(rt => rt.return_date >= r.from && rt.return_date <= r.to);
          return {
            month: r.month,
            sales: sSales.reduce((s, x) => s + Number(x.total || 0), 0),
            cost: sSales.reduce((s, x) => s + Number(x.cost || 0), 0),
            profit: sSales.reduce((s, x) => s + Number(x.gross_profit || 0), 0),
            purchases: sPurch.reduce((s, x) => s + Number(x.total_amount || 0), 0),
            returns: sRet.reduce((s, x) => s + Number(x.refund_amount || 0), 0),
            count: sSales.length,
          };
        });

        // Top 10 products by sales amount (all time or recent 12mo)
        const { data: topProducts } = await supabase.from('qb_sales_history').select('item_number,description,total,gross_profit').gte('sale_date', windowFrom).lte('sale_date', windowTo).order('total', { ascending: false }).limit(500);
        const productMap = {};
        (topProducts || []).forEach(r => {
          const k = r.item_number || r.description;
          if (!productMap[k]) productMap[k] = { item: k, desc: r.description, amount: 0, profit: 0, count: 0 };
          productMap[k].amount += Number(r.total || 0);
          productMap[k].profit += Number(r.gross_profit || 0);
          productMap[k].count += 1;
        });
        const topItems = Object.values(productMap).sort((a, b) => b.amount - a.amount).slice(0, 10);

        return Response.json({ trend, top_products: topItems });
      }
      ...
      case 'dealer_order_detail': {
        ...
        if (!ord) return Response.json({ error: '訂單不存在' }, { status: 404 });

        // Fetch dealer user info
        let dealer = null;
        if (ord.dealer_user_id) {
          const { data: du } = await supabase
            .from('erp_dealer_users')
            .select('id, display_name, role, company_name, phone, email, line_user_id')
            .eq('id', ord.dealer_user_id)
            .maybeSingle();
          dealer = du || null;
        }

        // Fetch order items with live stock check
        ...
        const itemNumbers = (rawItems || []).map(i => i.item_number_snapshot).filter(Boolean);
        let stockMap = {};
        if (itemNumbers.length) {
          const { data: prods } = await supabase
            .from('quickbuy_products')
            .select('item_number, stock_qty, description')
            .in('item_number', itemNumbers);
          stockMap = Object.fromEntries((prods || []).map(p => [p.item_number, p]));
        }

        const items = (rawItems || []).map(item => {
          const prod = stockMap[item.item_number_snapshot] || {};
          const stockQty = prod.stock_qty ?? 0;
          const needed = item.qty || 0;
          let stock_status = 'sufficient';
          let shortage = 0;
          if (stockQty <= 0) { stock_status = 'no_stock'; shortage = needed; }
          else if (stockQty < needed) { stock_status = 'partial'; shortage = needed - stockQty; }
          return { ...item, stock_qty: stockQty, stock_status, shortage };
        });

        return Response.json({ order: { ...ord, dealer }, items });
      }
      ...
      case 'po_items': {
        ...
        if (po) {
          ...
          // Check if PO came from an order → fetch customer info
          let poCustomerName = '';
          if (po.source_order_ids && Array.isArray(po.source_order_ids) && po.source_order_ids.length > 0) {
            ...
            const { data: linkedOrder } = await supabase.from('erp_orders').select('order_no, created_at, customer:erp_customers(name, company_name)').eq('id', sourceOrderId).maybeSingle();
            if (linkedOrder) {
              timeline.push({ event: `來自訂單 ${linkedOrder.order_no}`, time: linkedOrder.created_at, status: 'done' });
              poCustomerName = linkedOrder.customer?.company_name || linkedOrder.customer?.name || '';
            }
          }
        ...
        return Response.json({ items: enrichedData || [], timeline, customer_name: poCustomerName });
      }
      ...
      case 'po_item_allocation': {
        ...
        // Check if approval is enabled — if off, include draft orders too
        let approvalOn = true;
        try { const ef = await getQuickbuyConfigEntry('erp_features'); approvalOn = ef?.order_approval !== false; } catch {}
        const allocationStatuses = approvalOn ? ['confirmed', 'processing'] : ['draft', 'confirmed', 'processing'];
      ...
      /* ===================== 發票管理 ===================== */
      case 'invoices': {
        // 以銷貨單 (qb_sales_history) 為單位列表，保留發票號碼/日期/業務/金額
        const page = parseInt(searchParams.get('page') || '1', 10);
        const limit = parseLimit(30, 100);
        const offset = (page - 1) * limit;
        const q        = (searchParams.get('search')    || '').trim().slice(0, 200);
        const dateFrom = (searchParams.get('date_from') || '').trim();
        const dateTo   = (searchParams.get('date_to')   || '').trim();

        // Main query: qb_sales_history has invoice_number + sales_person directly
        let query = supabase
          .from('qb_sales_history')
          .select('id, slip_number, sale_date, created_at, invoice_number, customer_name, sales_person, total, total_amount', { count: 'exact' })
          .order('sale_date', { ascending: false })
          .order('created_at', { ascending: false })   // secondary: newest within same date first
          .range(offset, offset + limit - 1);
        ...
        if (q) {
          ...
          query = query.or(`slip_number.ilike.%${esc}%,customer_name.ilike.%${esc}%,invoice_number.ilike.%${esc}%`);
        }

        const { data: sales, count, error } = await query;
        if (error) return Response.json({ error: error.message }, { status: 500 });

        // Look up invoice_date + invoice_type + buyer_name + buyer_tax_id from erp_invoices by invoice_no
        const invNos = [...new Set((sales || []).map(s => s.invoice_number).filter(Boolean))];
        let invMap = {}; // invoice_no → { invoice_date, invoice_type, buyer_name, buyer_tax_id }
        if (invNos.length > 0) {
          const { data: invs } = await supabase
            .from('erp_invoices')
            .select('invoice_no, invoice_date, invoice_type, buyer_name, buyer_tax_id')
            .in('invoice_no', invNos);
          (invs || []).forEach(inv => {
            invMap[inv.invoice_no] = {
              invoice_date:  inv.invoice_date  || null,
              invoice_type:  inv.invoice_type  || 'B2B',
              buyer_name:    inv.buyer_name    || null,
              buyer_tax_id:  inv.buyer_tax_id  || null,
            };
          });
        }

        const rows = (sales || []).map(s => {
          const inv = s.invoice_number ? (invMap[s.invoice_number] || {}) : {};
          return {
            id:            s.id,
            ...
            invoice_no:    s.invoice_number || null,
            invoice_date:  inv.invoice_date  || null,
            invoice_type:  inv.invoice_type  || (s.invoice_number ? 'B2B' : null),
            buyer_name:    inv.buyer_name    || null,
            buyer_tax_id:  inv.buyer_tax_id  || null,
            total_amount:  Number(s.total || s.total_amount || 0),
          };
        });

        // Summary aggregates for the filtered period
        const buildBase = (sel, opts) => {
          let q2 = supabase.from('qb_sales_history').select(sel, opts);
          if (dateFrom) q2 = q2.gte('sale_date', dateFrom);
          if (dateTo)   q2 = q2.lte('sale_date', dateTo);
          return q2;
        };
        const [totalAmtRes, invoicedRes, notInvoicedRes] = await Promise.all([
          buildBase('total'),
          buildBase('id', { count: 'exact', head: true }).not('invoice_number', 'is', null),
          buildBase('id', { count: 'exact', head: true }).is('invoice_number', null),
        ]);
        const totalAmount = (totalAmtRes.data || []).reduce((s, r) => s + Number(r.total || 0), 0);

        return Response.json({
          rows, total: count || 0, page, limit,
          summary: {
            total_amount:      totalAmount,
            invoiced_count:    invoicedRes.count  || 0,
            not_invoiced_count: notInvoicedRes.count || 0,
          },
        });
      }
      ...
      case 'accounts_receivable': {
        ...
        // Fetch sales_person from linked orders
        const orderIds = [...new Set((data || []).map(r => r.order_id).filter(Boolean))];
        let orderSalesMap = {};
        if (orderIds.length > 0) {
          const { data: ordersRes } = await supabase.from('erp_orders').select('id, sales_person').in('id', orderIds);
          (ordersRes || []).forEach(o => { orderSalesMap[o.id] = o.sales_person || ''; });
        }

        // Fetch payment verified status per order
        let verifiedMap = {}; // order_id → { all_verified, verified_by }
        if (orderIds.length > 0) {
          const { data: payments } = await supabase.from('qb_payments')
            .select('erp_order_id, verified, verified_by')
            .in('erp_order_id', orderIds)
            .eq('status', 'confirmed');
          const byOrder = {};
          (payments || []).forEach(p => {
            if (!byOrder[p.erp_order_id]) byOrder[p.erp_order_id] = [];
            byOrder[p.erp_order_id].push(p);
          });
          for (const [oid, pays] of Object.entries(byOrder)) {
            const allVerified = pays.length > 0 && pays.every(p => p.verified);
            verifiedMap[oid] = { all_verified: allVerified, verified_by: pays.find(p => p.verified)?.verified_by || null, payment_count: pays.length };
          }
        }

        // Fetch sale_date from qb_sales_history via sale_id
        const saleIds = [...new Set((data || []).map(r => r.sale_id).filter(Boolean))];
        let saleDateMap = {};
        if (saleIds.length > 0) {
          const { data: salesRes } = await supabase.from('qb_sales_history').select('id, sale_date').in('id', saleIds);
          (salesRes || []).forEach(s => { saleDateMap[s.id] = s.sale_date; });
        }

        const rows = (data || []).map(r => {
          // erp_invoices.paid_amount 是唯一授權來源（由付款驗核流程自動維護）
          const invoicePaid = Number(r.paid_amount || 0);
          const total = Number(r.total_amount || 0);
          const displayStatus = invoicePaid >= total && total > 0 ? 'paid' : r.payment_status;
          return {
            ...r,
            customer_name: r.customer?.company_name || r.customer?.name || '-',
            sales_person: r.order_id ? (orderSalesMap[r.order_id] || '') : '',
            sale_date: r.sale_id ? (saleDateMap[r.sale_id] || null) : null,
            paid_amount_display: invoicePaid,
            balance: Math.max(0, total - invoicePaid),
            payment_status: displayStatus,
            verified_status: verifiedMap[r.order_id] || null,
          };
        });

        // Summary: use rows (which already have paid_amount_display with 訂金) for accurate totals
        const allTotal = rows.reduce((s, r) => s + Number(r.total_amount || 0), 0);
        const allPaid = rows.reduce((s, r) => s + Number(r.paid_amount_display || r.paid_amount || 0), 0);
        const allUnpaid = Math.max(0, allTotal - allPaid);
        const achieveRate = allTotal > 0 ? Math.round((allPaid / allTotal) * 100) : 0;

        return Response.json({
          rows, total: count || 0, page, limit,
          summary: {
            total_receivable: allTotal,
            total_paid: allPaid,
            total_unpaid: allUnpaid,
      ...
      case 'invoice_allocations': {
        ...
        const { data, error } = await supabase.from('erp_payment_allocations')
          .select('*, receipt:erp_payment_receipts!receipt_id(id, receipt_no, receipt_date, proof_url)')
        ...
        const rows = (data || []).map(r => ({
          ...
          proof_url: r.receipt?.proof_url || null,
        }));
        // Also fetch order-level payments (訂金等) linked to the same order
        ...
        let orderDeposits = [];
        const { data: inv } = await supabase.from('erp_invoices').select('order_id').eq('id', invoiceId).maybeSingle();
        if (inv?.order_id) {
          const [{ data: opRows }, { data: saleProofRows }] = await Promise.all([
            supabase.from('qb_payments')
              .select('id, payment_number, payment_type, payment_method, amount, confirmed_at, created_at, proof_url, verified, verified_by, verified_at')
              .eq('erp_order_id', inv.order_id).eq('status', 'confirmed').order('created_at'),
            // Fallback: 銷貨單補傳的憑證存在 qb_sales_history.proof_url
            supabase.from('qb_sales_history').select('proof_url').eq('source_id', inv.order_id).not('proof_url', 'is', null).limit(1),
          ]);
          const saleFallbackProof = saleProofRows?.[0]?.proof_url || null;
          orderDeposits = (opRows || []).map(p => ({
            id: p.id,
            receipt_no: p.payment_number,
            date: p.confirmed_at || p.created_at,
            amount: Number(p.amount || 0),
            type: payTypeLabels[p.payment_type] || p.payment_type || '-',
            method: payMethodLabels[p.payment_method] || p.payment_method || '-',
            is_order_payment: true,
            proof_url: p.proof_url || saleFallbackProof,
            verified: p.verified || false,
            verified_by: p.verified_by,
            verified_at: p.verified_at,
          }));
        }
        return Response.json({ rows, orderDeposits });
      }
      ...
      case 'approvals': {
        ...
        let query = supabase.from('erp_approvals').select('*').neq('status', 'withdrawn').order('created_at', { ascending: false });
        if (statusFilter) query = query.eq('status', statusFilter);
        ...
        const enriched = await Promise.all((data || []).map(async (a) => {
          try {
            ...
            if (a.doc_type === 'purchase_order' && a.doc_id) {
              const { data: po } = await supabase.from('erp_purchase_orders').select('id, po_no, total_amount, source_order_ids, vendor:erp_vendors(vendor_name)').eq('id', a.doc_id).maybeSingle();
              ...
              const vendorInfo = po?.vendor ? { company_name: po.vendor.vendor_name } : null;
              // 從 source_order_ids 關聯客戶名
              let poCustomer = null;
              if (po?.source_order_ids?.length > 0) {
                const { data: srcOrder } = await supabase.from('erp_orders').select('customer:erp_customers(name, company_name)').eq('id', po.source_order_ids[0]).maybeSingle();
                if (srcOrder?.customer) poCustomer = { company_name: srcOrder.customer.company_name || srcOrder.customer.name };
              }
              return { ...a, amount: a.amount || po?.total_amount || null, vendor: vendorInfo, customer: poCustomer, items };
            }
      ...
      case 'document_chain': {
        ...
        let receiptRows = [], allocationRows = [], orderPaymentRows = [], orderItemRows = [];
        const fetchPromises = [];
        if (resolvedOrderId) {
          fetchPromises.push(
            supabase.from('qb_sales_history').select('id, slip_number, status, sale_date, total, created_at, proof_url').eq('source_id', resolvedOrderId).order('created_at').then(r => { saleRows = r.data || []; }),
            ...
            supabase.from('erp_shipments').select('id, shipment_no, status, ship_date, created_at, erp_shipment_items(order_item_id, item_number, qty_shipped)').eq('order_id', resolvedOrderId).order('created_at').then(r => { shipmentRows = r.data || []; }),
            supabase.from('erp_purchase_orders').select('id, po_no, status, po_date, created_at').contains('source_order_ids', [resolvedOrderId]).order('created_at').then(r => { poRows = r.data || []; }),
            supabase.from('qb_payments').select('id, payment_number, payment_type, payment_method, amount, status, confirmed_at, created_at, proof_url, verified, verified_by, verified_at').eq('erp_order_id', resolvedOrderId).eq('status', 'confirmed').order('created_at').then(r => { orderPaymentRows = r.data || []; }),
            supabase.from('erp_order_items').select('id, item_number_snapshot, qty').eq('order_id', resolvedOrderId).then(r => { orderItemRows = r.data || []; }),
          );
        }
        ...
        // Order payments (訂金/部分付款 from qb_payments) — shown BEFORE invoice
        const orderDepositTotal = orderPaymentRows.reduce((s, p) => s + Number(p.amount || 0), 0);
        // Fallback proof: 銷貨單補傳的憑證 (qb_sales_history.proof_url)
        const saleFallbackProof = saleRows.map(s => s.proof_url).find(Boolean) || null;
        if (orderPaymentRows.length > 0) {
          orderPaymentRows.forEach(p => {
            ...
            chain.push({ dot: '#16a34a', label: `收${payTypeLabels[p.payment_type] || '款'}`, ref: p.payment_number, refType: 'payment', detail: `${fmtAmt(p.amount)}（${ml}）`, time: p.confirmed_at || p.created_at, status: 'done', proof_url: p.proof_url || saleFallbackProof, verified: p.verified || false, payment_id: p.id });
          });
        }
        // AR (Invoice) with financial summary — factor in order deposits
        if (invoiceData) {
          const invPk = (invoiceData.payment_status || 'unpaid').toLowerCase();
          const invTotal = Number(invoiceData.total_amount || 0);
          const arPaid = Number(invoiceData.paid_amount || 0);
          // arPaid 已由付款驗核流程（actions-post 3704）從 qb_payments 更新，不再另加 orderDepositTotal
          const effectivePaid = arPaid;
          const effectiveBal = Math.max(0, invTotal - effectivePaid);
          const allPaid = effectiveBal <= 0;
          let invDetail = `應收 ${fmtAmt(invTotal)}`;
          if (orderDepositTotal > 0) {
            invDetail += ` · 已收訂金 ${fmtAmt(orderDepositTotal)}`;
          }
          if (arPaid > 0) {
            invDetail += ` · 已沖帳 ${fmtAmt(arPaid)}`;
          }
          if (!allPaid) {
            invDetail += ` · 尾款 ${fmtAmt(effectiveBal)}`;
          } else {
            invDetail += ' · 已結清';
          }
          chain.push({ dot: allPaid ? '#16a34a' : '#f59e0b', label: '應收帳款', ref: invoiceData.invoice_no, refType: 'invoice', detail: invDetail, time: invoiceData.created_at, status: allPaid ? 'done' : 'current' });
        }
        // Remaining balance indicator after invoice (if deposits don't fully cover)
        if (orderPaymentRows.length > 0) {
          const orderTotal = Number(orderData?.total_amount || 0);
          const arPaid = invoiceData ? Number(invoiceData.paid_amount || 0) : 0;
          // arPaid 已含 qb_payments，不再另減 orderDepositTotal
          const remaining = orderTotal - arPaid;
          if (remaining > 0) {
            chain.push({ dot: '#f59e0b', label: '尾款', detail: `待收尾款 ${fmtAmt(remaining)}`, status: 'current' });
          }
        }
        ...
        if (allocationRows.length > 0) {
          ...
          if (invoiceData) {
            ...
            if (invPk === 'paid') {
              ...
              chain.push({ dot: '#f59e0b', label: '收款', detail: '待收款', detailColor: '#b45309', status: 'warning' });
            }
          } else if (orderData) {
            const pk = (orderData.payment_status || 'unpaid').toLowerCase();
            chain.push({ dot: pk === 'paid' ? '#16a34a' : '#f59e0b', label: '付款', detail: pk === 'paid' ? '已付款' : pk === 'partial' ? '部分付款' : '未付款', detailColor: pk === 'paid' ? undefined : '#b45309', status: pk === 'paid' ? 'done' : 'warning' });
          }
        }
        // Shipments — with item-level detail
        const oiMap = Object.fromEntries(orderItemRows.map(oi => [oi.id, oi]));
        if (shipmentRows.length > 0) {
          // Calculate total shipped per order_item across all shipments (by id AND item_number)
          const totalShippedByOi = {};
          const totalShippedByItemNo = {};
          shipmentRows.forEach(sh => { (sh.erp_shipment_items || []).forEach(si => {
            if (si.order_item_id) totalShippedByOi[si.order_item_id] = (totalShippedByOi[si.order_item_id] || 0) + Number(si.qty_shipped || 0);
            if (si.item_number) totalShippedByItemNo[si.item_number] = (totalShippedByItemNo[si.item_number] || 0) + Number(si.qty_shipped || 0);
          }); });
          ...
          shipmentRows.forEach(sh => {
            ...
            // Build item detail notes
            const itemNotes = (sh.erp_shipment_items || []).map(si => {
              const oi = oiMap[si.order_item_id];
              const name = oi?.item_number_snapshot || si.item_number || '';
              return name ? `${name} x${si.qty_shipped}` : `x${si.qty_shipped}`;
            });
            const detail = (shk === 'shipped' ? '已出貨' : shk === 'delivered' ? '已送達' : '準備中') + (itemNotes.length ? `：${itemNotes.join('、')}` : '');
            chain.push({ dot: shk === 'shipped' || shk === 'delivered' ? '#16a34a' : '#f59e0b', label: '出貨', ref: sh.shipment_no, refType: 'shipment', detail, time: sh.ship_date || sh.created_at, status: shk === 'shipped' || shk === 'delivered' ? 'done' : 'current' });
          });

          // Check for unshipped items (欠貨) — match by order_item_id OR item_number
          const backorderItems = orderItemRows.filter(oi => {
            const shipped = totalShippedByOi[oi.id] || totalShippedByItemNo[oi.item_number_snapshot] || 0;
            return shipped < Number(oi.qty || 0);
          });
          if (backorderItems.length > 0) {
            const boNotes = backorderItems.map(oi => {
              const shipped = totalShippedByOi[oi.id] || totalShippedByItemNo[oi.item_number_snapshot] || 0;
              const remaining = Number(oi.qty) - shipped;
              return `${oi.item_number_snapshot} 欠 ${remaining}`;
            });
            chain.push({ dot: '#f59e0b', label: '欠貨', detail: `待出貨：${boNotes.join('、')}`, status: 'current' });
          }
        } else if (orderData) {
        ...
        if (orderData) {
          ...
          const allShipped = ss === 'shipped' || ss === 'delivered' || ss === 'partial';
      ...
      case 'quote_detail': {
        ...
        if (quote) {
          ...
          if (linkedOrder) {
            ...
            // Check linked sales
            const { data: linkedSales } = await supabase.from('qb_sales_history').select('id, slip_number, status, created_at').eq('source_id', linkedOrder.id).order('created_at', { ascending: true });
      ...
      case 'procurement_center': {
        ...
        activeItems.forEach(i => {
          ...
          if (po && !a.po_list.find(p => p.po_id === i.po_id)) {
            ...
            // Track latest PO date for sorting
            if (po.po_date && (!a.latest_po_date || po.po_date > a.latest_po_date)) a.latest_po_date = po.po_date;
          }
        });
        ...
        // 5b. Get current stock + latest stock-in date
        let stockMap = {};
        let lastReceivedMap = {};
        if (itemNumbers.length) {
          const [{ data: stockRows }, { data: siRows }] = await Promise.all([
            supabase.from('quickbuy_products').select('item_number, stock_qty').in('item_number', itemNumbers),
            supabase.from('erp_stock_in_items').select('item_number, stock_in_id, qty_received, erp_stock_ins(created_at)').in('item_number', itemNumbers).order('created_at', { referencedTable: 'erp_stock_ins', ascending: false }),
          ]);
          (stockRows || []).forEach(s => { stockMap[s.item_number] = Number(s.stock_qty || 0); });
          (siRows || []).forEach(si => {
            const dt = si.erp_stock_ins?.created_at;
            if (dt && (!lastReceivedMap[si.item_number] || dt > lastReceivedMap[si.item_number])) {
              lastReceivedMap[si.item_number] = dt;
            }
          });
        }
        ...
        let rows = Object.values(itemAgg).map(a => {
          ...
          return {
            ...
            waiting_to_ship,
            stock_qty: stockMap[a.item_number] || 0,
            last_received_at: lastReceivedMap[a.item_number] || null,
            // Status badge logic
        ...
        // Sort: latest received first (newest stock-in on top), fallback to latest PO date
        rows.sort((a, b) => (b.last_received_at || '').localeCompare(a.last_received_at || '') || (b.latest_po_date || '').localeCompare(a.latest_po_date || '') || a.item_number.localeCompare(b.item_number));
    ...
    case 'shipment_analysis': {
      const dateFrom = searchParams.get('date_from') || new Date(Date.now() - 90*24*60*60*1000).toISOString().slice(0,10);
      const dateTo   = searchParams.get('date_to')   || new Date().toISOString().slice(0,10);
      const itemFilter = (searchParams.get('item_number') || '').trim().toUpperCase();

      // 1. Get shipments in date range
      const { data: ships } = await supabase
        .from('erp_shipments')
        .select('id, shipment_no, created_at, status, customer_id, erp_customers(company_name)')
      ...
      if (!shipIds.length) {
        return Response.json({ summary: { total_shipments: 0, total_qty: 0, unique_items: 0, unique_customers: 0 }, by_item: [], by_customer: [], by_month: [], recent: [] });
      }

      // 2. Get shipment items for those shipments
      let itemQ = supabase.from('erp_shipment_items').select('shipment_id, item_number, qty_shipped').in('shipment_id', shipIds).not('item_number', 'is', null);
      if (itemFilter) itemQ = itemQ.ilike('item_number', `%${itemFilter}%`);
      const { data: siRows } = await itemQ;
      ...
      for (const row of (siRows || [])) {
        const ship = shipMap[row.shipment_id];
        if (!ship) continue;
        const item = (row.item_number || '').toUpperCase();
        const qty  = Number(row.qty_shipped || 0);
        const customerName = ship.erp_customers?.company_name || '未知客戶';
        const month = (ship.created_at || '').slice(0, 7);

        if (!byItemMap[item]) byItemMap[item] = { item_number: item, total_qty: 0, shipment_count: 0 };
        byItemMap[item].total_qty       += qty;
        byItemMap[item].shipment_count  += 1;

        if (!byCustomerMap[customerName]) byCustomerMap[customerName] = { customer_name: customerName, total_qty: 0, shipment_count: 0 };
        byCustomerMap[customerName].total_qty      += qty;
        byCustomerMap[customerName].shipment_count += 1;

        if (!byMonthMap[month]) byMonthMap[month] = { month, total_qty: 0, shipment_count: 0 };
        byMonthMap[month].total_qty      += qty;
        byMonthMap[month].shipment_count += 1;

        recentList.push({ shipment_no: ship.shipment_no, created_at: ship.created_at, customer_name: customerName, item_number: item, qty_shipped: qty });
      }

      const byItem     = Object.values(byItemMap).sort((a,b) => b.total_qty - a.total_qty).slice(0, 30);
      const byCustomer = Object.values(byCustomerMap).sort((a,b) => b.total_qty - a.total_qty).slice(0, 15);
      const byMonth    = Object.values(byMonthMap).sort((a,b) => a.month.localeCompare(b.month));
      const recent     = recentList.sort((a,b) => (b.created_at || '').localeCompare(a.created_at || '')).slice(0, 200);

      // 5. Enrich item rows with product description & current stock
      const topItemNos = byItem.map(r => r.item_number);
      if (topItemNos.length) {
        const { data: prods } = await supabase.from('quickbuy_products').select('item_number, description, stock_qty').in('item_number', topItemNos);
        const prodMap = {};
        (prods || []).forEach(p => { prodMap[p.item_number] = p; });
        byItem.forEach(r => {
          const p = prodMap[r.item_number];
          if (p) { r.description = p.description || ''; r.stock_qty = Number(p.stock_qty || 0); }
        });
      }

      const totalQty        = Object.values(byItemMap).reduce((s, r) => s + r.total_qty, 0);
      const totalShipments  = new Set((siRows || []).map(r => r.shipment_id)).size;
      const uniqueCustomers = Object.keys(byCustomerMap).length;
      const uniqueItems     = Object.keys(byItemMap).length;

      return Response.json({
        summary: { total_shipments: totalShipments, total_qty: totalQty, unique_items: uniqueItems, unique_customers: uniqueCustomers },
        by_item: byItem,
    ...
    case 'inventory_turnover': {
      ...
      // All products with stock or had movements
      const { data: products } = await supabase
        .from('quickbuy_products')
        .select('item_number, description, category, stock_qty, safety_stock, cost_price, tw_retail_price')
      ...
      const { data: outbounds } = await supabase
        .from('qb_inventory_movements')
        .select('item_number, quantity, created_at')
      ...
      for (const m of (outbounds || [])) {
        const k = (m.item_number || '').toUpperCase();
        outMap[k] = (outMap[k] || 0) + Number(m.quantity || 0);
      }
      const inMap = {};
      for (const m of (inbounds || [])) {
        const k = (m.item_number || '').toUpperCase();
        inMap[k] = (inMap[k] || 0) + Number(m.quantity || 0);
      }
      ...
      for (const p of (products || [])) {
        const item = (p.item_number || '').toUpperCase();
        prodMap[item] = true;
        const curStock = Number(p.stock_qty || 0);
        const outQty = outMap[item] || 0;
        const inQty = inMap[item] || 0;
        const beginStock = Math.max(curStock + outQty - inQty, 0);
        const avgStock = Math.max((beginStock + curStock) / 2, 0.5);
        const turnoverRate = parseFloat((outQty / avgStock * 365 / days).toFixed(2));
        const daysOfInv = outQty > 0 ? Math.min(Math.round(curStock / (outQty / days)), 9999) : (curStock > 0 ? 9999 : 0);
        const safetyStock = Number(p.safety_stock || 0);
        const safetyRatio = safetyStock > 0 ? parseFloat((curStock / safetyStock).toFixed(1)) : null;
        const costPrice = Number(p.cost_price || 0);
        const stockValue = curStock * costPrice;
        rows.push({
          item_number: item,
          description: p.description || '',
          category: p.category || '',
      ...
      // Items with movements but not in products table
      for (const [item, qty] of Object.entries(outMap)) {
        if (!prodMap[item] && qty > 0) {
          rows.push({
            item_number: item, description: '', category: '', stock_qty: 0,
            safety_stock: 0, cost_price: 0, out_qty: qty, in_qty: inMap[item] || 0,
            turnover_rate: 999, days_of_inventory: 0, safety_ratio: null, stock_value: 0,
          });
        }
      }

      const slowMoving = rows.filter(r => r.stock_qty > 0 && r.out_qty === 0);
      const overStock = rows.filter(r => r.safety_ratio !== null && r.safety_ratio > 3 && r.stock_qty > 0);
      const highTurnover = rows.filter(r => r.out_qty > 0).sort((a, b) => b.turnover_rate - a.turnover_rate);
      const totalStockValue = rows.reduce((s, r) => s + r.stock_value, 0);
      const totalOutQty = rows.reduce((s, r) => s + r.out_qty, 0);
      ...
      return Response.json({
        summary: {
          ...
          active_count: rows.filter(r => r.out_qty > 0).length,
          total_out_qty: totalOutQty,
          days,
        },
        high_turnover: highTurnover.slice(0, 50),
        slow_moving: slowMoving.sort((a, b) => b.stock_qty - a.stock_qty).slice(0, 50),
        over_stock: overStock.sort((a, b) => b.safety_ratio - a.safety_ratio).slice(0, 30),
        all_items: rows.sort((a, b) => b.out_qty - a.out_qty || b.stock_qty - a.stock_qty),
      });
    }

    case 'customer_rfm': {
      const todayDate = new Date();
      const cutoff365 = new Date(todayDate - 365 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
      const todayStr = todayDate.toISOString().slice(0, 10);

      // All non-cancelled sales
      const { data: allSales } = await supabase
        .from('qb_sales_history')
        .select('id, sale_date, total, customer_name, payment_status')
        .neq('status', 'cancelled')
        .order('sale_date', { ascending: false })
        .limit(10000);
      ...
      for (const s of (allSales || [])) {
        ...
        c.total_amount_all += Number(s.total || 0);
        c.order_count_all += 1;
        if ((s.sale_date || '') > c.last_sale_date) c.last_sale_date = s.sale_date;
        if ((s.sale_date || '') >= cutoff365) {
          c.order_count_12m += 1;
          c.total_amount_12m += Number(s.total || 0);
        }
      }

      // Score RFM
      const rfmRows = Object.values(custMap).map(c => {
        const daysSinceLast = c.last_sale_date
          ? Math.max(0, Math.floor((new Date(todayStr) - new Date(c.last_sale_date)) / (24 * 60 * 60 * 1000)))
          : 9999;

        const rScore = daysSinceLast <= 14 ? 5 : daysSinceLast <= 30 ? 4 : daysSinceLast <= 90 ? 3 : daysSinceLast <= 180 ? 2 : 1;
        const fScore = c.order_count_12m >= 12 ? 5 : c.order_count_12m >= 6 ? 4 : c.order_count_12m >= 3 ? 3 : c.order_count_12m >= 1 ? 2 : 1;
        const mScore = c.total_amount_12m >= 300000 ? 5 : c.total_amount_12m >= 100000 ? 4 : c.total_amount_12m >= 30000 ? 3 : c.total_amount_12m >= 5000 ? 2 : 1;
        const rfmScore = rScore + fScore + mScore;

        let segment;
        if (rfmScore >= 13) segment = 'VIP';
        else if (rfmScore >= 10 || (rScore >= 4 && mScore >= 4)) segment = '高價值';
        else if (rfmScore >= 7 && rScore >= 3) segment = '成長潛力';
        else if (daysSinceLast > 180 && c.total_amount_12m > 0) segment = '流失風險';
        else if (daysSinceLast > 180) segment = '沉睡客戶';
        else if (rfmScore >= 5) segment = '一般活躍';
        else segment = '低頻客戶';
      ...
      }).sort((a, b) => b.rfm_score - a.rfm_score || b.total_amount_12m - a.total_amount_12m);

      // Segment summary
      const segMap = {};
      for (const r of rfmRows) {
        if (!segMap[r.segment]) segMap[r.segment] = { segment: r.segment, count: 0, total_amount: 0, avg_rfm: 0 };
        segMap[r.segment].count += 1;
        segMap[r.segment].total_amount += r.total_amount_12m;
      }
      ...
      return Response.json({
        summary: {
          total_customers: rfmRows.length,
          vip_count: rfmRows.filter(r => r.segment === 'VIP').length,
          high_value_count: rfmRows.filter(r => r.segment === '高價值').length,
          at_risk_count: rfmRows.filter(r => r.segment === '流失風險').length,
          dormant_count: rfmRows.filter(r => r.segment === '沉睡客戶').length,
          total_12m_revenue: Math.round(rfmRows.reduce((s, r) => s + r.total_amount_12m, 0)),
        },
        customers: rfmRows.slice(0, 150),
        segments,
      });
    }

    case 'purchase_efficiency': {
      const peDateFrom = searchParams.get('date_from') || new Date(Date.now() - 365 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
      const peDateTo = searchParams.get('date_to') || new Date().toISOString().slice(0, 10);

      // Get POs with vendor info
      const { data: pos } = await supabase
        .from('erp_purchase_orders')
        .select('id, po_no, vendor_id, po_date, expected_date, status, total_amount, erp_vendors(name)')
        .gte('po_date', peDateFrom)
        .lte('po_date', peDateTo)
        .order('po_date', { ascending: false })
        .limit(2000);

      // Get confirmed stock-ins linked to those POs
      ...
      if (poIds.length) {
        ...
        for (let i = 0; i < poIds.length; i += chunkSize) {
          ...
          const { data: sis } = await supabase
            .from('erp_stock_ins')
            .select('id, po_id, stock_in_date, status, total_amount')
            .in('po_id', chunk)
            .eq('status', 'confirmed');
          for (const si of (sis || [])) {
            if (!stockInsMap[si.po_id]) stockInsMap[si.po_id] = [];
            stockInsMap[si.po_id].push(si);
          }
        }
      }
      ...
      const poRows = (pos || []).map(po => {
        const vendorName = po.erp_vendors?.name || '未知廠商';
        const sis = stockInsMap[po.id] || [];
        const firstSi = sis.sort((a, b) => (a.stock_in_date || '').localeCompare(b.stock_in_date || ''))[0];

        let leadDays = null;
        let onTime = null;
        if (firstSi?.stock_in_date && po.po_date) {
          leadDays = Math.max(0, Math.floor((new Date(firstSi.stock_in_date) - new Date(po.po_date)) / (24 * 60 * 60 * 1000)));
          if (po.expected_date) {
            onTime = firstSi.stock_in_date <= po.expected_date;
          }
        }
        return {
          po_no: po.po_no,
          ...
          stock_in_date: firstSi?.stock_in_date || null,
          lead_days: leadDays,
          on_time: onTime,
          fulfilled: sis.length > 0,
        };
      });
      ...
      for (const po of poRows) {
        ...
        if (!vendorMap[v]) vendorMap[v] = {
          vendor_name: v, po_count: 0, total_amount: 0,
          fulfilled_count: 0, on_time_count: 0, on_time_eligible: 0,
          lead_days_sum: 0, lead_days_count: 0,
        };
        const vm = vendorMap[v];
        vm.po_count += 1;
        vm.total_amount += po.total_amount;
        if (po.fulfilled) vm.fulfilled_count += 1;
        if (po.on_time !== null) { vm.on_time_eligible += 1; if (po.on_time) vm.on_time_count += 1; }
        if (po.lead_days !== null) { vm.lead_days_sum += po.lead_days; vm.lead_days_count += 1; }
      }

      const byVendor = Object.values(vendorMap).map(v => ({
        ...v,
        total_amount: Math.round(v.total_amount),
        fulfillment_rate: v.po_count > 0 ? Math.round(v.fulfilled_count / v.po_count * 100) : 0,
        on_time_rate: v.on_time_eligible > 0 ? Math.round(v.on_time_count / v.on_time_eligible * 100) : null,
        avg_lead_days: v.lead_days_count > 0 ? Math.round(v.lead_days_sum / v.lead_days_count) : null,
      })).sort((a, b) => b.total_amount - a.total_amount);

      // Monthly trend
      const byMonthMap = {};
      for (const po of poRows) {
        const month = (po.po_date || '').slice(0, 7);
        if (!month) continue;
        if (!byMonthMap[month]) byMonthMap[month] = { month, po_count: 0, total_amount: 0, fulfilled_count: 0 };
        byMonthMap[month].po_count += 1;
        byMonthMap[month].total_amount += po.total_amount;
        if (po.fulfilled) byMonthMap[month].fulfilled_count += 1;
      }

      const totalPos = poRows.length;
      const fulfilledCount = poRows.filter(p => p.fulfilled).length;
      const onTimeCount = poRows.filter(p => p.on_time === true).length;
      const eligibleOnTime = poRows.filter(p => p.on_time !== null).length;
      const leadArr = poRows.filter(p => p.lead_days !== null).map(p => p.lead_days);
      const avgLead = leadArr.length ? Math.round(leadArr.reduce((a, b) => a + b, 0) / leadArr.length) : null;
      const totalAmt = Math.round(poRows.reduce((s, p) => s + p.total_amount, 0));

      return Response.json({
        summary: {
          total_pos: totalPos,
          total_amount: totalAmt,
          fulfillment_rate: totalPos > 0 ? Math.round(fulfilledCount / totalPos * 100) : 0,
          on_time_rate: eligibleOnTime > 0 ? Math.round(onTimeCount / eligibleOnTime * 100) : null,
          avg_lead_days: avgLead,
          unique_vendors: Object.keys(vendorMap).length,
        },
        by_vendor: byVendor.slice(0, 20),
        by_month: Object.values(byMonthMap).sort((a, b) => a.month.localeCompare(b.month)),
        recent_pos: poRows.slice(0, 100),
      });
    }

    case 'ar_aging': {
      const arToday = new Date();
      const arTodayStr = arToday.toISOString().slice(0, 10);

      function ageBucket(dateStr) {
        if (!dateStr) return 'unknown';
        const days = Math.floor((arToday - new Date(dateStr)) / (24 * 60 * 60 * 1000));
      ...
      // ERP invoices not fully paid
      const { data: erpInvoices } = await supabase
        .from('erp_invoices')
        .select('id, invoice_no, invoice_date, due_date, total_amount, paid_amount, balance, payment_status, customer_id, erp_customers(company_name)')
        .not('payment_status', 'eq', 'paid')
        .order('invoice_date', { ascending: false })
        .limit(1000);

      // QB sales not paid
      const { data: qbSales } = await supabase
        .from('qb_sales_history')
        .select('id, sale_date, invoice_date, total, customer_name, payment_status, payment_date')
        .not('payment_status', 'eq', 'paid')
        .neq('status', 'cancelled')
        .order('sale_date', { ascending: false })
        .limit(2000);
      ...
      // Process ERP invoices
      const erpRows = [];
      for (const inv of (erpInvoices || [])) {
        const balance = Number(inv.balance || (Number(inv.total_amount || 0) - Number(inv.paid_amount || 0)));
        if (balance <= 0) continue;
        const custName = inv.erp_customers?.company_name || '未知客戶';
        const refDate = inv.due_date || inv.invoice_date;
        const bucket = ageBucket(refDate);
        buckets[bucket] = (buckets[bucket] || 0) + balance;

        if (!byCustomer[custName]) byCustomer[custName] = { customer_name: custName, total_ar: 0, current: 0, '1_30': 0, '31_60': 0, '61_90': 0, '91_120': 0, '120_plus': 0, invoice_count: 0 };
        byCustomer[custName].total_ar += balance;
        byCustomer[custName][bucket] = (byCustomer[custName][bucket] || 0) + balance;
        byCustomer[custName].invoice_count += 1;

        erpRows.push({ source: 'erp', invoice_no: inv.invoice_no, customer_name: custName, invoice_date: inv.invoice_date, due_date: inv.due_date, balance, bucket, payment_status: inv.payment_status });
      }

      // Process QB sales
      const qbRows = [];
      for (const s of (qbSales || [])) {
        const amount = Number(s.total || 0);
        if (amount <= 0) continue;
        const refDate = s.invoice_date || s.sale_date;
        const bucket = ageBucket(refDate);
        buckets[bucket] = (buckets[bucket] || 0) + amount;
        qbRows.push({ source: 'qb', ref_id: s.id, customer_name: s.customer_name || '未知', sale_date: s.sale_date, amount, bucket, payment_status: s.payment_status });
      }

      const totalAR = Object.values(buckets).reduce((a, b) => a + b, 0);
      const overdueAR = buckets['1_30'] + buckets['31_60'] + buckets['61_90'] + buckets['91_120'] + buckets['120_plus'];

      return Response.json({
        summary: {
          total_ar: Math.round(totalAR),
          overdue_amount: Math.round(overdueAR),
          current_amount: Math.round(buckets.current),
          bucket_1_30: Math.round(buckets['1_30']),
          bucket_31_60: Math.round(buckets['31_60']),
          bucket_61_90: Math.round(buckets['61_90']),
          bucket_91_120: Math.round(buckets['91_120']),
          bucket_120_plus: Math.round(buckets['120_plus']),
          erp_count: erpRows.length,
          qb_count: qbRows.length,
        },
        by_customer: Object.values(byCustomer).map(c => ({
          ...c,
          total_ar: Math.round(c.total_ar),
        })).sort((a, b) => b.total_ar - a.total_ar).slice(0, 50),
        erp_invoices: erpRows.sort((a, b) => (b.invoice_date || '').localeCompare(a.invoice_date || '')),
        qb_sales: qbRows.sort((a, b) => (b.sale_date || '').localeCompare(a.sale_date || '')).slice(0, 100),
      });
    }
      ...
      case 'notifications': {
        ...
        const { data, count, error } = await query;
        if (error) return Response.json({ error: error.message }, { status: 500 });
        return Response.json({ rows: data || [], total: count || 0, unread_count: (data || []).filter(n => !n.is_read).length });
      }

      default: {
        // Try Pulse module actions
        const { handlePulseGetAction } = await import('./actions-pulse');
        const pulseResult = await handlePulseGetAction(action, searchParams);
        if (pulseResult) return pulseResult;

        // Try HR module actions
        const { handleHrGetAction } = await import('./actions-hr');
        const hrResult = await handleHrGetAction(action, searchParams);
        if (hrResult) return hrResult;

        // Try Warranty module actions
        const { handleWarrantyGetAction } = await import('./actions-warranty');
        const warrantyResult = await handleWarrantyGetAction(action, searchParams);
