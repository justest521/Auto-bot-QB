      ...
      case 'document_chain': {
        ...
        if (resolvedOrderId) {
          fetchPromises.push(
            ...
            supabase.from('erp_shipments').select('id, shipment_no, status, shipped_at, created_at, erp_shipment_items(order_item_id, qty_shipped)').eq('order_id', resolvedOrderId).order('created_at').then(r => { shipmentRows = r.data || []; }),
        ...
        if (shipmentRows.length > 0) {
          ...
          shipmentRows.forEach(sh => {
            ...
            chain.push({ dot: shk === 'shipped' || shk === 'delivered' ? '#16a34a' : '#f59e0b', label: '出貨', ref: sh.shipment_no, refType: 'shipment', detail, time: sh.shipped_at || sh.created_at, status: shk === 'shipped' || shk === 'delivered' ? 'done' : 'current' });
          });
