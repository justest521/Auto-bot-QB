      ...
      case 'accounts_receivable': {
        ...
        if (orderIds.length > 0) {
          const [ordersRes, paymentsRes] = await Promise.all([
            ...
            supabase.from('qb_payments').select('erp_order_id, amount, payment_type').in('erp_order_id', orderIds).eq('status', 'confirmed'),
          ]);
        ...
        const rows = (data || []).map(r => {
          ...
          // 用 Math.max 避免雙重計算：erp_invoices.paid_amount 已包含 qb_payments 的分配金額
          const totalPaid = Math.max(invoicePaid, orderPaid);
          const total = Number(r.total_amount || 0);
          const displayStatus = totalPaid >= total && total > 0 ? 'paid' : r.payment_status;
          return {
            ...r,
