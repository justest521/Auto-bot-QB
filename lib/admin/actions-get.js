...
export async function handleGetAction(action, searchParams) {
  ...
  switch (action) {
      ...
      case 'order_items_with_stock': {
        ...
        const { data: orderData } = await supabase.from('erp_orders').select('order_no').eq('id', orderId).single();
        let linkedPOs = [];
        ...
        // Build timeline for order
        const timeline = [];
        const { data: order } = await supabase.from('erp_orders').select('*').eq('id', orderId).single();
        if (order) {
          // Order created
          timeline.push({ event: '建立訂單', time: order.created_at, status: 'done' });

          // Check if order came from a quote
          if (order.quote_id) {
            const { data: linkedQuote } = await supabase.from('erp_quotes').select('quote_no, created_at').eq('id', order.quote_id).single();
            if (linkedQuote) {
              timeline.push({ event: `從報價單 ${linkedQuote.quote_no} 轉入`, time: linkedQuote.created_at, status: 'done' });
            }
          }

          // Check approvals for this order
          const { data: approvals } = await supabase.from('erp_approvals').select('status, created_at, approved_at, approved_by').eq('doc_id', orderId.toString()).order('created_at', { ascending: true });
          ...
          // Check for sales (銷貨單)
          const { data: salesList } = await supabase.from('qb_sales_history').select('slip_number, created_at').eq('source_id', orderId).order('created_at', { ascending: true });
          ...
          const { data: posList } = await supabase.from('erp_purchase_orders').select('po_no, created_at').or(`source_order_ids.cs.["${orderId}"]`).order('created_at', { ascending: true });
          ...
          // Check for shipments
          const { data: shipmentsList } = await supabase.from('erp_shipments').select('shipment_no, created_at').eq('order_id', orderId).order('created_at', { ascending: true });
      ...
      case 'sale_detail': {
        ...
        try {
          ...
          // Fallback 2: use the sales_history rows as line items
          ...
          if (first.erp_order_id) {
            const { data: linkedOrder } = await supabase.from('erp_orders').select('order_no, created_at').eq('id', first.erp_order_id).single();
            if (linkedOrder) {
      ...
      case 'products': {
        ...
        let query = supabase
          .from('quickbuy_products')
          .select(
            'item_number,description,us_price,tw_retail_price,tw_reseller_price,product_status,category,replacement_model,weight_kg,origin_country,image_url',
            { count: 'exact' }
          )
      ...
      case 'purchase_orders': {
        ...
        const [draftRes, sentRes, confirmedRes, shippedRes, receivedRes] = await Promise.all([
          supabase.from('erp_purchase_orders').select('*', { count: 'exact', head: true }).eq('status', 'draft'),
          supabase.from('erp_purchase_orders').select('*', { count: 'exact', head: true }).eq('status', 'sent'),
          supabase.from('erp_purchase_orders').select('*', { count: 'exact', head: true }).eq('status', 'confirmed'),
          supabase.from('erp_purchase_orders').select('*', { count: 'exact', head: true }).eq('status', 'shipped'),
          supabase.from('erp_purchase_orders').select('*', { count: 'exact', head: true }).eq('status', 'received'),
        ]);

        return Response.json({ rows: data || [], total: count || 0, page, limit, summary: { draft: draftRes.count || 0, sent: sentRes.count || 0, confirmed: confirmedRes.count || 0, shipped: shippedRes.count || 0, received: receivedRes.count || 0 } });
      }
      ...
      case 'po_items': {
        ...
        const timeline = [];
        const { data: po } = await supabase.from('erp_purchase_orders').select('*').eq('id', poId).single();
        if (po) {
          // PO created
          ...
          if (po.source_order_ids && Array.isArray(po.source_order_ids) && po.source_order_ids.length > 0) {
            ...
            const { data: linkedOrder } = await supabase.from('erp_orders').select('order_no, created_at').eq('id', sourceOrderId).single();
            if (linkedOrder) {
              timeline.push({ event: `來自訂單 ${linkedOrder.order_no}`, time: linkedOrder.created_at, status: 'done' });
            }
          }
        ...
        const enriched = await Promise.all((data || []).map(async (a) => {
          try {
            if (a.doc_type === 'order' && a.doc_id) {
              const { data: order } = await supabase.from('erp_orders').select('id, order_no, total_amount, customer:erp_customers(name, company_name, phone)').eq('id', a.doc_id).maybeSingle();
              ...
              return { ...a, amount: a.amount || order?.total_amount || null, customer: order?.customer || null, items };
            }
            if (a.doc_type === 'sale' && a.doc_id) {
              const { data: sale } = await supabase.from('qb_sales_history').select('id, slip_number, total_amount, customer:erp_customers(name, company_name, phone)').eq('id', a.doc_id).maybeSingle();
              // Sale items come from the source order items
              ...
              const items = (rawItems || []).map(i => ({ item_number: i.item_number_snapshot, description: i.description_snapshot, quantity: i.qty, unit_price: i.unit_price, subtotal: i.line_total }));
              return { ...a, amount: a.amount || sale?.total_amount || null, customer: sale?.customer || null, items };
            }
      ...
      case 'ticket_detail': {
        ...
        const { data: ticket } = await supabase.from('erp_tickets').select('*').eq('id', ticketId).single();
      ...
      case 'quote_detail': {
        ...
        const { data: quote } = await supabase.from('erp_quotes').select('*, customer:erp_customers(name, company_name, phone, email, tax_id, address)').eq('id', quoteId).single();
        ...
        return Response.json({ quote, items: items || [], timeline });
      }
