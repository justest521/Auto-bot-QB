        ...
        if (order) {
          ...
          const [quoteRes, approvalsRes, shipmentsRes] = await Promise.all([
            order.quote_id
              ...
              : { data: null },
            supabase.from('erp_approvals').select('status, created_at, approved_at, approved_by, remark').eq('doc_id', orderId.toString()).order('created_at', { ascending: true }),
