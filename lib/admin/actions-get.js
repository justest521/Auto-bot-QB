  ...
  switch (action) {
      ...
      case 'document_chain': {
        ...
        if (shipmentRows.length > 0) {
          // Calculate total shipped per order_item across all shipments
          const totalShippedByOi = {};
          shipmentRows.forEach(sh => { (sh.erp_shipment_items || []).forEach(si => { totalShippedByOi[si.order_item_id] = (totalShippedByOi[si.order_item_id] || 0) + Number(si.qty_shipped || 0); }); });
          ...
          // Check for unshipped items (欠貨)
          const backorderItems = orderItemRows.filter(oi => {
            const shipped = totalShippedByOi[oi.id] || 0;
