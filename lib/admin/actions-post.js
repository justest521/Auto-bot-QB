  ...
  switch (action) {
      ...
      case 'verify_payment': {
        ...
        const { error: vpErr } = await supabase.from('qb_payments').update({
          verified: isVerified,
          verified_by: isVerified ? (auth?.user?.display_name || auth?.user?.username || 'admin') : null,
      ...
      case 'record_order_payment': {
        ...
        if (payProofData) {
          try {
            const ext = (payProofName || 'proof.jpg').split('.').pop() || 'jpg';
            const proofPath = `payment-proofs/${payNo}.${ext}`;
            const proofBuffer = Buffer.from(payProofData, 'base64');
            await supabase.storage.from('company-assets').upload(proofPath, proofBuffer, { contentType: `image/${ext}`, upsert: true });
            const { data: urlData } = supabase.storage.from('company-assets').getPublicUrl(proofPath);
      ...
      case 'instock_to_sale': {
        ...
        if (!allowedStatuses.includes(order.status)) {
          ...
          if (prevApproval || existingSales) {
            ...
            const evidence = prevApproval ? `原核准紀錄 ${prevApproval.id}` : `已有銷貨紀錄 ${existingSales.id}`;
            await supabase.from('erp_approvals').insert({
