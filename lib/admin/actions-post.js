  ...
  switch (action) {
      ...
      case 'update_sale_invoice': {
        ...
        if (invoice_number) {
          ...
          // Fallback：invoice_no 找不到時，用 order_id 找「DB 層直接過濾尚未綁定號碼的行」
          // 用 .is('invoice_no', null) 在 DB 層過濾，避免多行時 maybeSingle 出錯
          if (!existingInvId && linkedOrderId) {
            const { data: existByOrder } = await supabase.from('erp_invoices')
              .select('id')
              .eq('order_id', linkedOrderId)
              .is('invoice_no', null)
              .limit(1)
              .maybeSingle();
            existingInvId = existByOrder?.id || null;
          }
