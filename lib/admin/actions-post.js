...
export async function handlePostAction(action, body) {
  switch (action) {
      ...
      case 'record_order_payment': {
        // 直接登記訂單付款（不經由發票）
        const { order_id: payOrderId, amount: payAmount, method: payMethod, remark: payRemark } = body;
        if (!payOrderId || !payAmount) return Response.json({ error: 'order_id 和 amount 為必填' }, { status: 400 });

        const { data: payOrder } = await supabase.from('erp_orders').select('total_amount, payment_status, order_no').eq('id', payOrderId).maybeSingle();
        ...
        const { error: payInsertErr } = await supabase.from('qb_payments').insert({
          ...
          confirmed_by: 'admin',
          remark: payRemark || `訂單 ${payOrder.order_no} 付款`,
        });
        if (payInsertErr) return Response.json({ error: `付款記錄寫入失敗: ${payInsertErr.message}` }, { status: 500 });

        // Calculate total paid
        const { data: allPayments } = await supabase.from('qb_payments').select('amount').eq('order_id', payOrderId).eq('status', 'confirmed');
        ...
        return Response.json({ success: true, total_paid: totalPaid, payment_status: newPayStatus, message: newPayStatus === 'paid' ? '已全額收款' : `已收款 NT$${totalPaid.toLocaleString()}` });
      }
      ...
      case 'instock_to_sale': {
        ...
        return Response.json({ success: true, sale: { slip_number: slipNumber }, processed_count: processItems.length, total_items: (items || []).length, order_status: newOrderStatus });
      }
      ...
      case 'update_sale_invoice': {
        const { sale_id, invoice_number, remark } = body;
      ...
      case 'update_po_vendor': {
        ...
        const updatePayload = { vendor_id: vendor_id ? Number(vendor_id) : null };
