  ...
  switch (action) {
      ...
      case 'create_shipment': {
        ...
        if (order_id) {
          ...
          if (!['confirmed', 'processing'].includes(order.status)) {
            return Response.json({ error: `訂單狀態為「${order.status}」，必須核准（confirmed）後才能出貨` }, { status: 400 });
          }
        ...
        if (safeItems.length > 0) {
          ...
          for (const it of itemPayload) {
            ...
            const normalizedItemNo = itemNumber.toUpperCase();
            // CAS 扣庫存
            let retries = 3;
            while (retries > 0) {
      ...
      case 'execute_vendor_matching': {
        ...
        for (const va of vpAllocs) {
          ...
          // 3.4 Update payable paid_amount & balance（正確處理負數 credit）
          ...
          if (vaPay) {
            ...
            const vaBalance = vaTotal - Math.abs(vaNewPaid);
            const vaPayStatus = Math.abs(vaNewPaid) >= vaTotal ? 'paid' : Math.abs(vaNewPaid) > 0 ? 'partial' : 'unpaid';
      ...
      case 'submit_approval': {
        ...
        if (doc_type === 'order') {
          ...
          if (ord && ord.status === 'rejected') {
            return Response.json({ error: '此訂單已被駁回，請先修改訂單內容（改為草稿狀態）再重新送審' }, { status: 400 });
          }
