...
export async function handlePostAction(action, body) {
  switch (action) {
      ...
      case 'create_shipment': {
        ...
        // Prevents creating/deleting empty shipment records and avoids potential DB constraint issues
        ...
        if (safeItems.length === 0 && sale_id) {
          ...
          // Note: instock_to_sale stores sale items in qb_order_items with order_id = sale.id
          const { data: saleItems } = await supabase.from('qb_order_items').select('id, item_number, quantity').eq('order_id', sale_id);
          // Cross-table fix: old-code shipments stored erp_order_items.id as order_item_id.
          // Build item_number → erp_order_items.id map to detect those as already-shipped.
          let erpIdByItemNo = {};
          if (orderId && (saleItems || []).length > 0) {
            const itemNos = (saleItems || []).map(si => si.item_number).filter(Boolean);
            if (itemNos.length > 0) {
              const { data: erpItems } = await supabase.from('erp_order_items')
                .select('id, item_number_snapshot').eq('order_id', orderId).in('item_number_snapshot', itemNos);
              (erpItems || []).forEach(i => { erpIdByItemNo[i.item_number_snapshot] = i.id; });
            }
          }
          safeItems = (saleItems || []).map(si => {
            const ordered = Number(si.quantity || 0);
            let shipped = alreadyShippedMap[String(si.id)] || alreadyShippedMap[String(si.product_id || '')] || 0;
            // Fallback: check by erp_order_items.id for old-code shipments (orderId path)
            if (!shipped && si.item_number && erpIdByItemNo[si.item_number]) {
              shipped = alreadyShippedMap[String(erpIdByItemNo[si.item_number])] || 0;
            }
            const remaining = Math.max(0, ordered - shipped);
            return { id: si.id, product_id: si.product_id, item_number: si.item_number, qty_shipped: remaining };
        ...
        if (safeItems.length === 0 && sale_id) {
          ...
          // qb_order_items.id is bigint (not UUID), so we use item_number as the tracking key
          const { data: saleItems } = await supabase.from('qb_order_items').select('id, item_number, quantity').eq('order_id', sale_id);
          safeItems = (saleItems || []).map(si => {
            const ordered = Number(si.quantity || 0);
