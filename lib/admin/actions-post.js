...
export async function handlePostAction(action, body) {
  switch (action) {
      ...
      case 'instock_to_sale': {
        ...
        // 運費：首次出貨帶入全額，後續部分出貨不重複收
        let orderShipping = 0;
        if (Number(order.shipping_fee || 0) > 0) {
          const { data: existingSales } = await supabase.from('qb_sales_history')
            .select('shipping_fee').eq('source_id', order_id).neq('status', 'cancelled');
          const alreadyCharged = (existingSales || []).some(s => Number(s.shipping_fee || 0) > 0);
          orderShipping = alreadyCharged ? 0 : Number(order.shipping_fee);
        }
