...
// ── 自動核准（庫存充足 + 金額在門檻內）──
const AUTO_APPROVE_AMOUNT_LIMIT = 50000; // NT$50,000

// Helper: 自動核准檢查 — 庫存充足 + 金額在門檻內
async function tryAutoApproveOrder(orderId, orderNo, totalAmount, orderItems) {
  // ── 審核關閉 → 直接核准 ──
  ...
  // 金額超過門檻 → 需人工審核
  if (amount > AUTO_APPROVE_AMOUNT_LIMIT) {
    return { approved: false, reason: `金額 NT$${amount.toLocaleString()} 超過自動核准門檻 NT$${AUTO_APPROVE_AMOUNT_LIMIT.toLocaleString()}` };
  }

  // 檢查所有品項庫存
  const itemNumbers = orderItems.map(i => i.item_number_snapshot || i.item_number).filter(Boolean);
  if (itemNumbers.length === 0) return { approved: false, reason: '無品項' };

  const { data: products } = await supabase
    .from('quickbuy_products')
    .select('item_number, stock_qty')
    .in('item_number', itemNumbers);
  const stockMap = Object.fromEntries((products || []).map(p => [p.item_number, Number(p.stock_qty || 0)]));

  const outOfStock = [];
  for (const oi of orderItems) {
    const itemNo = oi.item_number_snapshot || oi.item_number;
    if (!itemNo) continue;
    const stock = stockMap[itemNo] || 0;
    const needed = Number(oi.qty || 0);
    if (stock < needed) outOfStock.push(`${itemNo} 需${needed}/庫存${stock}`);
  }

  if (outOfStock.length > 0) {
    return { approved: false, reason: `庫存不足：${outOfStock.join('、')}` };
  }
  ...
  await supabase.from('erp_approvals').insert({
    ...
    requested_by: 'system', status: 'approved',
    approved_by: 'system_auto', approved_at: new Date().toISOString(),
    amount, remark: `庫存充足且金額 NT$${amount.toLocaleString()} ≤ 門檻 NT$${AUTO_APPROVE_AMOUNT_LIMIT.toLocaleString()}，系統自動核准`,
  });
...
async function checkOrderCompletion(orderId) {
  ...
  const shipped = order.shipping_status === 'shipped' || order.shipping_status === 'delivered';
  if (paid && shipped) {
...
export async function handlePostAction(action, body) {
  switch (action) {
      ...
      case 'create_shipment': {
        ...
        // ── Calculate already-shipped quantities to prevent over-shipping ──
        let alreadyShippedMap = {}; // product_id → total already shipped
        if (sale_id || orderId) {
          const shipQuery = sale_id
            ? supabase.from('erp_shipment_items').select('product_id, qty_shipped, erp_shipments!inner(sale_id, status)').eq('erp_shipments.sale_id', sale_id).neq('erp_shipments.status', 'cancelled')
            : supabase.from('erp_shipment_items').select('product_id, qty_shipped, erp_shipments!inner(order_id, status)').eq('erp_shipments.order_id', orderId).neq('erp_shipments.status', 'cancelled');
          const { data: prevShipped } = await shipQuery;
          (prevShipped || []).forEach(si => {
            const pid = si.product_id;
            if (pid) alreadyShippedMap[pid] = (alreadyShippedMap[pid] || 0) + Number(si.qty_shipped || 0);
          });
        }

        // Insert shipment items — use provided items, or auto-populate from sale/order
        let safeItems = Array.isArray(items) ? items.filter(Boolean) : [];
        if (safeItems.length === 0 && sale_id) {
          // Auto-populate from sale line items (only remaining qty)
          const { data: saleItems } = await supabase.from('qb_order_items').select('id, product_id, quantity').eq('sale_id', sale_id);
          safeItems = (saleItems || []).map(si => {
            const ordered = Number(si.quantity || 0);
            const shipped = alreadyShippedMap[si.product_id] || 0;
            const remaining = Math.max(0, ordered - shipped);
            return { product_id: si.product_id, qty_shipped: remaining };
          }).filter(si => si.qty_shipped > 0);
        }
        if (safeItems.length === 0 && orderId) {
          // Auto-populate from order items (only remaining qty)
          const { data: oi } = await supabase.from('erp_order_items').select('id, product_id, qty').eq('order_id', orderId);
          safeItems = (oi || []).map(o => {
            const ordered = Number(o.qty || 0);
            const shipped = alreadyShippedMap[o.product_id] || 0;
            const remaining = Math.max(0, ordered - shipped);
            return { order_item_id: o.id, product_id: o.product_id, qty_shipped: remaining };
          }).filter(o => o.qty_shipped > 0);
        }

        // If no items remain to ship, reject
        if (safeItems.length === 0) {
          // Clean up the empty shipment we just created
          await supabase.from('erp_shipments').delete().eq('id', shipment.id);
        ...
        try {
          ...
          // Check if invoice already exists for this order (avoid duplicates)
          const existingInv = orderId
            ? (await supabase.from('erp_invoices').select('id').eq('order_id', orderId).limit(1)).data
            : [];
      ...
      case 'create_purchase_order': {
        const { vendor_id, expected_date, remark, items, tax_excluded } = body;
        const safeItems = Array.isArray(items) ? items.filter(Boolean) : [];
      ...
      case 'execute_vendor_matching': {
        ...
        for (const va of vpAllocs) {
          ...
          // Update payable paid_amount & balance
          ...
          if (vaPay) {
            ...
            const vaTotal = Math.abs(Number(vaPay.total_amount || 0));
            const vaPayStatus = vaNewPaid >= vaTotal ? 'paid' : vaNewPaid > 0 ? 'partial' : 'unpaid';
            await supabase.from('erp_vendor_payables').update({
              paid_amount: vaNewPaid, balance: Number(vaPay.total_amount || 0) - vaNewPaid,
      ...
      case 'confirm_stock_in_with_inventory': {
        ...
        for (const item of (siItems || [])) {
          ...
          if (product) {
            ...
            // 如果商品尚未設定安全庫存，自動以本次入庫量為基準設定
            if (!product.safety_stock || product.safety_stock <= 0) {
              updatePayload.safety_stock = Math.max(Math.ceil((item.qty_received || 1) * 0.5), 1);
            }
            await supabase
              .from('quickbuy_products')
          ...
          // Update PO items qty_received (FIFO across unfulfilled PO items)
          try {
        ...
        // ── 自動產生應付帳款 ──
        try {
          const siTotal = (siItems || []).reduce((s, it) => s + (Number(it.qty_received) || 0) * (Number(it.unit_cost) || 0), 0);
          if (siTotal > 0 && si.vendor_id) {
            ...
            await supabase.from('erp_vendor_payables').insert({
              ...
              total_amount: siTotal, paid_amount: 0, balance: siTotal,
      ...
      case 'record_payment': {
        ...
        try {
          ...
          const { data: rcptData } = await supabase.from('erp_payment_receipts').insert({
            ...
            confirmed_at: new Date().toISOString(),
            confirmed_by: 'system',
            remark: `應收帳款收款自動建立 | 發票:${inv.invoice_no}`,
          }).select().single();
      ...
      case 'generate_reconciliation': {
        ...
        const { data: stmtData, error: stmtErr } = await supabase.from('erp_reconciliation_statements').insert({
          ...
          net_amount: reconNet, previous_balance: 0, current_balance: reconNet,
        ...
        if (reconItems.length > 0) await supabase.from('erp_reconciliation_items').insert(reconItems);

        return Response.json({ success: true, data: stmtData, items_count: reconItems.length, message: `對帳單 ${stmtNo} 已產生，含 ${reconItems.length} 筆明細` });
      }
      ...
      case 'record_order_payment': {
        // 直接登記訂單付款（不經由發票）
        const { order_id: payOrderId, amount: payAmount, method: payMethod, remark: payRemark, payment_type: payType } = body;
        ...
        // === Auto-update erp_invoices paid_amount (應收帳款沖帳) ===
        try {
          const { data: relatedInv } = await supabase.from('erp_invoices').select('id, total_amount, paid_amount').eq('order_id', payOrderId).limit(1);
          if (relatedInv?.[0]) {
            const inv = relatedInv[0];
            const newPaid = Math.min(Number(inv.total_amount || 0), totalPaid);
            const invPayStatus = newPaid >= Number(inv.total_amount || 0) ? 'paid' : newPaid > 0 ? 'partial' : 'unpaid';
            await supabase.from('erp_invoices').update({
              paid_amount: newPaid,
              balance: Number(inv.total_amount || 0) - newPaid,
        ...
        return Response.json({ success: true, total_paid: totalPaid, payment_status: newPayStatus, message: newPayStatus === 'paid' ? '已全額收款' : `已收${typeLabel[payType] || '款'} NT$${Number(payAmount).toLocaleString()}，累計 NT$${totalPaid.toLocaleString()}` });
      }
      ...
      case 'instock_to_sale': {
        ...
        // 若訂單曾核准過（有 approved 記錄），即使目前是 pending_approval 也允許（PO到貨場景）
        const allowedStatuses = ['confirmed', 'processing'];
        if (!allowedStatuses.includes(order.status)) {
          // Check if order was previously approved (PO arrival scenario)
          const { data: prevApproval } = await supabase.from('erp_approvals')
            .select('id')
            .eq('doc_type', 'order').eq('doc_id', order_id).eq('status', 'approved')
            .limit(1).maybeSingle();
          if (prevApproval) {
            // Order was previously approved — allow and restore to processing + audit log
            ...
            await supabase.from('erp_approvals').insert({
              ...
              remark: `PO 到貨自動恢復 processing（原核准紀錄 ${prevApproval.id}），允許處理剩餘品項`,
            });
        ...
        const taxableBase = Math.max(0, saleTotal - orderDiscount + orderShipping);
        ...
        // 方案 A：銷貨免審 — 直接扣庫存 + 記錄庫存異動
        for (let idx = 0; idx < processItems.length; idx++) {
          const item = processItems[idx];
          const deductQty = processQtys[idx];
          const itemNo = item.item_number_snapshot;
          if (!itemNo || !deductQty) continue;
          const { data: prod } = await supabase.from('quickbuy_products').select('stock_qty').eq('item_number', itemNo).maybeSingle();
          if (prod) {
            const newStock = Math.max(0, Number(prod.stock_qty || 0) - deductQty);
            await supabase.from('quickbuy_products').update({ stock_qty: newStock }).eq('item_number', itemNo);
          }
          await supabase.from('qb_inventory_movements').insert({
            item_number: itemNo, movement_type: 'out', quantity: deductQty,
            reference_type: 'sale_issued', reference_id: String(sale.id),
            notes: `銷貨出庫 ${slipNumber}（自動核准）`, created_by: 'admin',
          });
        }

        // 方案 A：銷貨免審 — 訂單已審核，銷貨自動核准（記錄用）
        const { error: approvalErr } = await supabase.from('erp_approvals').insert([{
          ...
          amount: saleTotal,
          status: 'approved',
          remark: `訂單 ${order.order_no} 已核准，銷貨自動通過（含 ${processItems.length} 項）`,
        }]);
      ...
      case 'update_sale_invoice': {
        const { sale_id, invoice_number, invoice_date, remark } = body;
        if (!sale_id) return Response.json({ error: 'sale_id 為必填' }, { status: 400 });
        const salePayload = {};
      ...
      case 'submit_approval': {
        ...
        // Guard: prevent re-submitting orders that already have approved history + linked sales
        if (doc_type === 'order') {
      ...
      case 'update_quote_item': {
        ...
        const taxableBase = Math.max(0, subtotal - Number(quote?.discount_amount || 0) + Number(quote?.shipping_fee || 0));
      ...
      case 'receive_po_items': {
        ...
        await insertManyWithColumnFallback(supabase, 'erp_stock_in_items', siItemsPayload);

        // Update inventory
        for (const it of stockInItems) {
          if (!it.item_number) continue;
          const { data: inv } = await supabase.from('products').select('id, stock_qty').eq('item_number', it.item_number).maybeSingle();
          if (inv) {
            await supabase.from('products').update({ stock_qty: (inv.stock_qty || 0) + it.qty_received }).eq('id', inv.id);
          }
        }
      ...
      case 'update_erp_features': {
        ...
        return Response.json({ success: true, features: merged, message: '功能設定已更新' });
      }
