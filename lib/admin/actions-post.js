...
export async function handlePostAction(action, body) {
  switch (action) {
      ...
      case 'instock_to_sale': {
        ...
        // 運費：僅在「所有可出品項都在此次轉銷貨」時帶入，避免部分出貨重複計
        const allRemaining = (items || []).filter(i => {
          const rem = Number(i.qty || 0) - Number(i.sold_qty || 0);
          return rem > 0 && i.item_number_snapshot;
        });
        const isFullShipment = processItems.length === allRemaining.length &&
          processItems.every((pi, idx) => processQtys[idx] >= (Number(pi.qty || 0) - Number(pi.sold_qty || 0)));
        const orderShipping = isFullShipment ? Number(order.shipping_fee || 0) : 0;
      ...
      case 'receive_po_items': {
        ...
        await insertManyWithColumnFallback('erp_stock_in_items', siItemsPayload);
