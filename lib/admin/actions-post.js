  ...
  switch (action) {
      ...
      case 'update_sale_invoice': {
        ...
        if (invoice_number) {
          ...
          // 透過 order_id 找已存在的發票記錄（最準確）
          let existingInvId = null;
          if (linkedOrderId) {
            const { data: existByOrder } = await supabase.from('erp_invoices').select('id').eq('order_id', linkedOrderId).maybeSingle();
            existingInvId = existByOrder?.id || null;
          }
          // Fallback：透過 invoice_no 找
          if (!existingInvId) {
