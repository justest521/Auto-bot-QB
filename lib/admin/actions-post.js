...
export async function handlePostAction(action, body) {
  switch (action) {
      ...
      /* ── 財務核帳（僅會計/管理員） ── */
      case 'verify_payment': {
        const vpRole = body.__auth_user?.role;
        if (vpRole !== 'admin' && vpRole !== 'accountant') {
          return Response.json({ error: '僅會計人員或管理員可核帳' }, { status: 403 });
        }
      ...
      /* ── 批次核帳：一次核帳某訂單的所有收款（僅會計/管理員） ── */
      case 'verify_order_payments': {
        const voRole = body.__auth_user?.role;
        if (voRole !== 'admin' && voRole !== 'accountant') {
          return Response.json({ error: '僅會計人員或管理員可核帳' }, { status: 403 });
        }
