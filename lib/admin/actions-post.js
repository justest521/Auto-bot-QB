...
export async function handlePostAction(action, body) {
  switch (action) {
      ...
      case 'instock_to_sale': {
        ...
        // 更新訂單狀態：檢查是否所有品項已轉完
        const { data: allOrderItems } = await supabase.from('erp_order_items').select('qty, sold_qty').eq('order_id', order_id);
        const allFulfilled = (allOrderItems || []).every(i => Number(i.sold_qty || 0) >= Number(i.qty || 0));
        const newOrderStatus = allFulfilled ? 'completed' : 'processing';
        await supabase.from('erp_orders').update({ status: newOrderStatus, updated_at: new Date().toISOString() }).eq('id', order_id);
