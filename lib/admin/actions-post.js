  ...
  switch (action) {
      ...
      case 'update_sale_invoice': {
        ...
        if (invoice_number) {
          ...
          // 先查是否有對應的 erp_invoices（透過 sale 的 source_id 找 order_id）
          const { data: saleRef } = await supabase.from('qb_sales_history').select('source_id').eq('id', sale_id).maybeSingle();
