...
export async function handlePostAction(action, body) {
  switch (action) {
      ...
      case 'create_shipment': {
        ...
        // Guard: insertSingleWithColumnFallback may return {data:null, error:null} in edge cases
        ...
        // Key: String(order_item_id) preferred, fallback to String(product_id)
        // This handles items where product_id is null (custom/manual items)
        let alreadyShippedMap = {};
        if (sale_id || orderId) {
          const shipQuery = sale_id
            ? supabase.from('erp_shipment_items').select('order_item_id, product_id, qty_shipped, erp_shipments!inner(sale_id, status)').eq('erp_shipments.sale_id', sale_id).neq('erp_shipments.status', 'cancelled')
            : supabase.from('erp_shipment_items').select('order_item_id, product_id, qty_shipped, erp_shipments!inner(order_id, status)').eq('erp_shipments.order_id', orderId).neq('erp_shipments.status', 'cancelled');
          const { data: prevShipped } = await shipQuery;
          (prevShipped || []).forEach(si => {
            const key = String(si.order_item_id || si.product_id || '');
        ...
        if (safeItems.length === 0 && sale_id) {
          ...
          // Note: instock_to_sale stores sale items in qb_order_items with order_id = sale.id (not sale_id column)
          const { data: saleItems } = await supabase.from('qb_order_items').select('id, product_id, item_number, quantity').eq('order_id', sale_id);
          safeItems = (saleItems || []).map(si => {
            const ordered = Number(si.quantity || 0);
            // Use si.id (qb_order_items PK) as primary key; product_id as fallback
            const shipped = alreadyShippedMap[String(si.id)] || alreadyShippedMap[String(si.product_id || '')] || 0;
            const remaining = Math.max(0, ordered - shipped);
            return { id: si.id, product_id: si.product_id, qty_shipped: remaining };
        ...
        if (safeItems.length === 0 && orderId) {
          ...
          safeItems = (oi || []).map(o => {
            ...
            // Use o.id (erp_order_items PK) as primary key; product_id as fallback
            const shipped = alreadyShippedMap[String(o.id)] || alreadyShippedMap[String(o.product_id || '')] || 0;
            const remaining = Math.max(0, ordered - shipped);
        ...
        if (prFinalStatus === 'approved') {
          ...
          if (prDeduct > 0 && prRet.stock_in_id) {
            const { data: linkedAP } = await supabase.from('erp_vendor_payables')
              .select('*').eq('source_type', 'stock_in').eq('source_id', prRet.stock_in_id).maybeSingle();
            if (linkedAP) {
