  ...
  switch (action) {
      ...
      case 'instock_to_sale': {
        ...
        const saleItems = processItems.map((i, idx) => ({
          ...
          subtotal: Number(i.unit_price) * processQtys[idx],
        }));
        await supabase.from('qb_order_items').insert(saleItems);
        ...
        for (let idx = 0; idx < processItems.length; idx++) {
          ...
          await supabase.from('erp_order_items').update(updateData).eq('id', item.id);
        }
      ...
      case 'shortage_to_po': {
        ...
        const { data: order } = await supabase.from('erp_orders').select('*').eq('id', poOrderId).single();
        if (!order) return Response.json({ error: 'Order not found' }, { status: 404 });
        ...
        await supabase.from('erp_purchase_order_items').insert(poItems);
      ...
      case 'submit_approval': {
        const { doc_type, doc_id, doc_no, requested_by, amount } = body;
        ...
        const { data, error } = await supabase.from('erp_approvals').insert({
          doc_type, doc_id, doc_no: doc_no || '', requested_by: requested_by || 'system', status: 'pending', amount: finalAmount,
        }).select().single();
      ...
      case 'process_approval': {
        ...
        if (newStatus === 'approved') {
          if (approval.doc_type === 'purchase_order') {
            ...
            if (sale) {
              const { data: saleItems } = await supabase.from('qb_order_items').select('item_number, quantity').eq('order_id', sale.id);
              for (const item of (saleItems || [])) {
                ...
                if (product) {
                  ...
                  await supabase.from('quickbuy_products').update({ stock_qty: newStock }).eq('item_number', item.item_number);
                }
                // Record inventory movement
                await supabase.from('qb_inventory_movements').insert({
              ...
              // Update sale status to issued
              await supabase.from('qb_sales_history').update({ status: 'issued', updated_at: new Date().toISOString() }).eq('id', approval.doc_id);
            }
            ...
            if (ret) {
              ...
              await supabase.from('erp_purchase_returns').update({ status: 'approved', updated_at: new Date().toISOString() }).eq('id', approval.doc_id);
            }
          }
        }
      ...
      case 'delete_quote': {
        const { quote_id } = body;
        if (!quote_id) return Response.json({ error: 'Missing quote_id' }, { status: 400 });
        const { data: quote } = await supabase.from('erp_quotes').select('status').eq('id', quote_id).single();
