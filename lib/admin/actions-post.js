...
export async function handlePostAction(action, body) {
  ...
  switch (action) {
      ...
      case 'convert_quote_to_order': {
        ...
        // 確保 sales_person 有正確寫入（insertSingleWithColumnFallback 可能在重試時丟掉）
        if (quote.sales_person && order.sales_person !== quote.sales_person) {
          await supabase.from('erp_orders').update({ sales_person: quote.sales_person }).eq('id', order.id);
          order.sales_person = quote.sales_person;
        }
      ...
      case 'create_purchase_order': {
        ...
        if (!vendor_id) return Response.json({ error: '廠商為必填，請先選擇廠商' }, { status: 400 });
        const safeItems = Array.isArray(items) ? items.filter(Boolean) : [];
      ...
      /* 確認入庫前更新數量 */
      case 'update_stock_in_quantities': {
        const { stock_in_id, items: updateItems } = body;
        if (!stock_in_id || !updateItems?.length) return Response.json({ error: 'stock_in_id and items required' }, { status: 400 });
        // Only allow updates on pending stock-ins
        const { data: siCheck } = await supabase.from('erp_stock_ins').select('status').eq('id', stock_in_id).maybeSingle();
        if (!siCheck) return Response.json({ error: '找不到進貨單' }, { status: 404 });
        if (siCheck.status !== 'pending') return Response.json({ error: '已入庫的進貨單不可修改數量' }, { status: 400 });
        // Update each item's qty_received and recalculate line_total
        for (const item of updateItems) {
          const qty = Math.max(0, Number(item.qty_received || 0));
          const { data: existing } = await supabase.from('erp_stock_in_items').select('unit_cost').eq('id', item.id).maybeSingle();
          const lineTotal = qty * Number(existing?.unit_cost || 0);
          await supabase.from('erp_stock_in_items').update({ qty_received: qty, line_total: lineTotal }).eq('id', item.id);
        }
        // Recalculate stock-in total
        const { data: allItems } = await supabase.from('erp_stock_in_items').select('line_total').eq('stock_in_id', stock_in_id);
        const newTotal = (allItems || []).reduce((s, i) => s + Number(i.line_total || 0), 0);
        await supabase.from('erp_stock_ins').update({ total_amount: newTotal }).eq('id', stock_in_id);
        return Response.json({ success: true, message: '數量已更新', total_amount: newTotal });
      }
      ...
      case 'update_order_sales_person': {
        const { order_id, sales_person } = body;
        if (!order_id) return Response.json({ error: 'order_id 為必填' }, { status: 400 });
        const { error } = await supabase.from('erp_orders').update({ sales_person: sales_person || null }).eq('id', order_id);
        if (error) return Response.json({ error: error.message }, { status: 500 });
        return Response.json({ success: true, message: '業務已更新' });
      }
