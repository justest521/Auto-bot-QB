...
export async function handlePostAction(action, body) {
  switch (action) {
      ...
      case 'quick_stock_in': {
        ...
        const itemPayload = safeItems.map(it => ({
          stock_in_id: si.id,
          item_number: cleanCsvValue(it.part_no),
        ...
        for (const it of safeItems) {
          const itemNumber = cleanCsvValue(it.part_no);
      ...
      case 'parse_receive_image': {
        ...
        try {
          ...
          if (file_hash) {
            ...
            if (cached?.items) {
              // 更新命中次數
              await supabase.from('receive_parse_cache')
                .update({ hit_count: supabase.rpc ? undefined : 1, last_hit_at: new Date().toISOString() })
                .eq('file_hash', file_hash);
              await supabase.rpc('increment_cache_hit', { hash_val: file_hash }).catch(() => {});
              return Response.json({ items: cached.items, cached: true });
            }
          }

          // ── AI 解析 ──
          // PDF 用 document 類型，圖片用 image 類型
          ...
          return Response.json({ items: result, cached: false });
        } catch (e) {
