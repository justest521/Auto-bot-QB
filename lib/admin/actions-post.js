...
export async function handlePostAction(action, body) {
  switch (action) {
      ...
      case 'record_payment': {
        ...
        // === Auto-create erp_payment_receipts from invoice payment ===
        try {
          const rcptNo = `RCPT-${new Date().toISOString().replace(/[-:TZ.]/g, '').slice(0, 14)}`;
          await supabase.from('erp_payment_receipts').insert({
            receipt_no: rcptNo,
          ...
          console.error('Auto-create receipt from invoice payment error:', rcptErr);
        }
      ...
      case 'instock_to_sale': {
        ...
        const saleTax = Math.round(taxableBase * 0.05);
