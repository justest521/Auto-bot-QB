  ...
  switch (action) {
      ...
      case 'update_sale_invoice': {
        ...
        if (invoice_number) {
          ...
          if (invoice_date) erpInvFields.invoice_date = invoice_date;

          // 查 sale 基本資料（source_id = erp_orders.id, total 用於新行的 total_amount）
          ...
          // 優先用 invoice_no 精確查詢（同一訂單可能有多張發票，order_id 不夠精確）
          let existingInvId = null;
          const { data: existByNo } = await supabase.from('erp_invoices').select('id').eq('invoice_no', invoice_number).limit(1).maybeSingle();
          existingInvId = existByNo?.id || null;
          // Fallback：invoice_no 找不到時，用 order_id 找「尚未綁定發票號碼」的記錄
          if (!existingInvId && linkedOrderId) {
            const { data: existByOrder } = await supabase.from('erp_invoices').select('id, invoice_no').eq('order_id', linkedOrderId).maybeSingle();
            if (existByOrder && !existByOrder.invoice_no) {
              existingInvId = existByOrder.id;
            }
          }

          if (existingInvId) {
            await supabase.from('erp_invoices').update(erpInvFields).eq('id', existingInvId);
          } else {
            // 沒有既有記錄，插入新記錄（帶完整欄位避免應收帳款出現空行）
            await supabase.from('erp_invoices').insert({
              invoice_no:     invoice_number,
            ...
            // 新增行後，重新依訂單已付款金額分配各發票的 paid_amount
            // （避免舊行 paid_amount 超額、新行永遠為 0 的情況）
            if (linkedOrderId) {
              try {
