  ...
  return (
    <div>
      ...
      <div style={{ ...S.card, marginBottom: 18, background: '#fff8eb', borderColor: '#f7d699' }}>
        ...
        {resetStatus ? (
          <div style={{ ...S.panelMuted, marginBottom: 12, background: resetStatus.includes('已清空') ? t.color.successBg : '#fff4f4', borderColor: resetStatus.includes('已清空') ? '#bdeccb' : '#ffc7cf', color: resetStatus.includes('已清空') ? '#127248' : t.color.error }}>
            {resetStatus}
          </div>
        ) : null}
      ...
      <div style={{ display: 'grid', gap: t.spacing.lg }}>
        {Object.entries(IMPORT_DATASETS).map(([datasetId, dataset]) => (
          <div key={datasetId} style={S.card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: isMobile ? 'stretch' : 'flex-start', gap: 12, flexWrap: 'wrap', flexDirection: isMobile ? 'column' : 'row' }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: t.fontSize.h2, color: t.color.textPrimary, fontWeight: t.fontWeight.bold }}>{dataset.title}</div>
                <div style={{ fontSize: t.fontSize.body, color: t.color.textSecondary, marginTop: 6, lineHeight: 1.7 }}>{dataset.desc}</div>
                <div style={{ fontSize: t.fontSize.tiny, color: t.color.textMuted, marginTop: 8, ...S.mono }}>{dataset.fields}</div>
              </div>
              ...
              return (
                <div key={`${entry.imported_at || 'history'}-${index}`} style={{ ...S.panelMuted, display: 'grid', gap: 6 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: isMobile ? 'flex-start' : 'center', gap: 12, flexWrap: 'wrap', flexDirection: isMobile ? 'column' : 'row' }}>
                    <div style={{ fontSize: t.fontSize.body, color: t.color.textPrimary, fontWeight: t.fontWeight.bold }}>
                      {dataset?.title || entry.dataset || '未知資料集'}
                    </div>
                    <div style={{ fontSize: t.fontSize.tiny, color: t.color.textMuted, ...S.mono }}>{fmtDate(entry.imported_at)}</div>
                  </div>
                  <div style={{ fontSize: t.fontSize.body, color: t.color.textSecondary, lineHeight: 1.7 }}>
                    <div><span style={{ color: t.color.textMuted, ...S.mono }}>FILE</span> {entry.file_name || '-'}</div>
                    <div><span style={{ color: t.color.textMuted, ...S.mono }}>ROWS</span> {fmt(entry.count || 0)} 筆</div>
                    {'inserted' in entry || 'updated' in entry ? (
                      <div><span style={{ color: t.color.textMuted, ...S.mono }}>DETAIL</span> 新增 {fmt(entry.inserted || 0)} / 更新 {fmt(entry.updated || 0)}</div>
                    ) : null}
                    <div><span style={{ color: t.color.textMuted, ...S.mono }}>BY</span> {entry.imported_by || 'admin'}</div>
                  </div>
                </div>
              );
            })}
