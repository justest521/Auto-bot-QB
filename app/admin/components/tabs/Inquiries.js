  ...
  return (
    <div>
      ...
      <div style={{ ...S.statGrid, gap: 10, marginBottom: 10 }}>
        <StatCard code="OPEN" label="待處理" value={fmt(sm.open)} tone="blue" accent={t.color.warning} />
        <StatCard code="QUOT" label="已報價" value={fmt(sm.quoted)} tone="blue" accent={t.color.info} />
        <StatCard code="CLSD" label="已結案" value={fmt(sm.closed)} tone="blue" accent={t.color.brand} />
      </div>
      ...
      {loading ? <Loading /> : data.inquiries.length === 0 ? <EmptyState text="目前沒有詢價記錄" /> : data.inquiries.map(inq => (
        <div key={inq.id} style={{ ...S.card, padding: '10px 16px', marginBottom: 10 }}>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '130px minmax(0,1fr) 80px 100px 140px', gap: 12, alignItems: 'center' }}>
            <div><div style={{ fontSize: t.fontSize.tiny, color: t.color.textMuted, ...S.mono }}>INQ_NO</div><div style={{ fontSize: t.fontSize.h3, fontWeight: t.fontWeight.bold, color: t.color.link, ...S.mono }}>{inq.inquiry_no || '-'}</div></div>
            <div><div style={{ fontSize: t.fontSize.h3, fontWeight: t.fontWeight.semibold, color: t.color.textPrimary }}>{inq.subject || '-'}</div><div style={{ fontSize: t.fontSize.caption, color: t.color.textSecondary, marginTop: 4 }}>{(inq.description || '').slice(0, 80)}{(inq.description || '').length > 80 ? '...' : ''}</div></div>
            <div><span style={S.tag(priorityColor(inq.priority))}>{inq.priority || 'normal'}</span></div>
            <div><div style={{ fontSize: t.fontSize.caption }}>{fmtDate(inq.inquiry_date || inq.created_at)}</div></div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={S.tag(statusColor(inq.status))}>{statusLabel(inq.status)}</span>
              {inq.status === 'open' && <button onClick={() => handleStatus(inq.id, 'quoted')} style={{ ...S.btnGhost, padding: '3px 8px', fontSize: t.fontSize.tiny }}>已報價</button>}
              {inq.status === 'quoted' && <button onClick={() => handleStatus(inq.id, 'closed')} style={{ ...S.btnGhost, padding: '3px 8px', fontSize: t.fontSize.tiny }}>結案</button>}
            </div>
          </div>
        </div>
      ))}
      ...
      {createOpen && (
        <div style={{ position: 'fixed', inset: 0, background: t.color.overlay, zIndex: 999, display: 'flex', alignItems: isMobile ? 'flex-end' : 'center', justifyContent: 'center', padding: isMobile ? 0 : 20 }}>
          <div style={{ ...S.card, width: isMobile ? '100%' : 440, maxWidth: isMobile ? '100%' : '90vw', maxHeight: isMobile ? '90vh' : 'auto', overflowY: isMobile ? 'auto' : 'visible', borderRadius: isMobile ? '16px 16px 0 0' : t.radius.xl, padding: '10px 16px' }}>
            <h3 style={{ margin: '0 0 12px', fontSize: t.fontSize.h2 }}>新增詢價</h3>
