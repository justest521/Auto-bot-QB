    ...
    <div>
      ...
      <div style={{ ...S.card, marginBottom: 10, padding: isMobile ? '12px 14px' : '10px 16px' }}>
        <div style={{ display: 'flex', gap: isMobile ? 6 : 8, flexWrap: 'wrap', alignItems: 'center' }}>
          <input
            ...
            placeholder="搜尋客戶..."
            style={{ ...S.input, flex: '1 1 auto', minWidth: isMobile ? 0 : 180, fontSize: 13, padding: isMobile ? '8px 10px' : '6px 10px' }}
          />
          {customerSearch && customers.filter(c => (c.name || '').toLowerCase().includes(customerSearch.toLowerCase())).length > 0 && (
            <div style={{ position: 'absolute', top: isMobile ? 'auto' : 220, left: isMobile ? 16 : 'auto', right: isMobile ? 16 : 'auto', background: '#fff', border: '1px solid #e5e7eb', borderRadius: 6, maxHeight: 200, overflow: 'auto', zIndex: 100, width: isMobile ? 'calc(100% - 32px)' : 280 }}>
              {customers.filter(c => (c.name || '').toLowerCase().includes(customerSearch.toLowerCase())).map(c => (
                <div key={c.id} onClick={() => handleCustomerSelect(c.id)} style={{ padding: '10px 12px', fontSize: 13, borderBottom: '1px solid #f3f4f6', cursor: 'pointer', background: customerId === c.id ? '#dbeafe' : '#fff' }}>
                  {c.name}
                </div>
              ))}
            </div>
          ...
          {customerId && (
            <button onClick={() => { setCustomerId(''); loadData(''); }} style={{ ...S.btnGhost, padding: isMobile ? '6px 12px' : '6px 14px', fontSize: 13 }}>清除篩選</button>
          )}
        </div>
      </div>
      ...
      {loading ? <Loading /> : (
        <div style={{ display: isMobile ? 'block' : 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(2, 1fr)', gap: isMobile ? 12 : 16 }}>
                ...
                {(receipts || []).map(receipt => (
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: 10 }}>
                      <div>
                        <div style={{ fontSize: 14, fontWeight: 700, color: '#2563eb', ...S.mono }}>{receipt.receipt_no || '-'}</div>
                        <div style={{ fontSize: 12, color: '#6b7280', marginTop: 4 }}>{receipt.customer_name || '-'}</div>
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: 12, color: '#6b7280' }}>收款日期</div>
                        <div style={{ fontSize: 12, color: '#111827', ...S.mono }}>{receipt.receipt_date?.slice(0, 10) || '-'}</div>
                      </div>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, fontSize: 12, borderTop: '1px solid #e5e7eb', paddingTop: 10 }}>
                      <div>
                        <span style={{ color: '#6b7280' }}>收款金額</span>
                        <div style={{ fontSize: 14, fontWeight: 700, color: '#111827', ...S.mono }}>{fmtP(receipt.total_amount)}</div>
                      </div>
                      <div>
                        <span style={{ color: '#6b7280' }}>已沖金額</span>
                        <div style={{ fontSize: 13, fontWeight: 700, color: '#16a34a', ...S.mono }}>{fmtP(receipt.allocated_amount)}</div>
                      </div>
                      <div>
                        <span style={{ color: '#6b7280' }}>未沖金額</span>
                        <div style={{ fontSize: 13, fontWeight: 700, color: Number(receipt.remaining_amount) > 0 ? '#dc2626' : '#6b7280', ...S.mono }}>{fmtP(receipt.remaining_amount)}</div>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div>
            <h3 style={{ margin: '0 0 12px', fontSize: 16, fontWeight: 700, color: '#111827' }}>未付應收</h3>
            {selectedReceipt ? (
              <div>
                {isMobile && (
                  <div style={{ ...S.card, padding: '12px 14px', marginBottom: 12, background: '#f0f9ff', borderColor: '#2563eb' }}>
                    <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 6 }}>已選收款單</div>
                    <div style={{ fontSize: 14, fontWeight: 700, color: '#2563eb', ...S.mono }}>{selectedReceipt.receipt_no}</div>
                    <div style={{ fontSize: 12, color: '#111827', marginTop: 4 }}>未沖金額：<span style={{ fontWeight: 700, color: '#dc2626', ...S.mono }}>{fmtP(selectedReceipt.remaining_amount)}</span></div>
                  </div>
                )}
                {(invoices || []).length === 0 ? (
                        ...
                        <div key={inv.id} style={{ ...S.card, padding: '12px 14px' }}>
                          <div style={{ display: 'flex', gap: 10, alignItems: 'start', marginBottom: 10 }}>
                            ...
                            <div style={{ flex: 1 }}>
                              ...
                              <div style={{ fontSize: 12, color: '#6b7280', marginTop: 4 }}>{inv.customer_name || '-'}</div>
                            </div>
                          </div>
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, fontSize: 12, borderTop: '1px solid #e5e7eb', paddingTop: 10, marginBottom: 10 }}>
                            <div>
                              <span style={{ color: '#6b7280' }}>應收金額</span>
                              <div style={{ fontSize: 13, fontWeight: 700, color: '#111827', ...S.mono }}>{fmtP(inv.total_amount)}</div>
                            </div>
                            <div>
                              <span style={{ color: '#6b7280' }}>已付金額</span>
                              <div style={{ fontSize: 13, fontWeight: 700, color: '#16a34a', ...S.mono }}>{fmtP(inv.paid_amount)}</div>
                            </div>
                            <div>
                              <span style={{ color: '#6b7280' }}>餘額</span>
                              <div style={{ fontSize: 13, fontWeight: 700, color: balance > 0 ? '#dc2626' : '#6b7280', ...S.mono }}>{fmtP(balance)}</div>
                            </div>
                            <div>
                              <span style={{ color: '#6b7280' }}>到期日</span>
                              <div style={{ fontSize: 12, color: '#111827', ...S.mono }}>{inv.due_date?.slice(0, 10) || '-'}</div>
                            </div>
                          </div>
                          {isChecked && (
                            <div style={{ marginBottom: 0 }}>
                              <label style={{ ...S.label, fontSize: 12 }}>沖帳金額</label>
                              <input
                                type="number"
                                value={allocations[inv.id] || ''}
                                onChange={(e) => handleAllocationAmount(inv.id, e.target.value)}
                                style={{ ...S.input, fontSize: 13, padding: '6px 10px', ...S.mono }}
                              />
                            </div>
                          )}
                        </div>
      ...
      {selectedReceipt && !isMobile && (
        <div style={{ ...S.card, marginTop: 16, padding: '14px 16px', background: '#f9fafb', borderTop: '2px solid #e5e7eb' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 14 }}>
            <div>
              <div style={{ fontSize: 12, color: '#6b7280' }}>收款單</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: '#2563eb', marginTop: 4, ...S.mono }}>{selectedReceipt.receipt_no}</div>
            </div>
            <div>
              <div style={{ fontSize: 12, color: '#6b7280' }}>未沖金額</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: '#dc2626', marginTop: 4, ...S.mono }}>{fmtP(receiptRemaining)}</div>
            </div>
            <div>
              <div style={{ fontSize: 12, color: '#6b7280' }}>本次沖帳</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: totalAllocated > receiptRemaining ? '#dc2626' : '#16a34a', marginTop: 4, ...S.mono }}>{fmtP(totalAllocated)}</div>
            </div>
            <div>
              <div style={{ fontSize: 12, color: '#6b7280' }}>沖帳後餘額</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: receiptRemaining - totalAllocated > 0 ? '#f59e0b' : '#16a34a', marginTop: 4, ...S.mono }}>{fmtP(receiptRemaining - totalAllocated)}</div>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
            <button onClick={() => { setSelectedReceipt(null); setAllocations({}); }} style={{ ...S.btnGhost, padding: '8px 18px', fontSize: 13 }}>取消</button>
            <button onClick={() => setWriteOffDialog(selectedReceipt.id)} style={{ ...S.btnGhost, padding: '8px 18px', fontSize: 13 }}>折讓沖帳</button>
            <button onClick={() => { setWriteOffType('bad_debt'); setWriteOffDialog(selectedReceipt.id); }} style={{ ...S.btnGhost, padding: '8px 18px', fontSize: 13 }}>壞帳沖銷</button>
            <button onClick={handleExecuteMatching} disabled={Object.keys(allocations).length === 0 || totalAllocated > receiptRemaining} style={{ ...S.btnPrimary, padding: '8px 20px', fontSize: 13, opacity: (Object.keys(allocations).length === 0 || totalAllocated > receiptRemaining) ? 0.5 : 1, cursor: (Object.keys(allocations).length === 0 || totalAllocated > receiptRemaining) ? 'not-allowed' : 'pointer' }}>執行沖帳</button>
          </div>
        </div>
      )}
      ...
      {isMobile && selectedReceipt && (
        <div style={{ ...S.card, marginTop: 12, padding: '12px 14px', background: '#f9fafb' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
            <div>
              <div style={{ fontSize: 11, color: '#6b7280' }}>未沖金額</div>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#dc2626', marginTop: 3, ...S.mono }}>{fmtP(receiptRemaining)}</div>
            </div>
            <div>
              <div style={{ fontSize: 11, color: '#6b7280' }}>本次沖帳</div>
              <div style={{ fontSize: 13, fontWeight: 700, color: totalAllocated > receiptRemaining ? '#dc2626' : '#16a34a', marginTop: 3, ...S.mono }}>{fmtP(totalAllocated)}</div>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8, flexDirection: 'column' }}>
            ...
            <button onClick={() => setWriteOffDialog(selectedReceipt.id)} style={{ ...S.mobile.btnPrimary, background: '#f3f4f6', color: '#6b7280', border: '1px solid #e5e7eb' }}>折讓沖帳</button>
            <button onClick={() => { setWriteOffType('bad_debt'); setWriteOffDialog(selectedReceipt.id); }} style={{ ...S.mobile.btnPrimary, background: '#f3f4f6', color: '#6b7280', border: '1px solid #e5e7eb' }}>壞帳沖銷</button>
          </div>
        </div>
      )}
      ...
      {writeOffDialog && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ ...S.card, width: isMobile ? '90vw' : 420, maxWidth: '90vw', borderRadius: 14, padding: isMobile ? '16px 18px' : '20px 22px' }}>
            <h3 style={{ margin: '0 0 14px', fontSize: 16, fontWeight: 700 }}>{writeOffType === 'discount' ? '折讓沖帳' : '壞帳沖銷'}</h3>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', flexDirection: isMobile ? 'column-reverse' : 'row' }}>
              <button onClick={() => { setWriteOffDialog(null); setWriteOffAmount(''); setWriteOffRemark(''); setWriteOffType('discount'); }} style={{ ...(isMobile ? { ...S.mobile.btnPrimary, background: '#f3f4f6', color: '#6b7280', border: '1px solid #e5e7eb' } : S.btnGhost) }}>取消</button>
