      ...
      <div style={{ width: isMobile ? '100%' : 'min(1280px, 100%)', maxHeight: isMobile ? '90vh' : '92vh', overflowY: 'auto', background: '#f6f9fc', borderRadius: isMobile ? '16px 16px 0 0' : 14, padding: isMobile ? '12px 16px' : '16px 18px 20px', boxShadow: '0 24px 70px rgba(8,12,20,0.3)' }} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
          <div>
            ...
            <div style={{ fontSize: t.fontSize.h1, fontWeight: t.fontWeight.bold, color: t.color.textPrimary }}>建立報價單</div>
            <div style={{ fontSize: t.fontSize.tiny, color: t.color.textMuted, marginTop: 6 }}>先建立基本報價單，之後就能往訂單與銷貨流程接。</div>
          </div>
          <div style={{ display: 'flex', gap: 6, flexDirection: isMobile ? 'column' : 'row' }}>
            {(selectedCustomer || form.items.length > 0) && <button onClick={() => { if (confirm('確定清除所有已填資料？')) resetAll(); }} style={{ ...S.btnGhost, ...(isMobile && { width: '100%' }), fontSize: t.fontSize.tiny, color: t.color.error, borderColor: '#fecaca' }}>清除</button>}
            <button onClick={guardedClose} style={{ ...S.btnGhost, ...(isMobile && { width: '100%' }) }}>關閉</button>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1.1fr 0.9fr', gap: 10 }}>
          <div style={{ display: 'grid', gap: 10 }}>
            <div style={{ ...S.card, minHeight: 180 }}>
                      ...
                      {customerResults.map((customer) => {
                        ...
                        return (
                          <button key={customer.id} onClick={() => selectCustomer(customer)} style={{ ...S.panelMuted, width: '100%', textAlign: 'left', cursor: 'pointer', border: '1px solid #f0f2f5', background: '#fafbfc' }}>
                            <div style={{ fontSize: t.fontSize.body, color: t.color.textPrimary, fontWeight: t.fontWeight.bold }}>{displayName}</div>
                            {contactName && <div style={{ fontSize: t.fontSize.tiny, color: t.color.textMuted, marginTop: 2 }}>{contactName}</div>}
                            <div style={{ fontSize: t.fontSize.tiny, color: t.color.textSecondary, marginTop: 4 }}>{info || '-'}</div>
                          </button>
                        );
                      })}
                      <button onClick={() => { setShowNewCustomer(true); setNewCustomer(prev => ({ ...prev, name: customerSearch.trim() })); }} style={{ ...S.panelMuted, width: '100%', textAlign: 'center', cursor: 'pointer', color: t.color.link, borderColor: '#93c5fd', fontSize: t.fontSize.caption, fontWeight: t.fontWeight.semibold }}>都不是？新增客戶</button>
                    </div>
                  )}
              ...
              {selectedCustomer ? (
                <div style={{ ...S.panelMuted, borderColor: '#bde6c9', background: '#f4fbf6', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                  <div>
                    <div style={{ fontSize: t.fontSize.tiny, color: '#10b981', fontWeight: t.fontWeight.bold, ...S.mono }}>SELECTED CUSTOMER</div>
                    ...
                    <div style={{ fontSize: t.fontSize.tiny, color: t.color.textSecondary, marginTop: 4 }}>{selectedCustomer.customer_code || '-'} · {selectedCustomer.phone || '-'}</div>
                  </div>
                  <button onClick={() => setSelectedCustomer(null)} style={{ width: 28, height: 28, flexShrink: 0, borderRadius: 8, border: '1px solid #e5e7eb', background: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: t.fontSize.h2, color: t.color.textDisabled, transition: 'all 0.15s' }} title="更換客戶">&times;</button>
                </div>
              ) : showNewCustomer ? (
                <div style={{ ...S.panelMuted, display: 'grid', gap: 8 }}>
                  <div style={{ fontSize: t.fontSize.caption, fontWeight: t.fontWeight.bold, color: t.color.textPrimary }}>新增客戶</div>
                  ...
                  {dupWarning && (
                    <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8, padding: '8px 12px', fontSize: t.fontSize.tiny }}>
                      <div style={{ fontWeight: t.fontWeight.bold, color: '#dc2626', marginBottom: 4 }}>偵測到 {dupWarning.duplicates?.length} 筆重複客戶：</div>
                      {(dupWarning.duplicates || []).map((d, i) => (
                        <div key={i} style={{ display: 'flex', gap: 6, alignItems: 'center', padding: '2px 0', color: t.color.textSecondary }}>
                          ...
                          <span style={{ background: '#fee2e2', color: '#dc2626', borderRadius: 4, padding: '0px 5px', fontSize: t.fontSize.tiny, fontWeight: t.fontWeight.bold }}>符合：{(d.matchFields || []).join('、')}</span>
            ...
            <div style={{ ...S.card, minHeight: 200, marginBottom: 10 }}>
                  ...
                  <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 50, marginTop: 6, background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, boxShadow: '0 8px 24px rgba(0,0,0,0.12)', padding: 8, display: 'grid', gap: 6, maxHeight: 260, overflowY: 'auto' }}>
                    {productResults.map((product) => (
                      <button key={product.item_number} onClick={() => addProduct(product)} style={{ ...S.panelMuted, width: '100%', textAlign: 'left', cursor: 'pointer', border: '1px solid #f0f2f5', background: '#fafbfc' }}>
                        <div style={{ fontSize: t.fontSize.tiny, color: t.color.link, fontWeight: t.fontWeight.bold, ...S.mono }}>{product.item_number}</div>
                        <div style={{ fontSize: t.fontSize.caption, color: t.color.textPrimary, marginTop: 2 }}>{product.description || '-'}</div>
                        <div style={{ fontSize: t.fontSize.tiny, color: '#10b981', marginTop: 2, ...S.mono }}>{fmtP(product.tw_retail_price || product.tw_reseller_price)}</div>
                      </button>
                    ))}
                  </div>
                )}
              ...
              {form.items.length ? (
                <>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 10px', background: '#f0fdf4', borderRadius: 8, marginBottom: 8, fontSize: t.fontSize.caption }}>
                    <span style={{ color: t.color.textSecondary }}>共 <strong>{form.items.length}</strong> 項商品</span>
                    <span style={{ color: '#10b981', fontWeight: t.fontWeight.bold, ...S.mono }}>{fmtP(subtotal)}</span>
                  </div>
                    ...
                    {form.items.map((item, index) => (
                      <div key={`${item.item_number_snapshot}-${index}`} style={{ background: '#f9fafb', border: '1px solid #f0f2f5', borderRadius: 8, padding: '7px 10px', display: 'flex', alignItems: 'center', gap: 8 }}>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: t.fontSize.tiny, color: t.color.link, fontWeight: t.fontWeight.bold, ...S.mono, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.item_number_snapshot || '-'}</div>
                          <div style={{ fontSize: t.fontSize.caption, color: t.color.textPrimary, marginTop: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.description_snapshot || '-'}</div>
                        </div>
                        <input type="number" min="1" value={item.qty} onChange={(e) => updateItem(index, 'qty', Number(e.target.value || 1))} style={{ ...S.input, width: 52, textAlign: 'center', ...S.mono, padding: '4px 6px', fontSize: t.fontSize.caption, flexShrink: 0 }} />
                        <input type="number" min="0" value={item.unit_price} onChange={(e) => updateItem(index, 'unit_price', Number(e.target.value || 0))} style={{ ...S.input, width: 82, textAlign: 'right', ...S.mono, padding: '4px 6px', fontSize: t.fontSize.caption, flexShrink: 0 }} />
                        <div style={{ fontSize: t.fontSize.caption, color: '#10b981', fontWeight: t.fontWeight.bold, ...S.mono, whiteSpace: 'nowrap', flexShrink: 0, minWidth: 72, textAlign: 'right' }}>{fmtP(Number(item.qty || 0) * Number(item.unit_price || 0))}</div>
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            <div style={{ ...S.card, marginBottom: 10 }}>
              ...
              <div style={{ display: 'grid', gap: 10 }}>
                <div><label style={{ ...S.label, marginBottom: 6 }}>報價日期</label><input type="date" value={form.quote_date} onChange={(e) => setForm((current) => ({ ...current, quote_date: e.target.value }))} style={{ ...S.input, ...S.mobile.input }} /></div>
                <div><label style={{ ...S.label, marginBottom: 6 }}>有效期限</label><input type="date" value={form.valid_until} onChange={(e) => setForm((current) => ({ ...current, valid_until: e.target.value }))} style={{ ...S.input, ...S.mobile.input }} /></div>
                <div>
            ...
            <div style={{ ...S.card, marginBottom: 10 }}>
              ...
              <div style={{ display: 'grid', gap: 10 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <label htmlFor="tax_excluded" style={{ fontSize: t.fontSize.caption, color: t.color.textPrimary, fontWeight: t.fontWeight.semibold, cursor: 'pointer' }}>稅額外加 (5%)</label>
                </div>
                <div style={{ ...S.panelMuted, display: 'grid', gap: 8 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: t.fontSize.caption }}><span>小計</span><strong style={S.mono}>{fmtP(subtotal)}</strong></div>
                  {form.tax_excluded && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: t.fontSize.caption }}><span>稅額 (5%)</span><strong style={S.mono}>{fmtP(taxAmount)}</strong></div>}
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: t.fontSize.h2, color: '#10b981', fontWeight: t.fontWeight.bold }}><span>總額</span><strong style={S.mono}>{fmtP(totalAmount)}</strong></div>
                </div>
                <button onClick={submit} disabled={saving || !tableReady} style={{ ...S.btnPrimary, ...S.mobile.btnPrimary, width: '100%', opacity: saving || !tableReady ? 0.7 : 1 }}>{saving ? '建立中...' : '建立報價單'}</button>
              </div>
            </div>
          </div>
        </div>
