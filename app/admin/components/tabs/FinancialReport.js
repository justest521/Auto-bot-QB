  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 4 }}>
        ...
        <span style={{ fontSize: 11, color: t.color.textMuted, ...S.mono }}>{subLabel || `${pct.toFixed(0)}%`}</span>
      </div>
      ...
      {loading ? <Loading /> : (
        <>
          ...
          <div style={{ ...S.card, background: d.net_cash >= 0 ? t.color.infoBg : t.color.errorBg, borderColor: d.net_cash >= 0 ? '#b8d4f5' : '#f5b8b8', marginBottom: 16, textAlign: 'center', padding: isMobile ? 20 : 28 }}>
            <div style={{ fontSize: 10, fontWeight: t.fontWeight.bold, color: t.color.textMuted, letterSpacing: 1, marginBottom: 8 }}>NET CASH FLOW</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 16, marginBottom: 16 }}>
            {/* Receivable side */}
            <div style={{ ...S.card }}>
              <div style={{ fontSize: 10, fontWeight: t.fontWeight.bold, color: t.color.brand, letterSpacing: 1, marginBottom: 14 }}>RECEIVABLE</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
                <div>
                  <div style={{ fontSize: 10, color: t.color.textMuted, marginBottom: 4 }}>銷貨收入</div>
                  <div style={{ fontSize: 18, fontWeight: t.fontWeight.bold, ...S.mono }}>{fmtP(d.revenue)}</div>
                </div>
                <div>
                  <div style={{ fontSize: 10, color: t.color.textMuted, marginBottom: 4 }}>已收款</div>
                  <div style={{ fontSize: 18, fontWeight: t.fontWeight.bold, color: t.color.brand, ...S.mono }}>{fmtP(d.received)}</div>
                </div>
              </div>
              ...
              <div style={{ ...S.card, background: d.receivable > 0 ? t.color.warningBg : t.color.successBg, border: `1px solid ${d.receivable > 0 ? '#fde68a' : '#a7f3d0'}`, padding: '12px 14px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: 16, fontWeight: t.fontWeight.bold, color: d.receivable > 0 ? t.color.warning : t.color.brand, ...S.mono }}>{fmtP(d.receivable)}</span>
                </div>
              </div>
            </div>
            ...
            {/* Payable side */}
            <div style={{ ...S.card }}>
              <div style={{ fontSize: 10, fontWeight: t.fontWeight.bold, color: t.color.link, letterSpacing: 1, marginBottom: 14 }}>PAYABLE</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
                <div>
                  <div style={{ fontSize: 10, color: t.color.textMuted, marginBottom: 4 }}>進貨支出</div>
                  <div style={{ fontSize: 18, fontWeight: t.fontWeight.bold, ...S.mono }}>{fmtP(d.purchase)}</div>
                </div>
                <div>
                  <div style={{ fontSize: 10, color: t.color.textMuted, marginBottom: 4 }}>已付款</div>
                  <div style={{ fontSize: 18, fontWeight: t.fontWeight.bold, color: t.color.link, ...S.mono }}>{fmtP(d.paid)}</div>
                </div>
              </div>
              ...
              <div style={{ ...S.card, background: d.payable > 0 ? t.color.errorBg : t.color.successBg, border: `1px solid ${d.payable > 0 ? '#fecaca' : '#a7f3d0'}`, padding: '12px 14px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: 16, fontWeight: t.fontWeight.bold, color: d.payable > 0 ? t.color.error : t.color.brand, ...S.mono }}>{fmtP(d.payable)}</span>
                </div>
              </div>
            </div>
          </div>
