...
export default function Tickets() {
  ...
  return (
    <div>
      ...
      {/* Ticket detail panel */}
      {detail && (
        <div style={{ ...S.card, padding: '10px 16px', marginBottom: 10, borderLeft: `3px solid ${STATUS_MAP[detail.status]?.color || '#3b82f6'}` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            <div>
              <span style={{ fontSize: 16, fontWeight: 700, color: '#111827' }}>{detail.title}</span>
              <span style={{ marginLeft: 6, ...S.tag(''), background: STATUS_MAP[detail.status]?.color || '#3b82f6', color: '#fff', fontSize: 10 }}>{STATUS_MAP[detail.status]?.label || detail.status}</span>
          ...
          {detail.description && <div style={{ fontSize: 13, color: '#374151', marginBottom: 10, padding: '10px', background: '#f3f4f6', borderRadius: 6 }}>{detail.description}</div>}
      ...
      {loading ? <Loading /> : (data.rows || []).length === 0 ? <EmptyState text="沒有工單" /> : (data.rows || []).map(t => {
        ...
        return (
          <div key={t.id} style={{ ...S.card, padding: '10px 16px', marginBottom: 10, cursor: 'pointer', borderLeft: `3px solid ${st.color}` }} onClick={() => openDetail(t)}>
      ...
      {createOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ ...S.card, width: 480, maxWidth: '90vw' }}>
            ...
            <div style={{ marginBottom: 10 }}><label style={S.label}>描述</label><textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} style={{ ...S.input, minHeight: 80 }} /></div>
