    ...
    <div style={{ maxWidth: isMobile ? '100%' : 560, width: '100%' }}>
      ...
      <div style={S.card}>
        ...
        <div style={{ display: 'flex', gap: 16, marginBottom: 20, flexDirection: isMobile ? 'column' : 'row' }}>
          <label style={{ color: t.color.textSecondary, fontSize: t.fontSize.caption, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, minHeight: isMobile ? 44 : 'auto', padding: isMobile ? '8px 0' : 0 }}><input type="checkbox" checked={rules.show_retail_price} onChange={e => setRules({ ...rules, show_retail_price: e.target.checked })} style={{ accentColor: t.color.link, width: 18, height: 18, cursor: 'pointer' }} />顯示建議售價</label>
          <label style={{ color: t.color.textSecondary, fontSize: t.fontSize.caption, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, minHeight: isMobile ? 44 : 'auto', padding: isMobile ? '8px 0' : 0 }}><input type="checkbox" checked={rules.show_promo_hint} onChange={e => setRules({ ...rules, show_promo_hint: e.target.checked })} style={{ accentColor: t.color.link, width: 18, height: 18, cursor: 'pointer' }} />顯示優惠提示</label>
        </div>
        <button onClick={save} style={{ ...S.btnPrimary, ...(isMobile ? S.mobile.btnPrimary : {}), background: saved ? t.color.success : 'linear-gradient(180deg, #2d8cff 0%, #3b82f6 100%)', transition: 'background 0.3s', width: '100%', padding: isMobile ? '12px 16px' : '11px 0', fontSize: t.fontSize.h3, minHeight: isMobile ? 44 : 'auto' }}>{saved ? '✓ SAVED' : '儲存設定'}</button>
      </div>
    </div>
  );
}
