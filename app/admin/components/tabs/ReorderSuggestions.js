      ...
      {loading ? <Loading /> : suggestions.length === 0 ? <EmptyState text="目前沒有補貨建議，點擊「掃描庫存」檢查" /> : (
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead><tr style={{ background: '#f3f4f6' }}>
            ...
            <th style={{ padding: '10px 12px', textAlign: 'right', color: '#6b7280', fontWeight: 600 }}>現有</th>
            <th style={{ padding: '10px 12px', textAlign: 'right', color: '#6b7280', fontWeight: 600 }}>安全</th>
            <th style={{ padding: '10px 12px', textAlign: 'right', color: '#6b7280', fontWeight: 600 }}>建議採購</th>
          ...
          <tbody>{suggestions.map(s => (
            <tr key={s.id} style={{ borderTop: '1px solid #f0f0f0' }}>
              ...
              <td style={{ padding: '10px 12px', fontWeight: 600, color: '#3b82f6', ...S.mono }}>{s.item_number}</td>
