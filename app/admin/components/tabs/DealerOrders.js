            ...
            <div key={row.id} style={{ display: isMobile ? 'block' : 'grid', gridTemplateColumns: !isMobile ? gridTemplate : undefined, gap: 0, padding: isMobile ? '10px 12px' : 0, borderBottom: `1px solid ${t.color.borderLight}`, alignItems: isMobile ? 'flex-start' : 'center', background: selected.includes(row.id) ? '#dbeafe' : idx % 2 === 0 ? t.color.bgCard : '#fafbfd', cursor: 'pointer', transition: 'background 0.15s', minHeight: isMobile ? 44 : 'auto' }} onClick={() => !isMobile && setSelectedOrder(row)} onMouseEnter={(e) => !isMobile && (e.currentTarget.style.background = '#f0f7ff')} onMouseLeave={(e) => !isMobile && (e.currentTarget.style.background = selected.includes(row.id) ? '#dbeafe' : idx % 2 === 0 ? t.color.bgCard : '#fafbfd')}>
              {isMobile ? (
                ...
                <>
                  ...
                  <div style={{ padding: '8px 10px', borderRight: `1px solid ${t.color.border}`, display: 'flex', alignItems: 'center', minWidth: 0, overflow: 'hidden', textAlign: 'left' }}><div><div style={{ fontSize: t.fontSize.body, fontWeight: t.fontWeight.semibold, color: t.color.textPrimary }}>{row.dealer?.display_name || '-'}</div><div style={{ fontSize: t.fontSize.tiny, color: t.color.textSecondary }}>{row.dealer?.company_name || ''} {row.dealer?.role ? `(${row.dealer.role === 'dealer' ? '經銷' : row.dealer.role === 'sales' ? '業務' : '技師'})` : ''}</div></div></div>
                  ...
                  <div style={{ padding: '8px 10px', borderRight: 'none', display: 'flex', alignItems: 'center', minWidth: 0, overflow: 'hidden', fontSize: t.fontSize.body, color: t.color.success, textAlign: 'right', fontWeight: t.fontWeight.bold, ...S.mono }}>{fmtP(row.total_amount)}</div>
                </>
              )}
            </div>
          ))}
