      ...
      <PageLead eyebrow="Stock Transfer" title="調撥單" description="記錄商品在不同儲位 / 據點之間的調撥異動。"
        action={
          <div style={{ display: 'flex', gap: t.spacing.sm, alignItems: 'center', flexWrap: 'wrap', flexDirection: isMobile ? 'column' : 'row', width: isMobile ? '100%' : 'auto' }}>
            ...
            <button onClick={() => setCreateOpen(true)} style={{ ...S.btnPrimary, ...(isMobile ? S.mobile.btnPrimary : {}) }}>+ 新增調撥</button>
          </div>
        } />
        ...
        <div key={r.id} style={{ ...S.card, ...p.listCard }} onClick={() => setDetailRow(r)}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 6 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              <span style={p.docNo}>{r.transfer_no}</span>
              <span style={{ fontSize: t.fontSize.body, color: t.color.textSecondary }}>{fmtDate(r.transfer_date)}</span>
              {statusTag(r.status)}
            </div>
          ...
          <div style={{ ...(isMobile ? S.mobileModalBody : p.modalBody('md')) }} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: t.spacing.md }}>
              <div>
                ...
                <div style={p.modalTitle(isMobile)}>{detailRow.transfer_no}</div>
              </div>
            ...
            <div style={{ ...S.card, marginBottom: 10 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: t.spacing.sm }}>
                <div><span style={S.label}>日期</span><div style={p.fieldValue}>{fmtDate(detailRow.transfer_date)}</div></div>
                <div><span style={S.label}>狀態</span><div>{statusTag(detailRow.status)}</div></div>
                <div><span style={S.label}>來源</span><div style={p.fieldValue}>{detailRow.from_location}</div></div>
                <div><span style={S.label}>目的地</span><div style={p.fieldValue}>{detailRow.to_location}</div></div>
              </div>
            ...
            {detailRow.items?.length > 0 && (
              <div style={S.card}>
                <div style={p.sectionTitle}>調撥明細</div>
                {detailRow.items.map((it, idx) => (
                  <div key={idx} style={p.detailRow(idx === detailRow.items.length - 1)}>
                    ...
                    <div style={{ ...S.mono, fontWeight: t.fontWeight.semibold }}>{it.quantity}</div>
                  </div>
                ))}
              </div>
            )}
          ...
          <div style={{ ...(isMobile ? S.mobileModalBody : p.modalBody('lg')) }} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: isMobile ? 'flex-start' : 'center', marginBottom: t.spacing.md, gap: isMobile ? 10 : 0 }}>
              <div>
                ...
                <div style={p.modalTitle(isMobile)}>新增調撥單</div>
              </div>
            ...
            <div style={S.card}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                <span style={p.sectionTitle}>調撥明細</span>
              </div>
              <div style={{ maxHeight: isMobile ? 200 : 280, overflowY: 'auto', overflowX: 'auto', WebkitOverflowScrolling: 'touch', display: 'grid', gap: 5, paddingRight: 4 }}>
                {items.map((it, idx) => (
                  <div key={idx} style={p.inlineItemRow(isMobile)}>
                    ...
                    <input type="number" value={it.quantity} onChange={(e) => setItems(prev => { const n = [...prev]; n[idx] = { ...n[idx], quantity: e.target.value }; return n; })} style={{ ...S.input, ...p.inlineInput(isMobile), width: isMobile ? '100%' : 70, textAlign: 'center', flexShrink: 0, ...S.mono }} placeholder="數量" />
              ...
              <button onClick={() => setItems(prev => [...prev, { item_number: '', description: '', quantity: 1 }])} style={{ ...S.btnGhost, fontSize: isMobile ? t.fontSize.body : t.fontSize.caption, marginTop: t.spacing.sm, width: '100%', minHeight: isMobile ? 44 : 'auto' }}>+ 新增品項</button>
            </div>
