...
function ReturnDetailView({ ret: initRet, onBack, onRefresh }) {
  ...
  return (
    <div style={{ animation: 'fadeIn 0.25s ease', padding: '0 12px' }}>
      ...
      {msg && <div style={{ ...cardStyle, background: msg.includes('失敗') || msg.includes('拒絕') ? '#fff1f2' : '#edfdf3', borderColor: msg.includes('失敗') || msg.includes('拒絕') ? '#fecdd3' : '#bbf7d0', color: msg.includes('失敗') || msg.includes('拒絕') ? '#b42318' : '#15803d', marginBottom: 16, padding: '12px 20px', fontSize: 14 }}>{msg}</div>}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 20, alignItems: 'start' }}>
          ...
          <div>
            <div style={{ ...cardStyle, padding: 0, overflow: 'hidden', marginBottom: 16 }}>
              ...
              {items.length > 0 ? (
                <div>
                  <div style={{ display: 'grid', gridTemplateColumns: '100px minmax(0,1fr) 75px 90px 100px', gap: 8, padding: '10px 24px', background: '#f8f9fb', fontSize: 12, fontWeight: 700, color: '#b0b8c4', letterSpacing: 0.5, textTransform: 'uppercase' }}>
                  ...
                  {items.map((item, i) => (
                    <div key={item.id || i} style={{ display: 'grid', gridTemplateColumns: '100px minmax(0,1fr) 75px 90px 100px', gap: 8, padding: '16px 24px', borderTop: '1px solid #f3f5f7', background: '#fff', transition: 'background 0.1s' }} onMouseEnter={e => e.currentTarget.style.background = '#f8fafc'} onMouseLeave={e => e.currentTarget.style.background = '#fff'}>
                  ...
                  {/* Totals */}
                  <div style={{ padding: '20px 24px', background: 'linear-gradient(135deg, #fef2f2, #fff1f2)', borderTop: '2px solid #fecdd3' }}>
                    <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'flex-end', gap: 24 }}>
                      ...
                      <div style={{ borderLeft: '2px solid #fca5a5', paddingLeft: 20, textAlign: 'right' }}>
    ...
    <div>
      ...
      {loading ? <Loading /> : data.returns.length === 0 ? <EmptyState text="目前沒有退貨記錄" /> : (
        <div style={{ ...S.card, borderRadius: 14, padding: 0, overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '40px 160px minmax(0,1fr) 110px 100px 100px 140px', gap: 8, padding: '10px 24px', background: '#f8f9fb', fontSize: 12, fontWeight: 700, color: '#b0b8c4', letterSpacing: 0.5, textTransform: 'uppercase' }}>
          ...
          {data.returns.map((r, idx) => (
            <div key={r.id} onClick={() => setSelectedReturn(r)}
              style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '40px 160px minmax(0,1fr) 110px 100px 100px 140px', gap: 8, padding: '16px 24px', borderTop: '1px solid #f3f5f7', background: '#fff', cursor: 'pointer', transition: 'background 0.1s' }}
              ...
              <div style={{ fontSize: 13, color: '#374151', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.reason || '-'}</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: '#ef4444', textAlign: 'right', ...S.mono }}>{fmtP(r.refund_amount)}</div>
              <div style={{ fontSize: 13, color: '#6b7280', ...S.mono }}>{fmtDate(r.return_date || r.created_at)}</div>
              <div>
          ...
          <div style={{ width: 'min(620px, 100%)', maxHeight: '90vh', overflowY: 'auto', background: '#f6f9fc', borderRadius: 18, padding: '24px 22px 28px', boxShadow: '0 24px 70px rgba(8,12,20,0.3)' }} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
              <div>
                ...
                <div style={{ fontSize: 22, fontWeight: 700, color: '#111827' }}>建立退貨單</div>
              </div>
