...
export default function UserManagement() {
  ...
  if (loading) return <div style={{ padding: 40, textAlign: 'center', color: '#6b7280' }}>載入中...</div>;
  ...
  return (
    <div>
      ...
      {msg && <div style={{ padding: '10px 16px', borderRadius: 10, marginBottom: 16, fontSize: 13, background: msg.includes('失敗') || msg.includes('錯誤') ? '#fee2e2' : '#dcfce7', color: msg.includes('失敗') || msg.includes('錯誤') ? '#ef4444' : '#16a34a' }}>{msg}</div>}
              ...
              <thead>
                <tr style={{ background: '#f9fafb' }}>
                  {['帳號', '名稱', 'Email', '角色', '狀態', '最後登入', '操作'].map(h => (
                    <th key={h} style={{ padding: '12px 14px', textAlign: 'left', fontWeight: 600, color: '#374151', borderBottom: '1px solid #e5e7eb', fontSize: 12 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {users.map(u => (
                  <tr key={u.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                    <td style={{ padding: '12px 14px', fontWeight: 600, color: '#111827', ...S.mono }}>{u.username}</td>
                    <td style={{ padding: '12px 14px', color: '#374151' }}>{u.display_name}</td>
                    <td style={{ padding: '12px 14px', color: '#6b7280', ...S.mono, fontSize: 12 }}>{u.email}</td>
                    <td style={{ padding: '12px 14px' }}><span style={{ padding: '3px 10px', borderRadius: 6, fontSize: 11, fontWeight: 600, background: '#dcfce7', color: '#16a34a' }}>{u.role?.label || '-'}</span></td>
                    <td style={{ padding: '12px 14px' }}><span style={{ padding: '3px 10px', borderRadius: 6, fontSize: 11, fontWeight: 600, background: u.status === 'active' ? '#dcfce7' : '#fee2e2', color: u.status === 'active' ? '#16a34a' : '#ef4444' }}>{u.status === 'active' ? '啟用' : '停用'}</span></td>
                    <td style={{ padding: '12px 14px', color: '#6b7280', fontSize: 12, ...S.mono }}>{u.last_login_at ? new Date(u.last_login_at).toLocaleString('zh-TW') : '從未'}</td>
                    <td style={{ padding: '12px 14px', display: 'flex', gap: 6 }}>
                      ...
                      {u.status === 'active' && <button onClick={() => handleDisable(u.id, u.username)} style={{ ...S.btnGhost, padding: '5px 12px', fontSize: 11, cursor: 'pointer', color: '#ef4444', borderColor: '#fecaca' }}>停用</button>}
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <div style={{ ...S.card, padding: 0, overflow: 'hidden', order: isMobile && editingRole ? 2 : 0 }}>
              {roles.map(r => (
                <div
                  ...
                  style={{ padding: '14px 16px', cursor: 'pointer', borderBottom: '1px solid #f3f4f6', background: editingRole?.id === r.id ? '#dcfce7' : 'transparent', transition: 'background 0.15s' }}
                >
                  <div style={{ fontSize: 14, fontWeight: 600, color: editingRole?.id === r.id ? '#16a34a' : '#111827' }}>{r.label}</div>
                  <div style={{ fontSize: 11, color: '#6b7280', marginTop: 2 }}>{r.code}{r.is_system ? ' (系統)' : ''}</div>
                </div>
              ))}
            </div>
            ...
            <div style={{ ...S.card, padding: isMobile ? 16 : 24, order: isMobile && editingRole ? 1 : 0 }}>
              {!editingRole ? (
                ...
                <div>
                  ...
                  {editingRole.code === 'admin' ? (
                    <div style={{ padding: 20, background: '#f3f4f6', borderRadius: 12, textAlign: 'center', color: '#6b7280', fontSize: 13 }}>系統管理員擁有所有權限，無法修改</div>
                  ) : (
                        ...
                        return (
                          <div key={mod} style={{ border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden' }}>
                            <div
                              ...
                              style={{ padding: '10px 16px', background: '#f9fafb', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, borderBottom: '1px solid #e5e7eb' }}
                            >
                              <span style={{ width: 18, height: 18, borderRadius: 4, border: '2px solid ' + (allChecked ? '#16a34a' : '#d1d5db'), background: allChecked ? '#16a34a' : '#fff', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#fff', flexShrink: 0 }}>{allChecked ? '\u2713' : ''}</span>
                            ...
                            <div style={{ padding: '8px 16px', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fill, minmax(180px, 1fr))', gap: 4 }}>
                              {perms.map(p => {
                                ...
                                return (
                                  <label key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 4px', cursor: 'pointer', borderRadius: 6, fontSize: 13, color: checked ? '#111827' : '#6b7280' }}>
