...
export default function ProductSearch() {
  ...
  return (
    <div>
      ...
      {showAddForm && (
        <div style={{ ...S.card, borderColor: '#93c5fd', marginBottom: 10 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            <div style={{ fontSize: t.fontSize.h3, fontWeight: t.fontWeight.bold, color: t.color.textPrimary }}>新增商品</div>
            <button onClick={() => setShowAddForm(false)} style={{ ...S.btnGhost, padding: '4px 12px', fontSize: t.fontSize.caption }}>取消</button>
          </div>
      ...
      <div style={{ fontSize: t.fontSize.tiny, color: t.color.textMuted, marginBottom: 12, ...S.mono, display: 'flex', alignItems: 'center', gap: 8 }}>
        主檔共 {fmt(total)} 筆 {totalPages > 1 && `· P${page}/${totalPages}`}
        {loading && <span style={{ color: t.color.link, fontSize: t.fontSize.tiny }}>搜尋中...</span>}
      </div>
      {!loading && products.length === 0 ? <EmptyState text={search ? '找不到符合的產品' : '輸入料號或關鍵字開始搜尋'} /> : (
        <div ref={tableRef} style={{ opacity: loading ? 0.5 : 1, transition: 'opacity 0.15s', pointerEvents: loading ? 'none' : 'auto', overflowX: isMobile ? 'auto' : 'visible' }}>
          {!isMobile && <div style={{ display: 'flex', padding: '8px 16px', fontSize: t.fontSize.caption, fontWeight: t.fontWeight.semibold, color: t.color.textMuted, ...S.mono, borderBottom: `1px solid ${t.color.borderLight}`, marginBottom: 4 }}>
          ...
          {products.map(p => (
            <div key={p.item_number}>
              <div onClick={() => setExpanded(expanded === p.item_number ? null : p.item_number)} style={{ ...S.card, cursor: 'pointer', padding: isMobile ? '8px 12px' : '10px 16px', marginBottom: 10, display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 8 : 0, borderColor: expanded === p.item_number ? '#93c5fd' : t.color.border, minHeight: isMobile ? 44 : 'auto' }}>
                <div style={{ width: isMobile ? '100%' : 150, fontWeight: t.fontWeight.bold, color: t.color.link, fontSize: t.fontSize.h3, ...S.mono, flexShrink: 0 }}>{p.item_number}</div>
                <div style={{ width: 36, height: 28, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {p.image_url ? <a href={`https://shop.snapon.com/product/${p.item_number}`} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} style={{ width: 28, height: 28, borderRadius: 4, overflow: 'hidden', background: t.color.bgMuted, border: `1px solid ${t.color.border}`, display: 'block' }}><img src={p.image_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'contain' }} onError={(e) => { e.target.style.display = 'none'; }} /></a> : null}
                </div>
                <div style={{ flex: 1, width: isMobile ? '100%' : 'auto', fontSize: t.fontSize.body, color: t.color.textSecondary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: isMobile ? 'normal' : 'nowrap', minWidth: 0 }}>{p.description}</div>
                <div style={{ width: isMobile ? '100%' : 120, textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: isMobile ? 'flex-start' : 'center', whiteSpace: 'nowrap', flexShrink: 0 }}>{p.category && <span style={{ ...S.tag(''), fontSize: t.fontSize.tiny, whiteSpace: 'nowrap', maxWidth: 110, overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.category}</span>}</div>
                ...
                <div style={{ width: isMobile ? '100%' : 100, textAlign: isMobile ? 'left' : 'right', fontSize: t.fontSize.h3, color: t.color.textPrimary, ...S.mono, flexShrink: 0 }}>{isMobile ? `牌價 ${fmtP(p.tw_retail_price)}` : fmtP(p.tw_retail_price)}</div>
                <div style={{ width: isMobile ? '100%' : 100, textAlign: isMobile ? 'left' : 'right', fontSize: t.fontSize.h3, color: p.tw_reseller_price ? t.color.warning : t.color.textDisabled, fontWeight: t.fontWeight.semibold, ...S.mono, flexShrink: 0 }}>{p.tw_reseller_price ? (isMobile ? `進貨 ${fmtP(p.tw_reseller_price)}` : fmtP(p.tw_reseller_price)) : '-'}</div>
                <div style={{ width: isMobile ? '100%' : 80, display: 'flex', alignItems: 'center', justifyContent: isMobile ? 'flex-start' : 'center', flexShrink: 0 }}>
                  <button
                    ...
                    style={{ ...S.btnGhost, padding: isMobile ? '8px 12px' : '6px 10px', fontSize: t.fontSize.caption, minHeight: isMobile ? 40 : 'auto' }}
                  >
                    編輯
                  </button>
                </div>
              ...
              {expanded === p.item_number && (
                <div style={{ background: t.color.infoBg, border: `1px solid ${t.color.borderLight}`, borderRadius: 10, padding: isMobile ? '8px 12px' : '10px 16px', marginBottom: 10, marginTop: -2, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  {p.image_url && <a href={`https://shop.snapon.com/product/${p.item_number}`} target="_blank" rel="noopener noreferrer" style={{ width: 80, height: 80, borderRadius: 8, overflow: 'hidden', flexShrink: 0, background: t.color.bgCard, border: `1px solid ${t.color.border}`, display: 'block', cursor: 'pointer' }}><img src={p.image_url} alt={p.item_number} style={{ width: '100%', height: '100%', objectFit: 'contain', padding: 4 }} onError={(e) => { e.target.parentElement.style.display = 'none'; }} /></a>}
                  ...
                  <div><div style={S.label}>US PRICE</div><div style={{ color: t.color.textSecondary, fontSize: t.fontSize.h3, ...S.mono }}>{p.us_price ? `$${Number(p.us_price).toFixed(2)}` : '-'}</div></div>
                  <div><div style={S.label}>牌價</div><div style={{ color: t.color.textPrimary, fontSize: t.fontSize.h3, ...S.mono }}>{fmtP(p.tw_retail_price)}</div></div>
                  <div><div style={S.label}>進貨價</div><div style={{ color: t.color.warning, fontSize: t.fontSize.h3, fontWeight: t.fontWeight.bold, ...S.mono }}>{fmtP(p.tw_reseller_price)}</div></div>
                  <div><div style={S.label}>毛利率</div><div style={{ color: p.tw_retail_price > 0 && p.tw_reseller_price > 0 ? t.color.success : t.color.textDisabled, fontSize: t.fontSize.h3, fontWeight: t.fontWeight.bold, ...S.mono }}>{p.tw_retail_price > 0 && p.tw_reseller_price > 0 ? `${Math.round((1 - p.tw_reseller_price / p.tw_retail_price) * 100)}%` : '-'}</div></div>
                  <div><div style={S.label}>狀態</div><div style={{ color: t.color.textSecondary, fontSize: t.fontSize.h3 }}>{STATUS_LABEL[p.product_status] || p.product_status || '-'}</div></div>
                  <div><div style={S.label}>重量</div><div style={{ color: t.color.textSecondary, fontSize: t.fontSize.h3, ...S.mono }}>{p.weight_kg ? `${p.weight_kg} kg` : '-'}</div></div>
                  <div><div style={S.label}>產地</div><div style={{ color: t.color.textSecondary, fontSize: t.fontSize.h3, ...S.mono }}>{p.origin_country || '-'}</div></div>
                  <div><div style={S.label}>替代型號</div><div style={{ color: p.replacement_model ? t.color.link : t.color.textMuted, fontSize: t.fontSize.h3, ...S.mono }}>{p.replacement_model || '-'}</div></div>
                  <div><div style={S.label}>美國庫存</div><div style={{ fontSize: t.fontSize.body }}>{(() => {
                    const ss = snaponStock[p.item_number];
                    if (!ss || ss.loading) return <span style={{ color: t.color.textDisabled, fontSize: t.fontSize.caption }}>查詢中...</span>;
                    const colors = { in_stock: t.color.success, backordered: t.color.warning, discontinued: t.color.error, out_of_stock: t.color.error, unknown: t.color.textDisabled, error: t.color.textDisabled };
                    return <span style={{ color: colors[ss.stock_status] || t.color.textDisabled, fontWeight: t.fontWeight.semibold }}>{ss.stock_message || '-'}</span>;
                  })()}</div></div>
                  </div>
                </div>
              )}
