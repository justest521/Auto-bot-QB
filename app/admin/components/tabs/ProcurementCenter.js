    ...
    <div>
      ...
      {loading ? <Loading /> : data.rows?.length === 0 ? <EmptyState text="目前沒有採購中的品項" /> : (
        <div style={{ ...S.card, padding: 0, overflow: 'hidden' }}>
          {/* Header */}
          <div style={{ display: 'grid', gridTemplateColumns: '130px minmax(0,1fr) 70px 70px 70px 80px 80px 70px 90px', gap: 4, padding: '8px 12px', borderBottom: '2px solid #e6edf5', color: '#6b7280', fontSize: 12, fontWeight: 600, alignItems: 'center' }}>
            <div>料號</div>
            <div>品名</div>
            ...
            return (
              <div key={row.item_number}>
                ...
                <div
                  ...
                  style={{ display: 'grid', gridTemplateColumns: '130px minmax(0,1fr) 70px 70px 70px 80px 80px 70px 90px', gap: 4, padding: '10px 12px', borderTop: idx > 0 ? '1px solid #f0f2f5' : 'none', alignItems: 'center', cursor: 'pointer', background: isExpanded ? '#f8fafc' : idx % 2 === 0 ? '#fff' : '#fafbfd', transition: 'background 0.15s' }}
