            ...
            <div style={{ ...cardStyle, padding: 0, overflow: isMobile ? 'hidden' : 'visible', marginBottom: 10, opacity: isLocked ? 0.5 : 1, pointerEvents: isLocked ? 'none' : 'auto' }}>
              ...
              {items.length > 0 ? (
                <div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '36px minmax(100px,1fr) 95px 80px 100px 100px 120px 80px', gap: 0, background: '#f8f9fb', fontSize: t.fontSize.caption, fontWeight: t.fontWeight.bold, color: t.color.textDisabled, letterSpacing: 0.5, textTransform: 'uppercase', borderBottom: '2px solid #dde0e7' }}>
                    <div style={{ padding: '8px 10px', borderRight: '1px solid #e5e7eb' }}></div><div style={{ padding: '8px 10px', borderRight: '1px solid #e5e7eb' }}>料號</div><div style={{ padding: '8px 10px', borderRight: '1px solid #e5e7eb', textAlign: 'right' }}>單價</div>
                    <div style={{ padding: '8px 10px', borderRight: '1px solid #e5e7eb', textAlign: 'center' }}>數量</div>
                    <div style={{ padding: '8px 10px', borderRight: '1px solid #e5e7eb', textAlign: 'center' }}>庫存</div><div style={{ padding: '8px 10px', borderRight: '1px solid #e5e7eb', textAlign: 'right' }}>小計</div><div style={{ padding: '8px 10px', borderRight: '1px solid #e5e7eb' }}>備註</div><div style={{ padding: '8px 10px' }}></div>
                  </div>
                  )}
                  ...
                  {visibleItems.map((item) => {
                    ...
                    return (
                      <div key={item.id}>
                      <div onClick={() => !isEditing && toggleItemSelect(item.id)} style={{ display: 'grid', gridTemplateColumns: '36px minmax(100px,1fr) 95px 80px 100px 100px 120px 80px', gap: 0, borderTop: '1px solid #e5e7eb', alignItems: 'center', fontSize: t.fontSize.body, cursor: isEditing ? 'default' : 'pointer', background: rowBg, opacity: hasPO && !isEditing ? 0.7 : 1, transition: 'background 0.1s' }} onMouseEnter={e => !isChecked && !isEditing && (e.currentTarget.style.background= hasPO ? '#fafafa' : '#f8fafc')} onMouseLeave={e => !isChecked && !isEditing && (e.currentTarget.style.background= isEditing ? '#fffbeb' : isChecked ? '#f0f7ff' : hasPO ? '#fafafa' : '#fff')}>
                        <div style={{ padding: '8px 10px', borderRight: '1px solid #e5e7eb', textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 40 }}>
                          {fullySold ? (
                        ...
                        <div onClick={(e) => !cannotEdit && !isEditing && startEditItem(item, e)} style={{ padding: '8px 10px', borderRight: '1px solid #e5e7eb', color: t.color.textMuted, textAlign: 'right', ...S.mono, fontSize: t.fontSize.h3, cursor: cannotEdit || isEditing ? 'default' : 'pointer', transition: 'background 0.1s', whiteSpace: 'nowrap' }} onMouseEnter={(e) => !cannotEdit && !isEditing && (e.currentTarget.style.background = '#f3f4f6')} onMouseLeave={(e) => !cannotEdit && !isEditing && (e.currentTarget.style.background = 'transparent')}>
                          {isEditing ? (
                        ...
                        {/* ── 數量 cell: 訂 / 已出 / 待出 ── */}
                        <div onClick={(e) => !cannotEdit && !isEditing && startEditItem(item, e)} style={{ padding: '8px 10px', borderRight: '1px solid #e5e7eb', textAlign: 'center', cursor: cannotEdit || isEditing ? 'default' : 'pointer', whiteSpace: 'nowrap' }} onMouseEnter={(e) => !cannotEdit && !isEditing && (e.currentTarget.style.background = '#f3f4f6')} onMouseLeave={(e) => !cannotEdit && !isEditing && (e.currentTarget.style.background = 'transparent')}>
                          {isEditing ? (
                            ...
                            <span style={{ ...S.mono, fontSize: t.fontSize.h3 }}>
                              <span style={{ fontWeight: t.fontWeight.bold }}>{item.qty}</span>
                              {Number(item.sold_qty) > 0 && <span style={{ fontSize: t.fontSize.tiny, color: '#15803d' }}> ↑{item.sold_qty}</span>}
                              {Number(item.remaining_qty) > 0 && <span style={{ fontSize: t.fontSize.tiny, color: '#b45309' }}> ↓{item.remaining_qty}</span>}
                            </span>
                          )}
                        </div>
                        {/* ── 庫存 cell + PO 到貨狀態 ── */}
                        <div style={{ padding: '8px 10px', borderRight: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3, whiteSpace: 'nowrap' }}>
                          ...
                          {item.po_info && (
                            <span style={{ fontSize: t.fontSize.tiny, padding: '1px 4px', borderRadius: 99, fontWeight: t.fontWeight.semibold, whiteSpace: 'nowrap',
                              ...(['received','completed'].includes(item.po_info.status)
                                ? { background: '#dbeafe', color: '#1d4ed8' }
                                : { background: '#fef3c7', color: '#92400e' }) }}>
                              {['received','completed'].includes(item.po_info.status) ? '到貨' : '採購中'}
                            </span>
                          )}
                        </div>
                        ...
                        <div style={{ padding: '8px 10px', display: 'flex', gap: 5, justifyContent: 'center', alignItems: 'center' }}>
                          {isEditing ? (
                            <>
                              <button onClick={saveEditItem} style={{ width: 24, height: 24, borderRadius: t.radius.sm, border: 'none', background: '#16a34a', color: '#fff', cursor: 'pointer', fontSize: t.fontSize.h3, fontWeight: t.fontWeight.bold, padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✓</button>
                              <button onClick={cancelEdit} style={{ width: 24, height: 24, borderRadius: t.radius.sm, border: 'none', background: '#ef4444', color: '#fff', cursor: 'pointer', fontSize: t.fontSize.h3, fontWeight: t.fontWeight.bold, padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
                            </>
                          ) : (
                            <>
                              {!cannotEdit && <button onClick={(e) => startEditItem(item, e)} title="編輯" style={{ width: 24, height: 24, borderRadius: t.radius.sm, border: '1px solid #d1d5db', background: '#fff', color: t.color.textMuted, cursor: 'pointer', fontSize: t.fontSize.h3, fontWeight: t.fontWeight.bold, padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✎</button>}
                              {!cannotEdit && !hasPO && <button onClick={(e) => { e.stopPropagation(); setReplacingItemId(replacingItemId === item.id ? null : item.id); setReplaceSearch(''); setReplaceResults([]); }} title="替換" style={{ width: 24, height: 24, borderRadius: t.radius.sm, border: '1px solid #c4b5fd', background: '#f5f3ff', color: '#7c3aed', cursor: 'pointer', fontSize: t.fontSize.h3, fontWeight: t.fontWeight.bold, padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>⇄</button>}
                              {!cannotEdit && !hasPO && <button onClick={(e) => deleteItem(item.id, e)} title="刪除" style={{ width: 24, height: 24, borderRadius: t.radius.sm, border: '1px solid #fecaca', background: '#fef2f2', color: t.color.error, cursor: 'pointer', fontSize: t.fontSize.h3, fontWeight: t.fontWeight.bold, padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>}
                            </>
                          )}
                        </div>
                      </div>
