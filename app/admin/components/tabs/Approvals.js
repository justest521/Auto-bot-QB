    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={onClose}>
      <div style={{ ...S.card, ...(isMobile ? S.mobileModal : {}), width: isMobile ? undefined : 680, maxWidth: '92vw', maxHeight: isMobile ? '85vh' : '80vh', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
        <div style={{ ...(isMobile ? S.mobileModalHeader : {}), padding: isMobile ? '14px 14px' : '14px 18px', borderBottom: `1px solid ${t.color.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontSize: isMobile ? t.fontSize.body : t.fontSize.h2, fontWeight: t.fontWeight.bold, color: t.color.textPrimary }}>{customerName} — 歷史訂單</div>
            <div style={{ fontSize: isMobile ? t.fontSize.tiny : t.fontSize.tiny, color: t.color.textMuted, marginTop: 2 }}>
            ...
            return (
              <div key={o.order_id || idx} style={{ border: `1px solid ${t.color.border}`, borderRadius: t.radius.md, marginBottom: 6, overflow: 'hidden' }}>
                <div style={{ padding: isMobile ? '8px 10px' : '8px 12px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: isMobile ? 8 : 10, background: isOpen ? t.color.bgMuted : '#fff', flexWrap: isMobile ? 'wrap' : 'nowrap' }} onClick={() => setExpandedOrderId(isOpen ? null : (o.order_id || idx))}>
                  <span style={{ ...S.mono, fontSize: isMobile ? t.fontSize.tiny : t.fontSize.tiny, fontWeight: t.fontWeight.bold, color: t.color.textSecondary }}>{o.order_no}</span>
                  <span style={{ fontSize: isMobile ? t.fontSize.tiny : t.fontSize.tiny, color: t.color.textMuted }}>{o.date}</span>
                  <span style={{ fontSize: isMobile ? t.fontSize.tiny : t.fontSize.tiny, color: statusColor, fontWeight: t.fontWeight.semibold }}>{o.status === 'confirmed' ? '已核准' : o.status === 'pending' ? '待處理' : o.status}</span>
                  <span style={{ flex: isMobile ? undefined : 1 }} />
                  <span style={{ ...S.mono, fontSize: isMobile ? t.fontSize.tiny : t.fontSize.caption, fontWeight: 800, color: t.color.success }}>{fmtP(o.amount)}</span>
                  <span style={{ fontSize: isMobile ? t.fontSize.tiny : t.fontSize.tiny, color: t.color.textDisabled }}>{o.items?.length || 0} 品項 {isOpen ? '▲' : '▼'}</span>
                </div>
                {isOpen && o.items && o.items.length > 0 && (
                  <div style={{ borderTop: `1px solid ${t.color.border}`, padding: isMobile ? '6px 10px' : '6px 12px', background: t.color.bgMuted, ...(isMobile ? { overflowX: 'auto', WebkitOverflowScrolling: 'touch' } : {}) }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: isMobile ? t.fontSize.tiny : t.fontSize.tiny }}>
                      <thead>
      ...
      <div style={{ ...S.card, padding: isMobile ? '10px 12px' : '10px 16px', marginBottom: 10, display: 'flex', alignItems: 'center', gap: isMobile ? 12 : 16, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: t.radius.md, background: t.color.warningBg }}>
            <span style={{ ...S.mono, fontSize: isMobile ? t.fontSize.body : t.fontSize.h2, fontWeight: 800, color: t.color.warning }}>{data.pending_count || 0}</span>
            <span style={{ fontSize: isMobile ? t.fontSize.tiny : t.fontSize.tiny, color: t.color.textMuted }}>待審核</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: t.radius.md, background: t.color.infoBg }}>
            <span style={{ ...S.mono, fontSize: isMobile ? t.fontSize.body : t.fontSize.h2, fontWeight: 800, color: t.color.info }}>{data.total}</span>
            <span style={{ fontSize: isMobile ? t.fontSize.tiny : t.fontSize.tiny, color: t.color.info }}>全部</span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {Object.entries(STATUS_MAP).map(([k, v]) => (
            <button key={k} onClick={() => { setStatusFilter(k); load(k); }} style={{ ...S.btnGhost, padding: isMobile ? '3px 8px' : '3px 10px', fontSize: isMobile ? t.fontSize.tiny : t.fontSize.tiny, borderColor: statusFilter === k ? v.color : t.color.border, background: statusFilter === k ? v.color : '#fff', color: statusFilter === k ? '#fff' : v.color }}>{v.label}</button>
          ))}
        </div>
              ...
              return (
                <div key={a.id} style={{ ...S.card, marginBottom: 0, overflow: 'hidden', padding: 0 }}>
                  ...
                  <div style={{ padding: isMobile ? '10px 12px' : '10px 14px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: isMobile ? 10 : 12, flexWrap: isMobile ? 'wrap' : 'nowrap' }} onClick={() => setExpandedId(isExpanded ? null : a.id)}>
                    {/* Status badge */}
                    <span style={{ fontSize: isMobile ? t.fontSize.tiny : t.fontSize.tiny, fontWeight: t.fontWeight.bold, color: '#fff', background: st.color, padding: '3px 8px', borderRadius: t.radius.sm, whiteSpace: 'nowrap', minWidth: isMobile ? 40 : 42, textAlign: 'center' }}>{st.label}</span>

                    {/* Doc info */}
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, flexWrap: 'wrap' }}>
                        <span style={{ fontSize: isMobile ? t.fontSize.tiny : t.fontSize.caption, fontWeight: t.fontWeight.bold, color: t.color.textPrimary }}>{TYPE_MAP[a.doc_type] || a.doc_type}</span>
                    ...
                    <div style={{ display: 'flex', gap: isMobile ? 3 : 4, alignItems: 'center', flexShrink: 0, ...(isMobile ? { width: '100%', marginTop: 8 } : {}) }} onClick={e => e.stopPropagation()}>
                      {a.customer_history && (
                        <button onClick={() => setHistoryModal({ history: a.customer_history, customerName })} style={{ ...S.btnGhost, padding: isMobile ? '4px 6px' : '4px 8px', fontSize: isMobile ? t.fontSize.tiny : t.fontSize.tiny, borderColor: t.color.info, color: t.color.info, minHeight: isMobile ? 44 : undefined }}>{isMobile ? '歷史' : '客戶歷史'}</button>
                      )}
                      <button onClick={() => { const pdfType = a.doc_type === 'purchase_order' ? 'purchase_order' : a.doc_type === 'quote' ? 'quote' : a.doc_type === 'sale' ? 'sale' : 'order'; openPdf(pdfType, a.doc_id); }} style={{ ...S.btnGhost, padding: isMobile ? '4px 6px' : '4px 8px', fontSize: isMobile ? t.fontSize.tiny : t.fontSize.tiny, minHeight: isMobile ? 44 : undefined }}>PDF</button>
                      {a.status === 'pending' && (
                        <>
                          <button onClick={() => handleProcess(a, 'approved')} style={{ ...S.btnPrimary, padding: isMobile ? '4px 10px' : '4px 12px', fontSize: isMobile ? t.fontSize.tiny : t.fontSize.tiny, minHeight: isMobile ? 44 : undefined }}>核准</button>
                          <button onClick={() => { setNoteDialog(a); setNote(''); }} style={{ ...S.btnGhost, padding: isMobile ? '4px 10px' : '4px 12px', fontSize: isMobile ? t.fontSize.tiny : t.fontSize.tiny, borderColor: t.color.error, color: t.color.error, minHeight: isMobile ? 44 : undefined }}>駁回</button>
                        </>
                      )}
                    </div>
                  </div>
                      ...
                      <div style={{ borderTop: `1px solid ${t.color.border}`, padding: isMobile ? '8px 12px' : '8px 14px', background: t.color.bgMuted, ...(isMobile ? { overflowX: 'auto', WebkitOverflowScrolling: 'touch' } : {}) }}>
                            ...
                            {items.map((item, idx) => {
                              ...
                              return (
                                <tr key={idx} style={{ borderBottom: `1px solid ${t.color.bgMuted}` }}>
                                  ...
                                  {hasCost && !isPO && <td style={{ padding: '4px 6px', textAlign: 'right', ...S.mono, color: profit >= 0 ? t.color.brand : t.color.error, fontWeight: t.fontWeight.semibold, fontSize: t.fontSize.tiny }}>{fmtP(profit)} <span style={{ fontSize: t.fontSize.tiny, color: t.color.textDisabled }}>({itemMargin}%)</span></td>}
                                </tr>
                              );
                            })}
                          </tbody>
                        ...
                        {hasCost && !isPO && (
                          <div style={{ marginTop: 8, padding: isMobile ? '8px 8px' : '8px 10px', background: t.color.successBg, border: `1px solid ${t.color.border}`, borderRadius: t.radius.md, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 6, flexDirection: isMobile ? 'column' : 'row' }}>
                            <div style={{ display: 'flex', gap: isMobile ? 8 : 12, fontSize: isMobile ? t.fontSize.tiny : t.fontSize.tiny, color: t.color.textSecondary }}>
                            ...
                            <div style={{ display: 'flex', gap: 8, alignItems: 'baseline' }}>
                              <span style={{ fontSize: isMobile ? t.fontSize.tiny : t.fontSize.tiny, color: t.color.textSecondary }}>毛利</span>
                              <span style={{ ...S.mono, fontSize: isMobile ? t.fontSize.caption : t.fontSize.h2, fontWeight: 800, color: totalProfit >= 0 ? t.color.brand : t.color.error }}>{fmtP(totalProfit)}</span>
                              <span style={{ ...S.mono, fontSize: isMobile ? t.fontSize.tiny : t.fontSize.tiny, fontWeight: t.fontWeight.bold, color: totalProfit >= 0 ? t.color.brand : t.color.error, background: totalProfit >= 0 ? t.color.successBg : t.color.errorBg, padding: '1px 6px', borderRadius: t.radius.sm }}>{marginPct}%</span>
                            </div>
                          </div>
                        )}
                      </div>
                  ...
                  {a.status !== 'pending' && (
                    <div style={{ borderTop: `1px solid ${t.color.border}`, padding: isMobile ? '8px 12px' : '8px 14px', background: a.status === 'approved' ? t.color.successBg : t.color.errorBg, display: 'flex', gap: 8, alignItems: 'center', fontSize: isMobile ? t.fontSize.tiny : t.fontSize.tiny, flexWrap: 'wrap' }}>
      ...
      {noteDialog && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ ...S.card, ...(isMobile ? S.mobileModal : {}), width: isMobile ? undefined : 400, maxWidth: '90vw' }}>
            <h3 style={{ margin: '0 0 10px', fontSize: isMobile ? t.fontSize.body : t.fontSize.h2 }}>駁回原因</h3>
